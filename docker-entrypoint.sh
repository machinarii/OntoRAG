...
# Run the server as the non-root "ontorag" user (CIS Docker 4.1) while staying
...
if [ "${1#-}" != "$1" ]; then
    set -- python -m ontorag.api.ontorag_server "$@"
fi
...
if [ "$(id -u)" = "0" ]; then
    ...
    # handing it to ontorag avoids that PermissionError.
    #
    ...
    for _d in /app/data "$WORKING_DIR" "$INPUT_DIR" "$PROMPT_DIR" \
        ...
        chown -R ontorag:ontorag "$_d" 2>/dev/null || true
    done
