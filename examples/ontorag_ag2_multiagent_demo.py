"""OntoRAG + AG2 Multi-Agent Demo.

Demonstrates how AG2 agents can use OntoRAG's knowledge graph retrieval
as a tool. Multiple specialized agents collaborate to answer complex
questions over indexed documents.

Architecture:
    User -> AG2 GroupChat (Researcher + Analyst + Writer) -> OntoRAG queries
    - Researcher: uses OntoRAG hybrid search to gather facts
    - Analyst: uses OntoRAG naive (vector) search for complementary results
    - Writer: synthesizes findings into a final answer

Requires:
    pip install ontorag-hku "ag2[openai]>=0.11.4,<1.0"
    export OPENAI_API_KEY="..."

Usage:
    python examples/ontorag_ag2_multiagent_demo.py
"""
...
OntoRAG implements this approach with dual-level retrieval: local search focuses
...
async def setup_ontorag() -> OntoRAG:
    """Initialize OntoRAG and index sample documents."""
    ...
    print("OntoRAG initialized and documents indexed.\n")
    return rag


# --- Async Bridge ---
# AG2 runs tools in a background thread without an event loop.
# We maintain a dedicated event loop in a separate thread for OntoRAG async calls.
...
# Global reference to OntoRAG instance (set in main)
_rag_instance: OntoRAG = None


def create_agents():
    """Create AG2 agents with OntoRAG tools."""
    ...
    researcher = AssistantAgent(
        name="Researcher",
        system_message=(
            "You are a research specialist. Use the ontorag_query tool to search "
    ...
    analyst = AssistantAgent(
        ...
        system_message=(
            ...
            "the ontorag_query tool with mode='naive' to run a direct vector search. "
    ...
    # --- Register OntoRAG as a tool ---

    @user_proxy.register_for_execution()
    @researcher.register_for_llm(
        description=(
            "Query the OntoRAG knowledge base. "
    ...
    def ontorag_query(query: str, mode: str = "hybrid") -> str:
        """Query OntoRAG synchronously (wraps async call)."""
...
def run_multiagent_query(user_proxy, researcher, analyst, writer, question: str):
    """Run a multi-agent GroupChat to answer a question using OntoRAG."""
...
def main():
    ...
    # Start a background event loop for OntoRAG async calls.
    ...
    try:
        # Step 1: Set up OntoRAG (async, runs on the background loop)
        _rag_instance = _run_async(setup_ontorag())

        # Step 2: Create AG2 agents with OntoRAG tools
