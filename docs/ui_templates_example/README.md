...
- `brand.logo` is REQUIRED: a path, or an explicit `null` for "no logo".
  ...
  the OntoRAG logo under customer texts).
...
- `brand.copyright` is OPTIONAL plain text (written in the manifest, not a
  ...
  mean the same thing: no copyright line at all — OntoRAG ships no default
  text, so an uncustomized deployment shows none and OntoRAG's own notice
