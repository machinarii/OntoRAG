...
Current architecture for OntoRAG's knowledge graph and retrieval-augmented generation. Covers data model, storage backends, extraction pipeline, query modes, and context assembly. File:line citations point to the source of truth — read the code for full detail.
...
```
            ┌──────────────────────────────────────────────────────┐
            │                    OntoRAG class                    │
            │     (ontorag.py — composed from mixins, @final)     │
            └────────────────────────┬─────────────────────────────┘
                                     │
                  ┌──────────────────┼─────────────────────┐
                  ▼                  ▼                     ▼
...
The four storage types are coordinated by `OntoRAG` and accessed by both the ingestion pipeline (writes) and the query operations (reads). All instances honor a `workspace` parameter for tenant/dataset isolation.

## 2. Storage Layer

### 2.1 Storage Types

| Type | Abstract base (`ontorag/base.py`) | Used for |
|------|------------------------------------|----------|
...
Resolution lives in `ontorag/kg/__init__.py` (`STORAGE_IMPLEMENTATIONS`, `STORAGES`, `STORAGE_ENV_REQUIREMENTS`) and `ontorag/kg/factory.py` (`get_storage_class`). The default JSON/NetworkX/NanoVectorDB backends are imported eagerly; all other backends load lazily via `importlib.import_module`.
...
`ontorag/kg/networkx_impl.py:24-570`
...
`ontorag/utils_graph.py` exposes higher-level operations layered on top of `BaseGraphStorage` + the entity/relation vector DBs. All of these use `get_storage_keyed_lock` to keep graph + vdb writes atomic per entity/relation.
...
Entry point: `_PipelineMixin` in `ontorag/pipeline.py` (see the concurrency contract in `AGENTS.md`). For each chunk the extractor in `ontorag/operate.py` performs:
...
1. **LLM extraction** (`extract_entities` ≈ `operate.py:3232`, `_process_single_content` ≈ `:3320`)
   ...
   - Calls the configured extraction LLM with prompts from `ontorag/prompt.py` (`entity_extraction_system_prompt`, `entity_extraction_user_prompt`, few-shot `entity_extraction_examples`).
...
`OntoRAG.aquery` (`ontorag.py:1699`) is a back-compat wrapper around `aquery_llm` (`ontorag.py:1988-2051`):

```
mode in {local, global, hybrid, mix} → kg_query()        (operate.py:3659)
mode == "naive"                      → naive_query()     (operate.py:5575)
mode == "bypass"                     → direct LLM call   (ontorag.py:2010)
```
...
Defined in `ontorag/base.py:85-178`. Key fields:
...
`ontorag/rerank.py:182-281` and backends `cohere_rerank` (`:368`), `jina_rerank` (`:435`), `ali_rerank` (`:475`). Applied inside `process_chunks_unified` from `_build_context_str` (`:4764`). **Only text chunks are reranked** — entities and relations rely on their vdb similarity and graph degree, not the reranker.

### 4.6 Prompt Templates

`ontorag/prompt.py` (all under `PROMPTS[…]`):
...
- **Pinned version** — release dated 2020-02-24, files committed under `/Users/jin/OntoRAG/yago/`, SHA256s pinned in `ontorag/taxonomy/manifest.py`. The build CLI calls `verify_yago_files()` before parsing.
...
**Class field on nodes — `list[str]`, not pipe-joined string.** Stored as a native list of class IRIs ordered leaf → root. Native list type in Neo4j / Memgraph / PostgreSQL (AGE) / MongoDB / OpenSearch; **NetworkX backend (`ontorag/kg/networkx_impl.py`) requires a JSON-encode/decode shim** because GraphML cannot round-trip list-valued attributes. Shim is isolated to `_encode_node()` / `_decode_node()` in that one file; the `BaseGraphStorage` contract stays `list[str]` for every consumer. Rationale: indexed membership queries (`"all entities under class X"`) are O(log n) on real backends with native lists, vs. O(n) substring scans on a pipe-string. See §2 of the design discussion for the full per-backend tradeoff.

**Document-level fields** (added to `doc_status` rows — see `ontorag/kg/json_doc_status_impl.py` and equivalents):
...
- Fallback sentinel: `ontorag:Uncategorized` — documents that don't fit any YAGO class. Always populated (never an empty list) so filter UX doesn't break.
...
4. **Threshold rule**: always keep the top-scored class. Keep additional classes only if their score is ≥ 50% of the top score, up to 10 total. If no class scores above an absolute minimum (e.g. 0.3), assign `ontorag:Uncategorized`.
...
**Images as classification inputs (shipped ahead of Plan B).** The VLM image analysis (`prompt_multimodal.py` `image_analysis`) now returns two extra fields alongside `name` / `type` / `description`: `subject` — what the image is *about* (as opposed to `type`, its presentational medium: Chart / Photo / …) — and `ocr_text` — the verbatim text baked into the pixels (`""` when none). Both are persisted in the sidecar `llm_analyze_result` and rendered into the multimodal chunk as `[Image Subject]…` and `[Image Text]…` lines, so entity extraction already sees OCR'd names and axis labels. The chunk `sidecar` block also carries `image_type`, and `extract_entities` uses it to type the multimodal graph node by medium (`chart`, `flowchart`, …) instead of the literal `drawing`. The classifier itself is **not** yet called on images; when Plan B wires `DocumentClassifier` in, `name + subject + ocr_text + description` is the intended per-image input to `classify()`. Open decision for Plan B: whether an image's YAGO classes roll up into the document's assignment — images are the noisiest signal in a document (logos, stock photos, icons), so an unweighted roll-up would pollute document-level classes; the options are to exclude images or admit them at a discounted score under the §5.4 threshold rule.

**Document-level inputs from the pdf2md engine (shipped ahead of Plan B).** Every document converted by `pdf2md` (PDF/EPUB/DOCX/DOC/ODT/RTF → `.textpack`) now carries `doc_status.metadata.bibliographic` (title, authors, year, publisher, edition, isbn, arxiv, language — only what the source declared) and `doc_type` (`book` / `paper` / `deck` / `document`, with `doc_scores`). These are the intended document-level inputs for the classification step above: `bibliographic.title` + `doc_type` are a far stronger and cheaper signal for the candidate-class retrieval than the first N tokens of body text, and `doc_type` can gate the candidate set (a `deck` is rarely a `Novel`). Plan B should prefer them when present and fall back to body text otherwise.

**Classification capabilities shipped ahead of Plan B (paperless-ngx lessons).** The standalone stack now offers three things the ingestion hook should use: (1) `neighbor_class_candidates()` — the classes of the k most similar *already-classified* documents, voted by similarity and merged with the class-index candidates, so prior decisions (human or accepted LLM) steer new ones and the candidate set improves with corpus size; (2) a two-step prompt in which the model first names categories freely and then reconciles them to allow-listed candidates, with the leftovers returned as `unmatched_names` by `classify_detailed()` — this is the signal for vocabulary overlays that the §5.8 coverage gate needs, instead of a silent `Uncategorized`; (3) `NaiveBayesClassPrior`, a numpy-only prior fitted on confirmed assignments that re-ranks candidates and, above `DEFAULT_PRIOR_SKIP_THRESHOLD`, skips the LLM call. Plan B decides what "confirmed" means (a WebUI correction, or an LLM result an operator accepted) and where the prior is persisted.
...
**`QueryParam` additions** (`ontorag/base.py:85-178`):
...
**No changes to dispatch** in `aquery_llm` (`ontorag.py:1988-2051`) or `_perform_kg_search` (`operate.py:4195`). The enrichment is data-shaped — same code paths, richer payloads flowing through them.
...
- Storage interfaces: `ontorag/base.py`
- Backend registry & resolution: `ontorag/kg/__init__.py`, `ontorag/kg/factory.py`
- Reference graph backend: `ontorag/kg/networkx_impl.py`
- Graph CRUD utilities: `ontorag/utils_graph.py`
- Extraction + query operations: `ontorag/operate.py`
- Ingestion pipeline: `ontorag/pipeline.py` (and concurrency contract in `AGENTS.md`)
- Prompts: `ontorag/prompt.py`
- Reranking: `ontorag/rerank.py`
- Query entry: `OntoRAG.aquery` / `aquery_llm` in `ontorag/ontorag.py`
