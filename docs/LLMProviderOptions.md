...
described in [OntoRAG Server and WebUI](./OntoRAG-API-Server.md) — nor role-level
...
`ontorag/llm/binding_options.py`. This document is intentionally English-only: it is a
...
- The default values visible in `ontorag/llm/binding_options.py` (and in the generated
  `.env` sample of §12) are **documentation of the provider's typical default, not
  values OntoRAG ships**. `OLLAMA_LLM_THINK` shows `True` and
  `OPENAI_LLM_REASONING_EFFORT` shows `medium`, yet neither is sent unless you set it.
- `ontorag-server --llm-binding openai --help` therefore prints no default values.
...
`EMBEDDING_BINDING` (`ontorag/api/config.py`). With `LLM_BINDING=ollama`, every
...
| `lollms` | see note | none | **Base level: no options at all.** `ontorag-server --llm-binding lollms --help` prints no option group, and the base binding is wired with an empty options payload. Role level maps lollms to the Ollama option set, so `{ROLE}_OLLAMA_LLM_*` is read — but the lollms driver only consumes `temperature`, `top_k`, `top_p`, `repeat_penalty`, `repeat_last_n`, and `seed`; every other field is ignored. |
...
| `OPENAI_LLM_REASONING_EFFORT` | str | Reasoning effort for reasoning-capable models. Forwarded verbatim, so the accepted words are the provider's, not OntoRAG's — OpenAI currently accepts `minimal`/`low`/`medium`/`high`, other gateways add their own (e.g. `none`). |
...
- These options are merged into the Chat Completions request as keyword arguments and
  forwarded as-is. OntoRAG does not validate them against the target model, so a
...
| Environment variable | Type | Description |
| --- | --- | --- |
| `OLLAMA_LLM_NUM_CTX` | int | Context window size in tokens. Must be larger than `MAX_TOTAL_TOKENS + 2000`; `env.example` ships it set to `32768` because the Ollama default is usually too small for OntoRAG's prompts. |
...
- Reasoning levels (`low`/`medium`/`high`) additionally require an Ollama server that
  supports levels; `think` in any form requires `ollama>=0.5.4` on the OntoRAG side,
...
> [OntoRAG Server and WebUI](./OntoRAG-API-Server.md).
...
# OntoRAG; check that model's Converse API documentation. Example shape:
...
ontorag-server --llm-binding bedrock --help
...
```bash
python -m ontorag.llm.binding_options
```

Both outputs are generated from `ontorag/llm/binding_options.py`, so they are always
current. Remember that the values shown in the generated `.env` sample are the
providers' typical defaults, not values OntoRAG sends (§1.2) — the lines are commented
