...
This document explains how to run multiple isolated OntoRAG instances behind one host using a reverse proxy (nginx, Traefik, Kubernetes Ingress, …), with **one shared WebUI build** reused by every instance.
...
- Set `ONTORAG_API_PREFIX` per-instance, on the **backend only**. The WebUI is always mounted at `/webui` (not configurable).
...
docker run -e ONTORAG_API_PREFIX=/site01 -p 9621:9621 ontorag:latest
docker run -e ONTORAG_API_PREFIX=/site02 -p 9622:9621 ontorag:latest
```
...
Earlier versions of OntoRAG baked the site prefix into the JavaScript bundle at build time (via `VITE_API_PREFIX` / `VITE_WEBUI_PREFIX`). Every site that used a different prefix needed its own WebUI build, and reusing a single Docker image across sites required a rebuild step at deploy time. Since the runtime-config-injection refactor:
...
- **API base URL** and **in-app links** read their prefix from `window.__ONTORAG_CONFIG__`, which the FastAPI server injects into `index.html` on each response based on its own `ONTORAG_API_PREFIX`.

The result: a single `ontorag/api/webui/` directory (or Docker image) is reusable across any number of sites with no per-site build artifact.

---

## How runtime prefix injection works

Each request for `index.html` goes through `SmartStaticFiles` in `ontorag/api/ontorag_server.py`, which:
...
services:
  site01:
    image: ghcr.io/hkuds/ontorag:latest
    env_file: site01.env
    volumes:
...
WORKDIR /src/ontorag_webui
COPY ontorag_webui/package.json ontorag_webui/bun.lock ./
RUN bun install --frozen-lockfile
COPY ontorag_webui/ ./
COPY ontorag/api/webui/.gitkeep /src/ontorag/api/webui/.gitkeep
RUN bun run build

FROM python:3.11-slim
COPY --from=webui-build /src/ontorag/api/webui /app/ontorag/api/webui
# … rest of the image …
```
...
docker run --rm -p 9621:9621 ontorag:latest

# Same image, different prefixes — runtime decides.
docker run --rm -e ONTORAG_API_PREFIX=/site01 -p 9621:9621 ontorag:latest
docker run --rm -e ONTORAG_API_PREFIX=/site02 -p 9622:9621 ontorag:latest
```
...
Backends still set `ONTORAG_API_PREFIX=/site01` / `=/site02`.
...
>
> This is the deliberate consequence of `base: './'` in [`vite.config.ts`](../ontorag_webui/vite.config.ts) — the same setting that makes one production build reusable across any number of reverse-proxy mount points. Tying the dev URL to a prefix would force the build to bake the prefix back in.

The dev server mirrors production injection: it serves `index.html` via the same `transformIndexHtml` mechanism the FastAPI server uses at request time, so the SPA reads `window.__ONTORAG_CONFIG__` in dev exactly the way it does in prod. Only **two** environment variables matter:

| Variable | Purpose | Where it lives |
| --- | --- | --- |
| `VITE_BACKEND_URL` | Where the dev server forwards proxied API calls. | `ontorag_webui/.env*` |
| `VITE_DEV_API_PREFIX` | Prefix to **simulate** (matches the backend ONTORAG_API_PREFIX`). Empty → no prefix. | `ontorag_webui/.env*` |

`VITE_DEV_API_PREFIX` injects `apiPrefix` into `window.__ONTORAG_CONFIG__` in the browser, mirroring the backend behavior. It also serves as a prefix for `VITE_API_ENDPOINTS`, ensuring correct access to backend APIs. The matching `webuiPrefix` is derived as `${VITE_DEV_API_PREFIX}/webui/` automatically — you don't need a separate variable for it.
...
Run:
```bash
ontorag-server                  # in one terminal, no ONTORAG_API_PREFIX
cd ontorag_webui && bun run dev # in another; open http://localhost:5173/
```

### Scenario 2 — simulate a site prefix

You want the SPA to run under `/site01` (or whatever production prefix). Set `VITE_DEV_API_PREFIX=/site01`. Vite injects the matching `window.__ONTORAG_CONFIG__` and registers prefixed proxy keys; SPA requests like `fetch("/site01/documents/foo")` are forwarded verbatim to whatever `VITE_BACKEND_URL` points at. The upstream — local backend or production nginx — is responsible for understanding the prefix.
...
**A. Local backend with `ONTORAG_API_PREFIX=/site01`** (no nginx anywhere) — the simplest setup, two processes on your laptop. Vite's proxy itself plays the role of the reverse proxy.
...
```bash
# Terminal 1
ONTORAG_API_PREFIX=/site01 ontorag-server
# Terminal 2
cd ontorag_webui && bun run dev
```
...
- **Auth required**: if the upstream requires `ONTORAG_API_KEY`, log in via the dev SPA exactly as you would in prod — the auth token flows through the proxy unchanged.
...
| 2A. Simulate a prefix locally (no nginx) | `http://localhost:9621` | `/site01` | local backend with `ONTORAG_API_PREFIX=/site01` | `http://localhost:5173/` |
...
Confirm the backend is also running with the matching `ONTORAG_API_PREFIX`. The dev proxy forwards prefixed paths verbatim; if the backend has no prefix configured, it does not register routes under that path.
