...
The OntoRAG project includes a React-based WebUI frontend. This guide explains how frontend building works in different scenarios.
...
- Frontend build output goes to `ontorag/api/webui/`
...
# Output: dist/ontorag_hku-*.whl and dist/ontorag_hku-*.tar.gz
```
...
|----------|---------|-------------------|-----------------|
| From PyPI | `pip install ontorag-hku[api]` | Included | No (already installed) |
...
```
OntoRAG/
├── ontorag_webui/          # Frontend source code
│   ├── src/                 # React components
│   ├── package.json         # Dependencies
│   └── vite.config.ts       # Build configuration
│       └── outDir: ../ontorag/api/webui  # Build output
│
├── ontorag/
│   └── api/
...
└── .gitignore               # Excludes ontorag/api/webui/* (except .gitkeep)
```
...
- Or reinstall: `pip uninstall ontorag-hku && pip install ".[api]"`

### Q: Where are the built frontend files?

**A:** In `ontorag/api/webui/` after running `bun run build`
