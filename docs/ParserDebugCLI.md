...
## Equivalence with the `OntoRAG.parse_*` Production Path

This CLI directly calls the production code paths `OntoRAG.parse_native` / `parse_mineru` / `parse_docling` (via the lightweight RAG stand-in in `ontorag/parser_debug.py`), so:
