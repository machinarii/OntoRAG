...
- The browser tab title and the login card's heading and subtitle. The manifest cannot set them: `WEBUI_TITLE` supplies both the browser tab title and login heading (falling back to `OntoRAG`), while the subtitle remains the localized `login.description` in `LoginPage.tsx`.
...
**All-or-nothing per locale.** A visitor either sees your bundle's representation of a locale *entirely*, or OntoRAG's built-in branding *entirely*. Fields are never mixed one by one, so you cannot supply only a logo and inherit OntoRAG's welcome text.
...
cp -r docs/ui_templates_example ontorag_webui/ui_templates

# Edit the texts and drop in your own logo (PNG / JPEG / WebP / SVG all work, see §4.4)
$EDITOR ontorag_webui/ui_templates/locales/en/welcome.md
cp /path/to/your-logo.svg ontorag_webui/ui_templates/assets/logo.svg
```

Then add to `.env`:

```ini
UI_TEMPLATES_DIR=./ontorag_webui/ui_templates
```
...
docker compose up -d --force-recreate ontorag
```
...
| `brand.logo` | yes | string \| `null` | Path to the default logo, or an explicit `null` for "this deployment shows no logo". **Omitting the key fails startup** — a missing logo must never silently fall back to the OntoRAG logo underneath your texts. PNG, JPEG, WebP and SVG are all accepted — **detected from the file's bytes, not its extension**, see [§4.4](#44-limits-and-formats). |
| `brand.copyright` | no | string \| `null` | The copyright line for every locale, as **plain text written in the manifest** (not a Markdown file). Omitted, `null`, empty or whitespace-only all mean the same thing: **no line is shown**. There is no OntoRAG default to inherit — see [§4.5](#45-the-copyright-line). |
...
The copyright line is **your deployment's own legal statement**, so OntoRAG never writes one for you: without a bundle — or with a bundle that says nothing about copyright — the pages show no copyright line at all, and OntoRAG's own notice is never printed on your pages.
...
|---|---|---|
| From source | `ontorag_webui/ui_templates/` (git-ignored) | `./ontorag_webui/ui_templates` |
...
The leniency stops at the manifest's edge. Once `manifest.json` exists, the bundle is validated in full and any problem in it refuses startup ([§9](#9-troubleshooting)) — a half-copied bundle never quietly serves OntoRAG content.
...
The wizard also seeds `UI_TEMPLATES_DIR: "/app/data/ui_templates"` into the `ontorag` service's `environment:` block, so a wizard-generated deployment behaves exactly like the shipped compose files: inert until `./data/ui_templates` holds a bundle.
...
```bash
kubectl create configmap ontorag-ui-templates \
...
There is **no hot reload**. The whole bundle is validated once at startup and activated as an immutable in-memory snapshot; request handling never touches the disk again. After editing the bundle, restart the server — and with `ontorag-gunicorn` or any multi-worker setup, restart **all** workers, or some of them will keep serving the previous revision.
...
WARNING: UI customization: UI_TEMPLATES_DIR=/app/data/ui_templates holds no manifest.json — serving the built-in OntoRAG branding. …
...
- `"customized": false` → no bundle is active (`UI_TEMPLATES_DIR` unset, or pointed at a directory with no `manifest.json`). The frontend is showing OntoRAG's built-in branding.
...
- `logo_url: null` → this locale resolves to *no* logo (an explicit `null` somewhere), not to the OntoRAG logo.
...
Once the configured directory contains a `manifest.json`, anything invalid in the bundle makes the server **refuse to start** with a message beginning `UI_TEMPLATES_DIR bundle invalid:`. That is deliberate: silently falling back to OntoRAG content would leave you believing customer branding is live when it is not.
...
|---|---|
| `directory '…' does not exist or is not a directory` | Wrong path, or the container mount is missing. Check it from inside the container: `docker compose exec ontorag ls /app/data/ui_templates`. |
...
| Symptom | Cause |
|---|---|
| Server starts, but the page still shows OntoRAG branding | `UI_TEMPLATES_DIR` is unset — check the log line and `/ui/customization`. In Docker, remember a compose `environment:` entry overrides `.env`. |
| Startup logs `holds no manifest.json` and the branding is unchanged | The configured directory exists but has no bundle in it. Either you have not written one yet, or the server is reading a different directory than you populated — the warning names the path it read. `manifest.json` must sit directly in the bundle root, not one level down (a common cause is copying the parent directory). Check it from inside the container: `docker compose exec ontorag ls /app/data/ui_templates`. |
...
| Copyright line does not show | Neither `brand.copyright` nor the resolved locale's `copyright` declares text (or one of them is `null` / empty). Check `brand.copyright` in the endpoint response for the locale you are looking at. There is no OntoRAG default: no declaration means no line ([§4.5](#45-the-copyright-line)). |
...
- [OntoRAG-API-Server.md](./OntoRAG-API-Server.md) — the full server guide, including `AUTH_ACCOUNTS` and `TOKEN_SECRET`.
