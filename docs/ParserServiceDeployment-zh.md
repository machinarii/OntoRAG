...
MinerU 与 Docling 都是**外部服务**：OntoRAG 通过 HTTP 与它们通信，不会在进程内运行它们的模型。只有当你把文件路由到 `mineru` 或 `docling` 引擎、并且希望自行搭建该服务（而不是使用现成的服务端点）时，才需要本文档。

本文档内容全部是这两个上游项目的容器侧配置，其中没有任何 OntoRAG 环境变量。OntoRAG 侧的配置（哪个扩展名交给哪个引擎、服务端点、凭据、引擎参数）请见 [FileProcessingPipeline-zh.md](./FileProcessingPipeline-zh.md)。
...
在基础部署之上，建议为本地 MinerU 额外开启两项 MinerU **服务端**功能。这两项都改的是 MinerU 容器侧配置（容器内 `mineru.json` 与官方 `compose.yaml`），不涉及 OntoRAG 的 env 变量；其中标题层级修正还需要一个可用的 LLM API。
...
**步骤1：导出并修改 `mineru-ontorag.json`**

从官方镜像中把 `/root/mineru.json` 拷到宿主机当前目录的 `mineru-ontorag.json`（用固定容器名 `temp_mineru`，无需运行容器）：
...
然后修改 `mineru-ontorag.json` 中的 `llm-aided-config.title_aided`：填入 `api_key`，并把 `enable` 改为 `true`：
...
在 `mineru-api` 服务上做三处改动：`environment` 增加 `MINERU_TOOLS_CONFIG_JSON`（让 MinerU 读改过的配置而非镜像内置 `mineru.json`），`volumes` 把宿主机 `mineru-ontorag.json` 挂进容器，`command` 追加 `--enable-vlm-preload true` 开启 vLLM 预加载。改好后的完整 `mineru-api` profile 如下（以 `# <-- 新增` 标注三处增量）：
