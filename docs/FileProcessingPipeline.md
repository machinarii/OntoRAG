...
Each comma-separated item of `ONTORAG_PARSER` is one routing rule, matched against the file extension from left to right; the first usable rule wins. Taking `*:native-iteP,*:legacy-R` apart:
...
**If it failed:** the source file **stays in `INPUT_DIR`**, so you can fix the configuration and scan again. `error_msg` on the doc-status record is the diagnosis; §10 maps the common ones. Note that both the engine and the processing options are frozen at enqueue time — changing `ONTORAG_PARSER` or a hint does not affect an existing record, so re-running with different settings means deleting the document and uploading it again (§9.3). To reproduce a parse offline without the server, use `python -m ontorag.parser.cli` ([ParserDebugCLI.md](./ParserDebugCLI.md)).
...
Processing options control, on a per-file basis, the behavior with respect to multimodal analysis, knowledge graph construction, and text chunking. They can be set as per-rule defaults in `ONTORAG_PARSER` (see [§2.4](#24-default-rules-ontorag_parser)) or overridden for an individual file via a filename hint (see [§2.5](#25-single-file-override-filename-hints)). All options are optional; defaults are shown in the table below. At most one chunking method (F/R/V/P/C) is specified per file; the other options can be combined arbitrarily.
...
### 2.4 Default Rules: `ONTORAG_PARSER`
...
A chunk-strategy selector (`F` / `R` / `V` / `P` / `C`) — in a `ONTORAG_PARSER` rule or a filename hint — may carry per-strategy chunking parameters in parentheses. Inside the parentheses a comma **only** separates parameters; rule splitting is parenthesis-aware, so this comma is never mistaken for a rule separator (both `;` and `,` remain valid rule separators, but `;` is recommended).

```text
notes.[-R(chunk_ts=800,chunk_ol=80)].md                            # filename hint
ONTORAG_PARSER=pdf:legacy-R(chunk_ts=800,chunk_ol=80);*:legacy-R  # rule
```
...
- Validation is strict both at startup (`ONTORAG_PARSER`) and at upload (filename hint): an unknown parameter, a wrong type, an out-of-range value, or a parameter on a strategy that does not support it (e.g. `chunk_ol` on `V`) all raise a friendly error.

The text APIs expose the same path as `chunking.strategy="custom"`. Its `params` object uses the complete fixed-token/legacy contract (`chunk_token_size`, `chunk_overlap_token_size`, `split_by_character`, and `split_by_character_only`). `/documents/text` and `/documents/texts` return 422 unless `OntoRAG.chunking_func` was replaced with a non-default callback.
...
For both external engines the row above is the **baseline** set — what the engine handles out of the box. Their remaining input formats (legacy Office `doc` / `xls` / `ppt`, and for docling also ODF, EPUB, AsciiDoc, LaTeX, CSV, …) depend on components installed on the *service* side rather than in OntoRAG — legacy Office conversion requires LibreOffice there — and for MinerU also on the active `MINERU_API_MODE`. They are therefore not advertised globally: declare what your own deployment can actually handle with `MINERU_ADDITIONAL_SUFFIXES` / `DOCLING_ADDITIONAL_SUFFIXES` (see each engine's section below and the MinerU / Docling blocks in [env.example](https://github.com/machinarii/OntoRAG/blob/main/env.example)), then route those suffixes with a `ONTORAG_PARSER` rule or a per-file hint — declaring a suffix alone does not make a bare `x.doc` uploadable.
...
`legacy` is the fallback engine for every extension except `.textpack`, which the routing layer always sends to `native`. It is also what you get with `ONTORAG_PARSER` unset. It extracts plain text only, which has four consequences worth knowing before you route anything to it:
...
- **It has no raw cache directory**, so `ONTORAG_FORCE_REPARSE_*` does not apply to it (§3.7).
...
`native` is OntoRAG's built-in structured content extractor that runs **fully locally**: it does not depend on external services such as MinerU / Docling, the extraction stage never calls a VLM, and it works out of the box with no deployment. Its runtime dependencies are only `python-docx` + `defusedxml` (required); the markdown path additionally relies on the **optional** `cairosvg` for SVG rasterization (when missing, the SVG is skipped with a warning and the rest of the content is unaffected). Enabling the opt-in `smart_heading` engine parameter for docx additionally requires the pinned `zh_core_web_sm` / `en_core_web_sm` spaCy models (the `spacy` runtime ships with the `api` extra; install the models with `ontorag-download-cache --spacy-install` — the main Docker image already bundles them); deployments that never enable it need no models, and the smart_heading path also calls the EXTRACT-role LLM during parsing. Setting the `DOCX_SMART_HEADING=true` env var enables smart_heading by default for `.docx` files that resolve to the native engine — an explicit `native(smart_heading=false)` rule/hint opts a file back out — and makes the server verify the spaCy models at startup (fail fast instead of failing on the first parse); the same startup check triggers when a `ONTORAG_PARSER` rule carries `native(smart_heading=true)` (or its flag shorthand `native(smart_heading)`). The default applies at upload time only: already-ingested documents keep their persisted engine parameters on re-parse.

Supported extensions: `docx` / `md` / `textpack`. How to enable:

- `docx` and `md` still default to `legacy`; select native explicitly, e.g. a default rule `ONTORAG_PARSER=docx:native` / `ONTORAG_PARSER=md:native`, or a filename hint `report.[native-iet].docx` / `notes.[native].md` (syntax in [§2.4](#24-default-rules-ontorag_parser) / [§2.5](#25-single-file-override-filename-hints)).
...
- `ONTORAG_FORCE_REPARSE_NATIVE` (default `false`): discard the `.native_raw/` cache and re-download external images over the network.
...
The OntoRAG document processing pipeline supports MinerU as a document parser and offers two MinerU access modes:

- `official` mode: uses MinerU's cloud API v4 service. You need to register an account at the [MinerU official website](https://mineru.net/) and create an API-KEY first. Then add the following configuration to OntoRAG's `.env` file:
...
* `local` mode: uses a locally deployed MinerU service, see [ParserServiceDeployment.md §1](./ParserServiceDeployment.md#1-local-deployment-of-the-mineru-service). After the local MinerU service is started, add the following configuration to OntoRAG's `.env` file:
...
| `ONTORAG_FORCE_REPARSE_MINERU` | `false` | Bypass the raw cache and re-upload on every parse (§3.7) |
...
- It only makes the suffix **routable to docling**; it does not by itself make a bare `x.doc` uploadable. Pair it with a routing rule (`ONTORAG_PARSER=doc:docling`) or a per-file hint (`x.[docling].doc`) — otherwise such files still fall through to the default `legacy` engine and are rejected as an unsupported suffix. Conversely, a rule like `doc:docling` without this env fails startup validation, since `doc` is not among docling's capabilities.
...
ONTORAG_PARSER=pdf:mineru(language=en);*:legacy-R                       # rule
```
...
- Engine parameters are only accepted by engines that declare them (`mineru` / `docling` / `native`); attaching a parameter to `legacy`, or an unknown parameter to any engine, is a friendly error. Validation runs at startup (`ONTORAG_PARSER`) and at upload.
...
| --- | --- | --- | --- |
| `native` | `<base>.native_raw/` | external images downloaded by the markdown / textpack path | `ONTORAG_FORCE_REPARSE_NATIVE=true` |
| `mineru` | `<base>.mineru_raw/` | the artifact bundle returned by the MinerU service | `ONTORAG_FORCE_REPARSE_MINERU=true` |
| `docling` | `<base>.docling_raw/` | the artifact bundle returned by docling-serve | `ONTORAG_FORCE_REPARSE_DOCLING=true` |
...
**Install and route.** Initialize the reference with `git submodule update --init external/pdf2md`, or set `PDF2MD_REPO_PATH` to a separate checkout of [machinarii/pdf2md](https://github.com/machinarii/pdf2md). See [reference setup](Pdf2MdIntegration.md). Then install dependencies: `pip install 'ontorag[pdf2md]'` (PyMuPDF is AGPL-3.0; install the extra only where that licence is acceptable). For scanned PDFs the host also needs `tesseract` and `gs` (Ghostscript); for DOC/ODT/RTF, LibreOffice (`soffice` on `PATH`, or `PDF2MD_SOFFICE`). Then route the suffixes you want, e.g. `ONTORAG_PARSER=pdf:pdf2md-iteP,epub:pdf2md-iteP,doc:pdf2md-iteP,*:native-teP,*:legacy-R`. A rule that names `pdf2md` requires the extra and converter checkout to be installed (startup validation refuses otherwise, like an unconfigured external engine). `docx` is deliberately left on `native` in every shipped example: pdf2md's DOCX reader ignores tracked changes, comments and text boxes, while `native` extracts embedded images, tables and equations directly.
...
| Situation | Outcome |
| --- | --- |
| Extra or converter checkout not installed | Engine unavailable: per-file routing skips the rule; a `ONTORAG_PARSER` rule naming it fails startup validation with `pip install 'ontorag[pdf2md]'`. |
...
**What the image analysis returns.** For every drawing the VLM produces one JSON object: `name` (a short distinctive name), `type` (the presentational *medium*, one of `Photo / Illustration / Screenshot / Icon / Chart / Table / Infographic / Flowchart / Chat Log / Wireframe / Texture / Other`), `subject` (what the image is *about* — its subject matter, e.g. `Acme Corp quarterly revenue, FY2025`), `ocr_text` (the verbatim text baked into the pixels: titles, axis labels, legends, node labels; `""` when there is none) and `description` (interpretive prose). Like an out-of-enum `type` folding to `Other`, a missing `subject` folds to `""` with a warning rather than failing the document. All five land in the sidecar's `llm_analyze_result`; the multimodal chunk renders them as `[Image Name]…` / `[Image Type]…` / `[Image Subject]…`, the description, and an `[Image Text]…` section (omitted when empty), so entity extraction sees OCR'd names and labels. The image's own knowledge-graph node is typed by `type` (`chart`, `flowchart`, …) instead of the generic `drawing`. `subject` + `ocr_text` are the classification-ready inputs the taxonomy layer (see `docs/GraphAndRagArchitecture.md` §5.4) is meant to consume.
...
- **`CHUNK_OVERLAP_SIZE`** — `100`, int.
  Overlap fallback. Fills a slot only when the strategy has neither its own env (`CHUNK_F_OVERLAP_SIZE` / `CHUNK_R_OVERLAP_SIZE` / `CHUNK_P_OVERLAP_SIZE`) nor the SDK path's `OntoRAG(chunk_overlap_token_size=…)`.

#### F — fixed length

- **`CHUNK_F_SIZE`** — unset, int.
  F's own `chunk_token_size`, outranking the top-level fallback (`CHUNK_SIZE` and the SDK path's `OntoRAG(chunk_token_size=…)`). When unset, F inherits the top-level resolved value.
...
3. **Legacy constructor fields** — `OntoRAG(chunk_token_size=…, chunk_overlap_token_size=…)`; only effective on the SDK path, see §11.2. Strategy-agnostic, "coarse-grained default", fills only the slots still empty.
...
Force re-parse (bypass the cache entirely): `ONTORAG_FORCE_REPARSE_NATIVE` / `ONTORAG_FORCE_REPARSE_MINERU` / `ONTORAG_FORCE_REPARSE_DOCLING` (§3.7).

Concurrency safety: OntoRAG mandates `canonical_basename` uniqueness within a workspace (HTTP 409 on upload / enqueue), and the pipeline serializes work per document, so no bundle is ever written concurrently and no extra lock is needed.
...
| `ontorag` | MD5 of the sidecar's `*.blocks.jsonl` file (relative paths resolve against `INPUT_DIR`) |
...
- **`ontorag-rebuild-vdb` must run last.** It treats the graph and `text_chunks` as the truth: graph objects a half-finished delete should have removed will be faithfully re-embedded back into the vector store. It guarantees that vectors are **consistent** with the graph, not that the graph is **correct**. So settle the graph side first, then consider rebuilding vectors.

The full order for cause 1 is therefore:

1. **Stop the service** (the fence disappears with it — do not reach for `force_reset`).
2. Run `python -m ontorag.tools.kg_integrity_repair --verbose` for the report first: anchor gaps, plus the irrecoverable orphans that are already beyond saving. Run `--apply` only if there are gaps.
...
4. Once things are stable, if you suspect graph↔VDB drift, stop the service again and run `ontorag-rebuild-vdb`, using its read-only consistency check first to decide whether the embedding cost is worth paying.

Both tools must run with the service stopped and the workspace idle (concurrent writes make them read a moving target), and `ontorag-rebuild-vdb` must use the same `.env` as the server or the rebuilt vectors land in a different embedding space. See [README_KG_INTEGRITY_REPAIR.md](../ontorag/tools/README_KG_INTEGRITY_REPAIR.md) and [README_REBUILD_VDB.md](../ontorag/tools/README_REBUILD_VDB.md).
...
6. **How changes take effect**: every parameter comes from `.env` (or the environment) and is read once when the `OntoRAG` instance is constructed; restart the service after changing one.
...
Their full semantics (including how `MAX_UPLOAD_SIZE` relates to a reverse proxy's own body limit) are in [OntoRAG Server](./OntoRAG-API-Server.md).
...
| Re-chunk | Pick the strategy by the new `process_options.chunking`, with parameters read from `full_docs.chunk_options` (the enqueue snapshot; not overwritten by resume; env changes do not affect old documents that still chunk by the parameters from the moment of enqueue). The OntoRAG Document path uses paragraph_semantic for P, the built-in F/R/V chunkers for their selectors, and the custom callback for C (or its warned fixed-token fallback if the callback is no longer configured). |
...
| Server refuses to start after editing `ONTORAG_PARSER` | Startup validation is strict: unknown engine, malformed syntax, external engine without an endpoint, or a bad engine / chunk parameter (§2.6, §2.7, §3.6) | Read the startup error — it names the offending rule |
...
| `P` was requested but the chunks look like `R` output | No structured `OntoRAG Document` was produced, so `P` degraded (§2.7, §3.2) | Route the file to `native` / `mineru` / `docling` instead of `legacy` |
...
| Changed the engine or the options, but the output is unchanged | Both the engine and `process_options` are frozen into the doc-status record at enqueue. Automatic resume and `/documents/reprocess_failed` reuse the stored values; editing `ONTORAG_PARSER` or a hint affects new uploads only (§8.3, §9.3) | Delete the document (with "also delete file") and upload it again |
| An external engine keeps returning stale output after fixing its service config | The raw-artifact bundle cache was hit (§6.3) | Set the matching `ONTORAG_FORCE_REPARSE_*` flag (§3.7), or delete the document with "also delete file" |
...
| Startup fails asking for spaCy models | `DOCX_SMART_HEADING=true`, or a rule carrying `native(smart_heading=true)`, triggers a fail-fast model check (§3.3) | Run `ontorag-download-cache --spacy-install`, or use the main Docker image, which bundles them |
...
### 11.2 OntoRAG Constructor Parameters

`OntoRAG(chunk_token_size=…, chunk_overlap_token_size=…)` is **tier 3** in §5.3's priority chain: "legacy constructor field". Strategy-agnostic and coarse-grained default, fills only slots still empty:
...
`addon_params["enable_multimodal_pipeline"]` is deprecated. Multimodal analysis is now selected per document through the `i` / `t` / `e` processing options (§2.1), set either as a rule default in `ONTORAG_PARSER` (§2.4) or via a filename hint (§2.5). There is no global "analyze everything" flag any more, because the modality set a document actually contains is decided by the parsing engine and reported through sidecar existence, not by configuration.

Migration: replace the old global switch with the corresponding letters on your routing rules — for example `ONTORAG_PARSER=*:native-iteP,*:legacy-R`. Note that `VLM_PROCESS_ENABLE` is a separate, orthogonal switch: it gates **image** analysis only (tables and equations are analyzed by the `EXTRACT` role), and with `i` enabled but no VLM available an image that passes the pre-filters fails the document rather than being skipped.
...
With `ONTORAG_PARSER` unset, every extension still routes to the `legacy` content extractor with the legacy chunking behavior, exactly as before the upgrade. The new engines and chunking strategies are opt-in; see §1 for three ready-to-use presets.
...
Where to find each family of file-processing variables. This is an index, not a duplicate: the authoritative per-variable comments live in [env.example](https://github.com/machinarii/OntoRAG/blob/main/env.example).

| Family | Variables | Documented in |
| --- | --- | --- |
| Routing | `ONTORAG_PARSER` | §2.3, §2.4, §2.5 |
...
| Parse cache | `ONTORAG_FORCE_REPARSE_{NATIVE,MINERU,DOCLING}`, `{MINERU,DOCLING}_ENGINE_VERSION` | §3.7, §6.3 |
...
| Query-time (not chunking) | `ENABLE_CONTENT_HEADINGS` — appends each chunk's heading path when assembling the answer context; it does not change chunk boundaries or stored chunk text | [OntoRAG Server](./OntoRAG-API-Server.md) |
