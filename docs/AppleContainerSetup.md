# Running the OntoRAG storage stack on Apple `container`
...
`scripts/setup/apple-container.sh` brings up the full OntoRAG storage stack —
**PostgreSQL**, **Neo4j**, and **Milvus** (standalone, with its `etcd` and
`minio` sidecars) — plus the **OntoRAG API server**, on Apple `container`
...
```bash
# Start the whole stack (databases + OntoRAG server)
bash scripts/setup/apple-container.sh up

# Databases only (run the OntoRAG server on the host yourself)
bash scripts/setup/apple-container.sh up --no-ontorag

# See what is running
bash scripts/setup/apple-container.sh status

# Tail a service's logs
bash scripts/setup/apple-container.sh logs ontorag --follow
...
`make apple-logs SVC=ontorag`, `make apple-restart SVC=<service>`,
`make apple-pull`. Pass script flags via `SETUP_OPTS`, e.g.
`make apple-up SETUP_OPTS=--no-ontorag` or `make apple-down SETUP_OPTS=--purge`.

When the stack is up:

- OntoRAG WebUI: <http://127.0.0.1:9621/webui>
- OntoRAG health: <http://127.0.0.1:9621/health>
...
host (macOS 26, Apple Silicon)
  └─ 127.0.0.1:9621 ──▶ [ontorag] ──┐   (--network ontorag)
                                     ├─▶ postgres   :5432   (volume ontorag_pg)
                                     ├─▶ neo4j       :7687   (volume ontorag_neo4j)
                                     └─▶ milvus      :19530  (volume ontorag_milvus)
                                            ├─▶ milvus-etcd  :2379  (volume ontorag_etcd)
                                            └─▶ milvus-minio :9000  (volume ontorag_minio)
  [ontorag] ──── outbound HTTPS ────▶ api.openai.com
```

Only the OntoRAG server publishes a host port (`127.0.0.1:9621`). The databases
...
IP on the `ontorag` network. Containers are namespaced as `ontorag-<service>`
(e.g. `ontorag-postgres`), so the script never touches or reuses a same-named
...
| ontorag | `ghcr.io/hkuds/ontorag:latest` |
...
containerized OntoRAG server it writes a generated `.apple-container.env`
...
| `NEO4J_USERNAME` / `NEO4J_PASSWORD` | `neo4j` / `ontoragdev` | Neo4j auth (password ≥ 8 chars) |
| `MINIO_ACCESS_KEY_ID` / `MINIO_SECRET_ACCESS_KEY` | `minioadmin` / `minioadmin` | MinIO / Milvus object store |
| `MILVUS_DB_NAME` | `ontorag` | Milvus database name (required by MilvusVectorDBStorage) |
| `ONTORAG_AC_MEM_HEAVY` | `6G` | memory for Milvus and Neo4j VMs |
| `ONTORAG_AC_MEM_LIGHT` | `2G` | memory for Postgres and OntoRAG VMs |

## Data persistence

All data lives in named volumes (`ontorag_pg`, `ontorag_neo4j`, `ontorag_milvus`,
`ontorag_etcd`, `ontorag_minio`, `ontorag_ontorag`). Both the container and
volume names are namespaced from `ONTORAG_AC_PREFIX`, so a second stack started
...
- **`bind(...): Address already in use`** — something on the host already owns
  `9621`. Stop it (e.g. a host `ontorag-server`) and retry. Database ports are
...
- **Ingestion or a query fails while `/health` is green** — the stack is fine;
  ...
  `logs ontorag` shows the exact HTTP error.
...
container network delete ontorag                    # remove the network (optional)
```
