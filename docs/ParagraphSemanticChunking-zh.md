...
> 边界歧义：正文行若真以 `#␠` 开头会被误判为标题行——这是 `ontorag/parser/_markdown.py` 已记录并接受的同一启发式歧义，实际语料中概率极低。
...
`P` 是与引擎正交的 chunking 选项（`后缀:引擎-选项`），可与任何产出 sidecar 的引擎组合。启用 P 的典型 `ONTORAG_PARSER` 写法：
...
- 未给该格式配置能产出 sidecar 的引擎（如 `ONTORAG_PARSER=docx:native-...` / `pdf:mineru-...` / `*:docling-...`），实际走了 `legacy` 路径。
