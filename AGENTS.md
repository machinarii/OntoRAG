...
- **tests/**: Pytest coverage, organized into subdirectories that mirror `ontorag/` (see *Testing* below for layout); the YAGO suites (`test_yago_*.py`) stay at the root alongside upstream's root-level tests. Working datasets stay in `inputs/`, `rag_storage/`, and `temp/`; deployment collateral lives in `docs/`, `k8s-deploy/`, and compose files.
...
- **prompt_multimodal.py**: VLM / LLM prompts for the multimodal analysis stage. Image analysis returns `{name, type, subject, ocr_text, description}`: `type` is the presentational medium (`IMAGE_TYPE_ENUM`: Photo / Chart / Flowchart / …), `subject` is what the image is *about* (the classification-ready field; a missing value folds to `""` with a warning, like an out-of-enum `type` folds to `Other`), `ocr_text` is the verbatim text baked into the pixels (`""` when none). The builder renders them into the multimodal chunk as `[Image Subject]…` / `[Image Text]…`, and `operate.extract_entities` types the image's graph node by `type` (`chart`, `flowchart`, …) via `sidecar.image_type` instead of the literal `drawing`.
...
Both metadata keys are in the `_DOC_STATUS_METADATA_CARRY_OVER_KEYS` **and** `_DOC_STATUS_METADATA_DIRECTIVE_KEYS` whitelists in `ontorag/utils_pipeline.py`; dropping either at a transition or a FAILED→PENDING reset turns a resumable purge into a permanent refusal. Retiring one requires `doc_status_transition_metadata(..., drop=...)` — passing it via `extra` would persist the value, and omitting it lets carry-over restore it.
...
Every command below is run **from `ontorag_webui/`**, not the repository root.
...
- Derive the subset from the mirror layout below: `ontorag/api/config.py` → `tests/api/config/`, `ontorag/kg/redis_impl.py` → `tests/kg/redis_impl/`, `ontorag/chunker/` → `tests/chunker/`. When a change spans several modules, run each of their directories rather than widening to `tests/`.
- Run the full suite locally only at a milestone, or when the change is genuinely cross-cutting (`ontorag/base.py`, `ontorag/utils.py`, `ontorag/kg/shared_storage.py`, or anything every backend inherits).
- Backend tests use pytest; frontend unit tests use Bun's built-in runner — see *WebUI* above and *React component tests* below.
- CI (`.github/workflows/tests.yml`) runs **every non-integration test** and builds the WebUI first. Do not narrow it to `-m offline`: most of `tests/api/` and `tests/parser/markdown/` carry no marker and were silently never run under that filter.
- Two suites depend on the host: `tests/api/test_path_prefixes.py`'s `/webui` Mount tests skip until the WebUI is built (`cd ontorag_webui && bun run build`), and the SVG rasterization tests in `tests/parser/markdown/` skip when native libcairo is missing. On macOS, `brew install cairo` is not enough by itself — Python's ctypes does not search the Homebrew prefix — so `./scripts/test.sh` exports `DYLD_FALLBACK_LIBRARY_PATH=/opt/homebrew/lib` for you; export it yourself when running `pytest` directly or `ontorag-server` locally.
- `tests/parser/pdf2md/` needs `git submodule update --init external/pdf2md` and the `[pdf2md]` extra (`uv sync --extra pdf2md`); its tests `importorskip("pymupdf")` otherwise. OCRmyPDF and LibreOffice are mocked there, so the OCR/LibreOffice binaries are not required to run them.
- **A WebUI change runs the WHOLE frontend check set**, from `ontorag_webui/`: `bun install --frozen-lockfile` (see *WebUI* above — skip it after a branch switch and every later step fails on missing modules), then `bun test`, `bunx tsc --noEmit`, and `bun run lint`. The subsetting rule above is a backend rule and does not apply — all three together take well under a minute (test ~2 s, typecheck ~14 s, lint ~21 s), so there is nothing to save by running less. Report the pass count. `bun run build` transpiles WITHOUT checking types, so skipping `tsc --noEmit` means nothing checks them.
...
run from `ontorag_webui/`. From the repository root no preload loads at all
...
- **Seed the stores a page reads, rather than stubbing its requests.**
  ...
  a consumer that does `import { queryTextStream } from '@/api/ontorag'` and
...
- **The DOM is process-wide.** Bun evaluates every test file in one process, so
  ...
  helper. (Deleting a property OF window, such as `__ONTORAG_CONFIG__`, is
...
rag = OntoRAG(..., max_parallel_insert=4)  # Default: 3, max recommended: 10
```
...
For WebUI bugs whose symptoms only surface in the rendered DOM — layout/overflow/scrollbar issues, transient flashes, third-party libraries attaching helpers to `<body>` outside React's tree, or end-to-end verification of a fix — drive the running dev server (`http://localhost:5173`) with the `document-skills:webapp-testing` skill instead of reasoning from source alone. Seed state directly via `localStorage` (persist key `settings-storage`, schema in `ontorag_webui/src/stores/settings.ts`) to skip live LLM calls. Use `wait_until="domcontentloaded"` plus a selector wait — Vite dev's long-lived polling makes `networkidle` time out.
...
- pdf2md intake (`PDF2MD_REPO_PATH`, `MAX_PARALLEL_PARSE_PDF2MD`, `PDF_OCR_ENGINE`, `PDF_OCR_LANGUAGES`, `PDF_OCR_DESKEW`, `PDF_OCR_TIMEOUT`, `PDF_OCR_MODE`, `PDF_OCR_ROTATE_PAGES[_THRESHOLD]`, `PDF_OCR_CLEAN`, `PDF_OCR_OUTPUT_TYPE`, `PDF_OCR_MAX_IMAGE_MPIXELS`, `PDF_OCR_IMAGE_DPI`, `PDF_OCR_USER_ARGS`, `PDF2MD_ORIGINALS_DIRNAME`, `PDF2MD_SOFFICE`, `PDF2MD_FIGURE_DPI`)
- Scan safety (`SCAN_STABILITY_DELAY` — defer files modified within the last N seconds; 0 = off)
