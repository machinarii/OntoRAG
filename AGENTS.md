...
- `tests/parser/pdf2md/` needs the `[pdf2md]` extra (`uv sync --extra pdf2md`); its tests `importorskip("pymupdf")` otherwise. OCRmyPDF and LibreOffice are mocked there, so the OCR/LibreOffice binaries are not required to run them.
