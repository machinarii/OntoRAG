...
/**
 * Inject `<script>window.__ONTORAG_CONFIG__ = ...</script>` into index.html.
 *
 * This mirrors what the FastAPI server does at request time in production
 * (see `SmartStaticFiles._inject_runtime_config` in
 * `ontorag/api/ontorag_server.py`). Doing it in dev too means the SPA
 ...
 */
function ontoragRuntimeConfigPlugin(env: Record<string, string>): Plugin {
  ...
  const snippet = `<script>window.__ONTORAG_CONFIG__ = ${payload};</script>`

  return {
    name: 'ontorag-dev-runtime-config',
    apply: 'serve',
    transformIndexHtml(html: string) {
      return html.replace('<!-- __ONTORAG_RUNTIME_CONFIG__ -->', snippet)
    }
  }
}
...
function workspaceFirstLoadAuditPlugin(): Plugin {
  return {
    name: 'ontorag-workspace-first-load-audit',
    apply: 'build',
...
export default defineConfig(({ mode }) => {
  ...
  // running with ONTORAG_API_PREFIX=/site01.
  ...
  return {
    plugins: [
      react(),
      tailwindcss(),
      ontoragRuntimeConfigPlugin(env),
    ...
    base: './',
    build: {
      outDir: path.resolve(import.meta.dirname, '../ontorag/api/webui'),
      emptyOutDir: true,
