...
// and 'axios' (feeds the guest-token refresh; api/ontorag is imported
...
let apiModule: typeof import('./ontorag')
...
beforeAll(async () => {
  ...
  apiModule = await import('./ontorag')
...
describe('queryTextStream — auth headers', () => {
  test('includes Bearer token when stored', async () => {
    storageData.set('ONTORAG-API-TOKEN', 'test-jwt-token')
...
describe('queryTextStream — guest-token 401 retry', () => {
  test('retries with refreshed guest token on 401', async () => {
    storageData.set('ONTORAG-API-TOKEN', 'expired-guest-token')
