...
export interface UICustomizationBrand {
   ...
   * `customized: false` response — both mean "render no line". OntoRAG ships
   * no default text here: an uncustomized deployment shows no copyright at
   * all, and a customer's page never carries OntoRAG's.
   */
 ...
 * `/auth-status` timeout in `api/ontorag.ts`, the sibling request the
