...
import { queryGraphs } from '@/api/ontorag'
...
const createSigmaGraph = async (rawGraph: RawGraph | null): Promise<UndirectedGraph | null> => {
  ...
  if (skippedEdges > 0) {
    console.warn(`[useOntoragGraph] ${skippedEdges} edges could not be added to the graph`)
  }
...
const useLightrangeGraph = () => {
  ...
  useEffect(() => {
    ...
    if (!isFetching && !useGraphStore.getState().graphDataFetchAttempted) {
      ...
      if (lastFetchSignatureRef.current === fetchSignature) {
        console.warn(
          '[useOntoragGraph] Suppressed duplicate graph fetch:',
          fetchSignature,
      ...
      dataPromise
        .then(async (result) => {
          ...
          if (!data || !data.nodes || data.nodes.length === 0) {
            ...
            try {
              ...
              console.error('[useOntoragGraph] createSigmaGraph failed (graph build):', buildError)
            ...
            try {
              ...
              console.error(
                '[useOntoragGraph] a store/sigma subscriber threw while applying the new graph:',
                subscriberError
              )
            ...
            console.log(
              `[useOntoragGraph] sigma graph ready: ${newSigmaGraph?.order} nodes, ${newSigmaGraph?.size} edges`
            )
        ...
        .catch((error) => {
          console.error(
            '[useOntoragGraph] graph load failed (see preceding log for stage):',
            error
          )
          ...
          if (error instanceof GraphBuildError) {
            console.error(
              '[useOntoragGraph] graph build failed (not retrying — deterministic):',
              fetchSignature
            )
            lastFetchSignatureRef.current = fetchSignature
            retryStateRef.current = { signature: '', attempts: 0 }
          ...
          if (retry.attempts < MAX_FETCH_RETRIES) {
            ...
            console.warn(
              `[useOntoragGraph] graph fetch failed, retry ${retry.attempts}/${MAX_FETCH_RETRIES} in ${delay}ms:`,
              fetchSignature
            )
            if (retryTimerRef.current !== null) {
              clearTimeout(retryTimerRef.current)
            ...
            console.error(
              `[useOntoragGraph] graph fetch failed after ${MAX_FETCH_RETRIES} retries, giving up:`,
              fetchSignature
            )
            lastFetchSignatureRef.current = fetchSignature
            toast.error(
    ...
    const handleNodeExpand = async () => {
      ...
      try {
        ...
        for (const edgeId of edgesToAdd) {
          ...
          if (dynamicId === null) {
            console.warn('[useOntoragGraph] could not add expansion edge:', newEdge.id)
            continue
          }
  ...
  const ontorageGraph = useCallback(() => {
  ...
  return { ontorageGraph, getNode, getEdge }
}
