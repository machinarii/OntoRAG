 ...
 * REQUEST time: the FastAPI server replaces a `<!-- __ONTORAG_RUNTIME_CONFIG__ -->`
 * comment in `index.html` with a `<script>window.__ONTORAG_CONFIG__ = ...</script>`
 * snippet built from `ONTORAG_API_PREFIX` / `ONTORAG_WEBUI_PATH`. This module
...
const config =
  (typeof window !== 'undefined' && window.__ONTORAG_CONFIG__) || {}
...
export function getRuntimeWebuiTitle(): string | null | undefined {
  ...
  return window.__ONTORAG_CONFIG__?.webuiTitle
}
