 ...
 * `ontorag::…` keys (see `lib/storageKeys.ts`).
 *
 * Field mapping is FIXED and exhaustive — it is a spec, not a convention:
 *
 *   legacy settings-storage.querySettings    → ontorag::query-settings-storage
 *   legacy settings-storage.retrievalHistory → ontorag::webui-retrieval-history
 *   (no source)                              → ontorag::workspace-retrieval-history = []
 *
...
/** Envelope version of the freshly split `ontorag::…` persist keys. */
 ...
 */
const SETTINGS_LOST_MARKER = 'ontoragSettingsLostToQuota'
