...
import { checkHealth, OntoragStatus } from '@/api/ontorag'
 ...
 * ONTORAG-PREVIOUS-USER `if (currentUsername)`, so that user's logout
...
const TOKEN_STORAGE_KEY = 'ONTORAG-API-TOKEN';
...
const TOKEN_COMPANION_STORAGE_KEYS = ['ONTORAG-LAST-TOKEN-RENEWAL'];
...
export const initAuthState = (): { isAuthenticated: boolean; isGuestMode: boolean; coreVersion: string | null; apiVersion: string | null; username: string | null; webuiTitle: string | null; webuiDescription: string | null; lastTokenRenewal: string | null; tokenExpiresAt: number | null } => {
  ...
  const coreVersion = localStorage.getItem('ONTORAG-CORE-VERSION');
  const apiVersion = localStorage.getItem('ONTORAG-API-VERSION');
  const webuiTitle = localStorage.getItem('ONTORAG-WEBUI-TITLE');
  const webuiDescription = localStorage.getItem('ONTORAG-WEBUI-DESCRIPTION');
  // Read AFTER the validity check above so a just-cleared companion key is
  // not resurrected into the store.
  const lastTokenRenewal = localStorage.getItem('ONTORAG-LAST-TOKEN-RENEWAL');
...
export const useAuthStore = create<AuthState>(set => {
  ...
  return {
    ...
    login: (token, isGuest = false, coreVersion = null, apiVersion = null, webuiTitle = null, webuiDescription = null) => {
      localStorage.setItem('ONTORAG-API-TOKEN', token);

      if (coreVersion) {
        localStorage.setItem('ONTORAG-CORE-VERSION', coreVersion);
      }
      if (apiVersion) {
        localStorage.setItem('ONTORAG-API-VERSION', apiVersion);
      }

      if (webuiTitle) {
        localStorage.setItem('ONTORAG-WEBUI-TITLE', webuiTitle);
      } else {
        localStorage.removeItem('ONTORAG-WEBUI-TITLE');
      }

      if (webuiDescription) {
        localStorage.setItem('ONTORAG-WEBUI-DESCRIPTION', webuiDescription);
      } else {
        localStorage.removeItem('ONTORAG-WEBUI-DESCRIPTION');
      }
      ...
      localStorage.setItem('ONTORAG-LAST-TOKEN-RENEWAL', formattedTime);
    ...
    logout: () => {
      localStorage.removeItem('ONTORAG-API-TOKEN');
      localStorage.removeItem('ONTORAG-LAST-TOKEN-RENEWAL');
