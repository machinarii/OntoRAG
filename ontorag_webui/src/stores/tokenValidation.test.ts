...
beforeAll(async () => {
  ...
  realApiModule = { ...(await import('@/api/ontorag')) }
  mock.module('@/api/ontorag', () => ({
    ...realApiModule,
...
afterAll(() => {
  mock.module('@/api/ontorag', () => realApiModule)
})
...
describe('initAuthState local validation', () => {
  ...
  const seed = (token: string) => {
    ...
    localStorage.setItem('ONTORAG-LAST-TOKEN-RENEWAL', '2026-01-01 00:00:00 (UTC+0)')
  }
  ...
  test.each([
    ...
    expect(localStorage.getItem('ONTORAG-API-TOKEN')).toBeNull()
    expect(localStorage.getItem('ONTORAG-LAST-TOKEN-RENEWAL')).toBeNull()
  })
  ...
  test('valid unexpired token ⇒ authenticated, nothing cleared', () => {
    ...
    expect(localStorage.getItem('ONTORAG-API-TOKEN')).not.toBeNull()
    expect(localStorage.getItem('ONTORAG-LAST-TOKEN-RENEWAL')).not.toBeNull()
  })
  ...
  test('a base64url payload yields the CLAIMS, not a blank authenticated session', () => {
    ...
    // username, and `navigationService` records ONTORAG-PREVIOUS-USER only
