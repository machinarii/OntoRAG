...
OntoRAG WebUI is a React-based web interface for interacting with the OntoRAG system. It provides a user-friendly interface for querying, managing, and exploring OntoRAG's functionalities.
...
2. **Install Dependencies:**
    ...
    In the `ontorag_webui` directory, run the following command to install project dependencies:
...
3. **Build the Project:**
    ...
    This command will bundle the project and output the built files to the `ontorag/api/webui` directory.
...
1. **Move the project into the Linux filesystem.** Clone/copy OntoRAG somewhere under your WSL home (e.g. `~/OntoRAG`) instead of `/mnt/c/...`, then reinstall:
