...
This is the Helm chart for OntoRAG, used to deploy OntoRAG services on a Kubernetes cluster.

There are two recommended deployment methods for OntoRAG:
...
* **Helm** (v3.x+)
  * Kubernetes package manager used to install OntoRAG.
...
You can deploy OntoRAG using either the provided convenience script or direct Helm commands. Both methods configure the same environment variables defined in the `ontorag/values.yaml` file.
...
helm upgrade --install ontorag ./ontorag \
  --namespace rag \
  --set-string env.ONTORAG_KV_STORAGE=JsonKVStorage \
  --set-string env.ONTORAG_VECTOR_STORAGE=NanoVectorDBStorage \
  --set-string env.ONTORAG_GRAPH_STORAGE=NetworkXStorage \
  --set-string env.ONTORAG_DOC_STATUS_STORAGE=JsonDocStatusStorage \
...
kubectl --namespace rag port-forward svc/ontorag-dev 9621:9621
...
OntoRAG and its databases are deployed within the same Kubernetes cluster, making configuration straightforward.
...
```bash
# 1. Run this port-forward command in your terminal:
kubectl --namespace rag port-forward svc/ontorag 9621:9621
...
You can configure OntoRAG's resource usage by modifying the `values.yaml` file:
...
The `env` section in the `values.yaml` file contains all environment configurations for OntoRAG, similar to a `.env` file. When using helm upgrade or helm install commands, you can override these with the --set flag.
...
env:
  ...
  ONTORAG_KV_STORAGE: PGKVStorage              # Key-value storage type
  ONTORAG_VECTOR_STORAGE: PGVectorStorage      # Vector storage type
  ONTORAG_GRAPH_STORAGE: Neo4JStorage          # Graph storage type
  ONTORAG_DOC_STATUS_STORAGE: PGDocStatusStorage  # Document status storage type
```
...
- For more customized configurations, please refer to the official OntoRAG documentation
