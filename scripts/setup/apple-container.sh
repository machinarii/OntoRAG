#!/usr/bin/env bash
#
# apple-container.sh — Run the OntoRAG storage stack on Apple's native
...
#   ontorag      ghcr.io/hkuds/ontorag:latest                (API server + WebUI)
#
...
#
# Usage:
#   scripts/setup/apple-container.sh up [--no-ontorag]
...
# --------------------------------------------------------------------------- #
NETWORK="${ONTORAG_AC_NETWORK:-ontorag}"
...
CONTAINER_PREFIX="${ONTORAG_AC_PREFIX:-ontorag-}"
ENV_SOURCE="${ONTORAG_AC_ENV_FILE:-$REPO_ROOT/.env}"
...
IMG_PG="${ONTORAG_AC_IMG_PG:-pgvector/pgvector:pg18}"
IMG_NEO4J="${ONTORAG_AC_IMG_NEO4J:-neo4j:5-community}"
IMG_MILVUS="${ONTORAG_AC_IMG_MILVUS:-milvusdb/milvus:v2.6.11}"
IMG_ETCD="${ONTORAG_AC_IMG_ETCD:-quay.io/coreos/etcd:v3.5.25}"
IMG_MINIO="${ONTORAG_AC_IMG_MINIO:-minio/minio:RELEASE.2025-09-07T16-13-09Z}"
IMG_ONTORAG="${ONTORAG_AC_IMG_ONTORAG:-ghcr.io/hkuds/ontorag:latest}"

# Memory budgets (per-container VM). Milvus and Neo4j are heavy.
MEM_LIGHT="${ONTORAG_AC_MEM_LIGHT:-2G}"
MEM_HEAVY="${ONTORAG_AC_MEM_HEAVY:-6G}"
...
# env-file that the ontorag container connects with.
...
NEO4J_PASS="${NEO4J_PASSWORD:-$(read_env_value NEO4J_PASSWORD)}";                     NEO4J_PASS="${NEO4J_PASS:-ontoragdev}"
...
# OntoRAG's MilvusVectorDBStorage requires MILVUS_DB_NAME in addition to MILVUS_URI.
MILVUS_DB="${MILVUS_DB_NAME:-$(read_env_value MILVUS_DB_NAME)}";                      MILVUS_DB="${MILVUS_DB:-ontorag}"

SERVICES=(postgres neo4j milvus-etcd milvus-minio milvus ontorag)
...
VOLUME_NAMES=(pg neo4j milvus etcd minio ontorag)
VOLUME_PREFIX="${ONTORAG_AC_VOLUME_PREFIX:-${CONTAINER_PREFIX%-}_}"
...
# --------------------------------------------------------------------------- #
# Generated env-file for the ontorag container
# --------------------------------------------------------------------------- #
...
generate_env_file() {
  ...
  _set_kv "$ENV_GENERATED" ONTORAG_KV_STORAGE "PGKVStorage"
  _set_kv "$ENV_GENERATED" ONTORAG_DOC_STATUS_STORAGE "PGDocStatusStorage"
  _set_kv "$ENV_GENERATED" ONTORAG_GRAPH_STORAGE "Neo4JStorage"
  _set_kv "$ENV_GENERATED" ONTORAG_VECTOR_STORAGE "MilvusVectorDBStorage"
...
start_ontorag() {
  generate_env_file
  run_service ontorag -- -m "$MEM_LIGHT" \
    --env-file "$ENV_GENERATED" \
    --mount "type=volume,source=$(vname ontorag),target=/app/data" \
...
cmd_up() {
  ...
  # Discover storage IPs and start OntoRAG wired to them.
  ...
  log_step "Starting OntoRAG server..."
  start_ontorag
  wait_for ontorag 120 ontorag_health
...
cmd_pull() {
  preflight
  local img
  for img in "$IMG_PG" "$IMG_NEO4J" "$IMG_MILVUS" "$IMG_ETCD" "$IMG_MINIO" "$IMG_ONTORAG"; do
...
print_help() {
  ...
  cat <<EOF
${COLOR_BOLD}apple-container.sh${COLOR_RESET} — OntoRAG storage stack on Apple's 'container' CLI

USAGE:
  scripts/setup/apple-container.sh <command> [options]

COMMANDS:
  up [--no-ontorag]     Create network/volumes and start the stack (in order,
                         with health waits). --no-ontorag starts databases only.
...
NOTES:
  ...
  * Only the OntoRAG server publishes a host port (127.0.0.1:9621). The
