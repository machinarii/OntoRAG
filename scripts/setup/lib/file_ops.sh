...
generate_docker_compose() {
  ...
  if ! _ontorag_volumes_have_container_target "$tmp_file" "/app/data/ui_templates"; then
    ontorag_mounts+=("./data/ui_templates:/app/data/ui_templates:ro")
  fi
  ...
  # _WIZARD_COMPOSE_ONTORAG_KEYS, whose strip-then-reinject pass would delete
  # it. See docs/UserDefinedUI.md 7.3.
  if ! _ontorag_environment_has_key "$tmp_file" "UI_TEMPLATES_DIR"; then
    ontorag_env_entries+=("UI_TEMPLATES_DIR=${COMPOSE_ONTORAG_UI_TEMPLATES_DIR:-/app/data/ui_templates}")
  fi
...
# Return success when the ontorag service's environment block already
...
inject_ontorag_bind_mounts() {
  ...
  while IFS= read -r line || [[ -n "$line" ]]; do
    # A comment line is never a boundary, at any column -- except the wizard's
    # own managed-services marker, which stands in for the top-level sections
    # that replace it. Ending the ontorag
...
inject_ontorag_port_mapping() {
  ...
  while IFS= read -r line || [[ -n "$line" ]]; do
    ...
    if [[ "$in_ontorag" == "yes" && "$in_ports" == "yes" ]]; then
      ...
      # the mapping inside it. Mirrors inject_ontorag_bind_mounts.
      if [[ "$line" =~ ^[[:space:]]{4}[^[:space:]-] || "$line" =~ ^[[:space:]]{2}[^[:space:]] || "$line" =~ ^[^[:space:]] ]]; then
        if [[ "$inserted" == "no" ]]; then
