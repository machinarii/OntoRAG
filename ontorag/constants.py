"""
Centralized configuration constants for OntoRAG.

This module defines default values for configuration constants used across
different parts of the OntoRAG system. Centralizing these values ensures
...
# entity_name / src_id / tgt_id fields in ontorag/kg/milvus_impl.py.
...
# disable the ratio gate. See ontorag/parser/docx/zip_budget.py.
DEFAULT_DOCX_RATIO_FLOOR_BYTES = 16 * 1024 * 1024
# Member-count ceiling. Its "checked after the central-directory walk" scope
# note lives once, on the ontorag/parser/docx/zip_budget.py module docstring.
...
# by ontorag/parser/docx/smart_heading (same live-env pattern as the
...
# enqueue docs_format (the 'ontorag' ingestion entrypoint was removed).
FULL_DOCS_FORMAT_ONTORAG = "ontorag"  # content in OntoRAG Document files
FULL_DOCS_FORMAT_PENDING_PARSE = (
    "pending_parse"  # file saved but not yet parsed; parse_native will read from disk
)
# Marker prefix for full_docs.content when format=ontorag.
...
# preview without loading the full OntoRAG Document file.
ONTORAG_DOC_CONTENT_PREFIX = "{{LRdoc}}"
# Engine identifier strings (registry keys). The set of user-selectable
# engines and their suffix capabilities now live in
# ontorag.parser.registry (ParserSpec table) — the single source of truth.
...
# Per-file processing options carried by filename hints / ONTORAG_PARSER rules.
...
PROCESS_OPTION_CHUNK_CUSTOM: ProcessChunkingOption = (
    "C"  # Explicitly invoke OntoRAG.chunking_func
)
...
# called from places holding no ``OntoRAG`` instance, several instances sharing
...
# `OntoRAG.embedding_chunk_overlap_token_size`.
