...
from ontorag.kg.pipeline_ingress import PipelineIngressMessage
from ontorag.operate import merge_nodes_and_edges
from ontorag.parser.base import ParseContext
from ontorag.parser.exceptions import (
...
from ontorag.utils_pipeline import (
    ...
    # directly): the parser layer resolves these as ``ontorag.pipeline.<name>``
...
def _vlm_image_budget_limits() -> tuple[int, int]:
    ...
    from ontorag.constants import DEFAULT_MM_IMAGE_MIN_PIXEL
...
class _PipelineMixin:
    ...
    async def _reserve_ingress_slot(
        ...
        * **the reservation is what makes the write side mutually exclusive with
        ...
        """
        from ontorag.kg.shared_storage import acquire_enqueue_reservation
    ...
    async def apipeline_enqueue_documents(
        ...
        Args:
            ...
            ids: list of unique document IDs, if not provided, MD5 hash IDs will be generated (from content or file_path).
                **Providing ``ids`` marks the SDK raw direct-insert path**
                (:meth:`OntoRAG.ainsert`) and takes precedence over
            ...
            chunk_options: per-document chunker parameter snapshot.
                ...
                :func:`ontorag.parser.routing.resolve_chunk_options`
                ...
                :func:`ontorag.utils_pipeline.apply_trusted_sentence_split_regex`
        ...
        if docs_format not in (FULL_DOCS_FORMAT_RAW, FULL_DOCS_FORMAT_PENDING_PARSE):
            raise ValueError(
                ...
                "The 'ontorag' enqueue format was removed; already-parsed "
        ...
        def _process_options_at(index: int) -> str:
            if process_options is None:
                return ""
            from ontorag.parser.routing import sanitize_process_options
        ...
        def _chunk_options_at(index: int) -> dict[str, Any]:
            ...
            :func:`ontorag.parser.routing.resolve_chunk_options` itself
    ...
    async def _feeder_next_batch(self, ingress) -> list[PipelineIngressMessage]:
        ...
        Two ingress flavors (see :mod:`ontorag.kg.pipeline_ingress`):
    ...
    async def _run_pipeline_batch(
        ...
        *,
        ...
        def _group_concurrency(group: str) -> int:
            # Built-in groups keep their existing OntoRAG fields (env +
    ...
    async def _parse_worker(
        ...
        while True:
            ...
            try:
                ...
                # reuse/passthrough (raw/ontorag/unknown_source) are skipped.
                if (
                ...
                # has already persisted the parsed body to full_docs (ontorag /
    ...
    async def process_single_document(
        ...
        *,
        ...
        """
        from ontorag.parser.routing import parse_process_options
        ...
        async with ctx.semaphore:
            try:
                ...
                # strip the ontorag marker according to the stored parse_format
                # — parse persisted the body for every engine before enqueue.
                content = strip_ontorag_doc_prefix(
                ...
                if doc_process_opts.chunking_explicit:
                    ...
                    from ontorag.chunker.recursive_character import (
                    ...
                    # keeps ``OntoRAG.ainsert`` — which intentionally does NOT
                ...
                if blocks_path and sidecar_backfill_eligible:
                    from ontorag.sidecar import backfill_chunk_sidecars
    ...
    async def _purge_stale_extraction_if_resuming(
        ...
        *,
        ...
        from ontorag.parser.routing import normalize_parser_engine
            ...
            if source_name:
                for candidate in matches:
                    if candidate.name == source_name:
                        return str(candidate)
            if parser_engine:
                from ontorag.parser.routing import filename_parser_directives
