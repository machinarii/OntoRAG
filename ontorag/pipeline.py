"""Document ingestion pipeline mixin for the OntoRAG class.

This module isolates the document parse/enqueue/extraction pipeline so that
``ontorag.py`` stays focused on storage management, querying, and editing.
The mixin is wired into :class:`ontorag.OntoRAG` via multiple inheritance
...
from ontorag.exceptions import MultimodalAnalysisError, PipelineCancelledException
from ontorag.kg.shared_storage import get_namespace_data, get_namespace_lock
from ontorag.operate import merge_nodes_and_edges
...
from ontorag.utils_pipeline import (
    ...
    input_dir_path,
    load_ontorag_document_content,
    make_ontorag_doc_content,
    normalize_document_file_path,
    parsed_artifact_dir_for,
    resolve_doc_file_path,
    sidecar_blocks_path,
    sidecar_uri_for,
    strip_ontorag_doc_prefix,
)
...
class _PipelineMixin:
    """Mixin providing document ingestion pipeline methods for OntoRAG.

    Designed to be combined as a base of OntoRAG only.  Relies on
    OntoRAG-provided attributes (``self.full_docs``, ``self.doc_status``,
    ...
    async def apipeline_enqueue_documents(
        ...
        ontorag_document_paths: str | list[str] | None = None,
        ...
        1. Validate ids if provided or generate MD5 hash IDs and remove duplicate contents (skip content dedup when format is ontorag)
        2. Generate document initial status
        3. Filter out already processed documents
        4. Enqueue document in status

        Args:
            input: Single document string or list of document strings (can be empty when docs_format is ontorag)
            ids: list of unique document IDs, if not provided, MD5 hash IDs will be generated (from content or file_path when ontorag)
            file_paths: list of file paths corresponding to each document, used for citation
            track_id: tracking ID for monitoring processing status
            docs_format: "raw" (default) or "ontorag"; when "ontorag" content may be empty and content-dedup is skipped
            ontorag_document_paths: paths to OntoRAG Document (e.g. .blocks.jsonl dir or base path), when docs_format is ontorag
            ...
            chunk_options: per-document chunker parameter snapshot.
                ...
                :func:`ontorag.parser_routing.resolve_chunk_options`
                ...
                :meth:`OntoRAG.ainsert` — should call
        ...
        if isinstance(ontorag_document_paths, str):
            ontorag_document_paths = (
                [ontorag_document_paths] if ontorag_document_paths else None
            )
        ...
        is_ontorag_format = docs_format == FULL_DOCS_FORMAT_ONTORAG
        if is_ontorag_format and ontorag_document_paths is not None:
            if len(ontorag_document_paths) != len(input):
                raise ValueError(
                    "Number of ontorag_document_paths must match the number of documents"
                )
        ...
        def _process_options_at(index: int) -> str:
            if process_options is None:
                return ""
            from ontorag.parser_routing import sanitize_process_options
        ...
        def _chunk_options_at(index: int) -> dict[str, Any]:
            ...
            F-strategy runtime args (``split_by_character`` /
            ``split_by_character_only`` from :meth:`OntoRAG.ainsert`)
            are baked into the snapshot upstream — ainsert calls
            :func:`ontorag.parser_routing.resolve_chunk_options` itself
        ...
        # 1. Validate ids and build contents (when ontorag: no content dedup, content may be empty)
        if ids is not None:
        ...
        # Per-doc I/O failures from the ontorag-format branch.  Populated when
        # ``load_ontorag_document_content`` cannot read the user-supplied
        ...
        ontorag_load_errors: list[dict[str, Any]] = []
        ...
        def _add_content(
            ...
            *,
            ...
            # strip_ontorag_doc_prefix is a no-op for non-ontorag formats.
            body_length = len(strip_ontorag_doc_prefix(content, doc_format))

            # Compute content hash: skip for pending_parse (content extracted later).
            # RAW and ONTORAG both hash the bare merged text so the same body
            # carried by different envelopes (raw text vs sidecar) dedupes
            # against itself across formats.
            content_hash: str | None = None
            if doc_format in (FULL_DOCS_FORMAT_RAW, FULL_DOCS_FORMAT_ONTORAG):
                content_hash = compute_text_content_hash(
                    strip_ontorag_doc_prefix(content or "", doc_format)
                )
        ...
        if is_ontorag_format:
            # OntoRAG Document: no content hash dedup; content may be empty
            for i in range(len(file_paths)):
                path = file_paths[i]
                raw_path = (
                    ontorag_document_paths[i] if ontorag_document_paths else ""
                ) or path
                ...
                # for format=ontorag must be "{{LRdoc}}" + the merged body.
                ...
                try:
                    merged_text, _ = await load_ontorag_document_content(
                        sidecar_location
                    )
                except Exception as exc:
                    error_msg = f"load_ontorag_document_content failed: {exc}"
                    logger.warning(
                    ...
                    if blocks_path_str:
                        try:
                            ...
                            file_size = 0
                    ontorag_load_errors.append(
                        {
                            "file_path": path,
                            "error_description": (
                                "Failed to load OntoRAG Document blocks"
                            ),
                    ...
                    continue
                summary_content = make_ontorag_doc_content(merged_text)
                _add_content(
                    i,
                    summary_content,
                    FULL_DOCS_FORMAT_ONTORAG,
        ...
        def _initial_doc_status(content_data: dict[str, Any]) -> dict[str, Any]:
            # For ontorag-format full_docs the persisted content carries the
            # ``{{LRdoc}}`` marker; strip it so summary/length match raw
            # semantics (the marker is full_docs internal bookkeeping and
            # must not leak into doc_status).  strip_ontorag_doc_prefix
            # internally checks parse_format, so non-ontorag formats pass
            # through untouched.
            body_text = strip_ontorag_doc_prefix(
        ...
        async with enqueue_serialize_lock:
            ...
            # Flush ontorag-format I/O failures as FAILED stubs.  Done
            ...
            if ontorag_load_errors:
                await self.apipeline_enqueue_error_documents(
                    ontorag_load_errors, track_id=track_id
                )
            ...
            if not new_docs:
                ...
                # If FAILED stubs were just flushed (ontorag-format I/O
    ...
    async def _analyze_worker(self, ctx: _BatchRunContext) -> None:
        ...
        body (pending_parse → ontorag / raw documents start with empty
        ...
        async with ctx.semaphore:
            try:
                ...
                if not isinstance(chunk_opts, dict) or not chunk_opts:
                    ...
                    from ontorag.parser_routing import resolve_chunk_options
                ...
                # parse_format=raw, which silently mislabelled ontorag docs;
                ...
                extraction_meta = {
                    ...
                    or (
                        "native"
                        if persisted_format == FULL_DOCS_FORMAT_ONTORAG
                        else "legacy"
                    ),
                    "chunking_method": (
    ...
    async def _purge_stale_extraction_if_resuming(
        ...
        *,
        ...
        content_already_extracted = isinstance(content_data, dict) and (
            (
                content_data.get("parse_format") == FULL_DOCS_FORMAT_ONTORAG
        ...
        if intended_engine and stored_engine and intended_engine != stored_engine:
            log_message = (
                ...
                f"ONTORAG_PARSER implies engine="
    ...
    async def parse_native(
        ...
        """Phase 1 parse for native/raw, ontorag and pending_parse formats."""
        doc_format = content_data.get("parse_format", FULL_DOCS_FORMAT_RAW)
        if doc_format == FULL_DOCS_FORMAT_ONTORAG:
            ...
            merged_text = strip_ontorag_doc_prefix(
        ...
        if doc_format == FULL_DOCS_FORMAT_PENDING_PARSE:
            ...
            from ontorag.native_parser.docx.drawing_image_extractor import (
                DrawingExtractionContext,
                load_relationships,
            )
            from ontorag.native_parser.docx.parse_document import (
                extract_docx_blocks,
            )
            from ontorag.native_parser.docx.ir_builder import NativeDocxIRBuilder
            from ontorag.sidecar import write_sidecar
            ...
            await self._persist_parsed_full_docs(
                doc_id,
                {
                    "content": make_ontorag_doc_content(parsed_data["content"]),
                    "file_path": file_path,
    ...
    async def parse_mineru(
        ...
        :func:`ontorag.api.routers.document_routes.delete_file_variants_by_file_path`.
        """
        # Lazy imports keep this module import-cheap and avoid pulling httpx
        # into call paths that never touch the MinerU engine.
        from ontorag.external_parser.mineru import (
            MinerUIRBuilder,
            MinerURawClient,
            clear_dir_contents,
            is_bundle_valid,
        ...
        force_reparse = os.getenv("ONTORAG_FORCE_REPARSE_MINERU", "").lower() in {
            "1",
            "true",
            "yes",
            "on",
        ...
        if not force_reparse and is_bundle_valid(raw_dir, source_file_path):
            ...
            if force_reparse and raw_dir.exists():
                logger.info(
                    "[parse_mineru] ONTORAG_FORCE_REPARSE_MINERU set; "
    ...
    async def parse_docling(
        ...
        from ontorag.external_parser.docling import (
            DoclingIRBuilder,
            DoclingRawClient,
            clear_dir_contents,
            is_bundle_valid,
        ...
        force_reparse = os.getenv("ONTORAG_FORCE_REPARSE_DOCLING", "").lower() in {
            "1",
            "true",
            "yes",
            "on",
        ...
        if not force_reparse and is_bundle_valid(raw_dir, source_file_path):
            ...
            if force_reparse and raw_dir.exists():
                logger.info(
                    "[parse_docling] ONTORAG_FORCE_REPARSE_DOCLING set; "
                    "discarding bundle at %s",
                    raw_dir,
                )
    ...
    async def _persist_parsed_full_docs(
        ...
        # for ontorag-format) so cross-filename dedup fires regardless of
        # whether the same body was ingested as raw text or via a sidecar.
        # ``strip_ontorag_doc_prefix`` is a no-op for non-ontorag formats.
        if fmt in (FULL_DOCS_FORMAT_RAW, FULL_DOCS_FORMAT_ONTORAG):
            content_hash = compute_text_content_hash(
                strip_ontorag_doc_prefix(record.get("content") or "", fmt)
            )
    ...
    async def _write_ontorag_document_from_content_list(
        self,
        doc_id: str,
        file_path: str,
        ...
        """Convert parser content list to OntoRAG Document files and return parsed_data."""
        document_name = normalize_document_file_path(file_path)
        if document_name == "unknown_source":
        ...
        # Keep full_docs in sync so restart/reprocess can directly use OntoRAG Document.
        await self._persist_parsed_full_docs(
            doc_id,
            {
                "content": make_ontorag_doc_content(merged_text),
                "file_path": file_path,
                "parse_format": FULL_DOCS_FORMAT_ONTORAG,
                "sidecar_location": sidecar_uri_for(parsed_dir),
                "parse_engine": engine,
    ...
    async def analyze_multimodal(
        ...
        *,
        ...
        """Phase 2: Multimodal analysis (VLM). Writes llm_analyze_result to OntoRAG Document.
        ...
        # Docling, and pre-existing OntoRAG documents reused from disk
        ...
        try:
            ...
            from ontorag.prompt_multimodal import (
                IMAGE_TYPE_ENUM,
                IMAGE_TYPE_FALLBACK,
                MULTIMODAL_PROMPTS,
            )
            ...
            # in ontorag.utils).  When the flag is off we must NOT save the
            ...
            def _json_extract(text: str) -> dict[str, Any]:
                ...
                Mirrors :func:`ontorag.operate._process_json_extraction_result`
            ...
            async def _analyze_text_modality(
                ...
                if tokenizer is not None:
                    ...
                    from ontorag.multimodal_context import trim_content_to_budget
    ...
    def _build_mm_chunks_from_sidecars(
        ...
        def _render(
            ...
            # ``ontorag.operate._parse_mm_display_name`` (regex
