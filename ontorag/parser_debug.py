"""Shared debug OntoRAG stand-in for the parse_* entry points.

A minimal ``OntoRAG`` stand-in plus a deterministic ``datetime`` shim,
shared by the unified parser debug CLI (``ontorag/parser_cli.py``),
...
def build_debug_rag():
    """Build a minimal OntoRAG stand-in that exposes what ``parse_*`` reads.

    The import of ``OntoRAG`` is intentionally function-local: deferring
    it avoids a circular import when this helper is loaded during package
    init (the parser CLI resolves ``ontorag.parser_debug`` before
    ``ontorag`` itself is fully bound).

    OntoRAG-side attributes the three ``parse_*`` methods read off ``self`` —
    every entry MUST be provided by this stand-in, or the debug CLI / golden
    tests / regen script will all break in sync:

    - **methods** (rebound from :class:`OntoRAG`):
    ...
    When any of the three ``OntoRAG.parse_*`` methods grows a new
    ...
    class _DebugRag:
        ...
        parse_native = OntoRAG.parse_native
        parse_mineru = OntoRAG.parse_mineru
        parse_docling = OntoRAG.parse_docling
