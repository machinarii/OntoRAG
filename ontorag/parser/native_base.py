...
``NativeParserBase.parse`` fixes the common local-parse flow once:
    ...
    → persist full_docs (ontorag) → archive source
...
class NativeExtractRuntime:
    ...
    :func:`~ontorag.parser.exceptions.normalize_cancel_events` accepts, so a
...
class NativeParserBase(BaseParser):
    ...
    def _build_llm_submit(
        ...
        from ontorag.utils import get_llm_cache_identity, use_llm_func_with_cache
    ...
    def finalize_parse_warnings(
        ...
        *,
        ...
        :class:`~ontorag.parser.docx.parser.NativeDocxParser`). The base stays
    ...
    async def parse(self, ctx: ParseContext) -> ParseResult:
        from ontorag.parser.routing import decode_parse_engine, encode_parse_engine
