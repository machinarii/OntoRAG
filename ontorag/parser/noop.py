...
- :class:`ReuseParser` handles ``ontorag`` rows — a document already parsed
...
from ontorag.constants import FULL_DOCS_FORMAT_ONTORAG, FULL_DOCS_FORMAT_RAW
from ontorag.parser.base import BaseParser, ParseContext, ParseResult


class ReuseParser(BaseParser):
    """Reuse an already-parsed (``ontorag``-format) document's sidecar."""
    ...
    async def parse(self, ctx: ParseContext) -> ParseResult:
        ...
        doc_format = ctx.content_data.get("parse_format", FULL_DOCS_FORMAT_ONTORAG)
