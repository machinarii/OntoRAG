"""Third-party parser engine discovery (``ontorag.parsers`` entry points).

A third-party package exposes parser engines by declaring an entry point in
the ``ontorag.parsers`` group::
...
its own :func:`ontorag.parser.registry.register_parser` call(s).  The
...
routing-rule validation so ``ONTORAG_PARSER`` may reference third-party
engine names) and the debug CLI (``ontorag.parser.cli.main``, before
``--engine`` choices are built).  Library users embedding OntoRAG directly
...
ENTRY_POINT_GROUP = "ontorag.parsers"
...
def load_third_party_parsers(*, force: bool = False) -> list[str]:
    """Discover and run all ``ontorag.parsers`` entry points.
