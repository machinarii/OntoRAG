...
Invocation::
    ...
    python -m ontorag.parser.cli path/to/sample.docx --engine native
    python -m ontorag.parser.cli path/to/sample.pdf  --engine mineru
    python -m ontorag.parser.cli path/to/sample.pdf  --engine docling --force-reparse
...
def _normalize_direct_script_sys_path() -> None:
    ...
    # Direct execution adds ontorag/parser to sys.path, which makes the
...
def _build_parser() -> argparse.ArgumentParser:
    ...
    from ontorag.parser.registry import supported_parser_engines
...
async def _run(args: argparse.Namespace) -> int:
    ...
    from ontorag.parser.external._base import ExternalParserBase
    from ontorag.parser.registry import get_parser, suffix_capabilities
    from ontorag.parser.debug import build_debug_rag
    from ontorag.parser.docx.parse_document import DocxContentError
    from ontorag.utils import compute_mdhash_id
    import ontorag.pipeline as pipeline_mod
    import ontorag.utils_pipeline as utils_pipeline_mod
    ...
    with ExitStack() as stack:
        ...
        # ctx.archive_source -> ontorag.pipeline.archive_docx_source_after_...
        stack.enter_context(
            mock.patch.object(
                pipeline_mod,
...
def main(argv: list[str] | None = None) -> int:
    ...
    # so a `ontorag.parsers` entry-point engine is selectable here exactly
