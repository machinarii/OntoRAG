...
from ontorag.parser.param_schema import (
    _parse_bool,
    parse_chunk_params,
    parse_engine_params,
...
from ontorag.utils import get_env_value, logger, parse_optional_float
...
# there automatically (no filename hint / ONTORAG_PARSER rule needed). ``.md``
...
class ParserRoutingConfigError(ValueError):
    """Raised when ONTORAG_PARSER contains an invalid routing rule."""
...
class ProcessOptions:
    ...
    def chunking_explicit(self) -> bool:
        ...
        legacy externally-supplied :attr:`OntoRAG.chunking_func`.
        """
    ...
    * a ``separators`` value that is not a list/tuple has its key **removed**.
    ...
    Args:
        ...
        in_place: mutate ``chunker_config`` and its ``recursive_character``
            ...
            :func:`ontorag.addon_params.default_addon_params` — keeps pointing
...
def default_chunker_config() -> dict[str, Any]:
    ...
    *strategy-specific* env vars (``CHUNK_F_SIZE``,
    ...
    :meth:`OntoRAG._apply_chunk_size_overlay` apply the legacy
    constructor field (``OntoRAG(chunk_overlap_token_size=…)``) and
...
def resolve_chunk_options(
    ...
    *,
    ...
    The F runtime args from ``OntoRAG.ainsert`` overlay the
    ...
    if src is None:
        ...
        from ontorag.addon_params import ObservableAddonParams
...
def parser_rules_from_env() -> str:
    return os.getenv("ONTORAG_PARSER", "").strip()
...
def _rule_engine_and_options(engine_hint: str) -> tuple[str, str]:
    """Split a ``ONTORAG_PARSER`` rule's RHS (``engine[-options]``).
    ...
    boundary, since ``ONTORAG_PARSER`` rules cannot be options-only.
    """
...
def validate_parser_suffix_env_vars() -> None:
    ...
    it at startup mirrors the strictness ``ONTORAG_PARSER`` already gets.
    """
    errors = [
...
def validate_parser_routing_config(parser_rules: str | None = None) -> None:
    """Validate ONTORAG_PARSER syntax and required external parser endpoints.
    ...
    if errors:
        raise ParserRoutingConfigError(
            "Invalid ONTORAG_PARSER configuration: " + "; ".join(errors)
        )
...
def _rules_enable_smart_heading(rules: str) -> bool:
    """True when a docx-matching native ONTORAG_PARSER rule sets ``smart_heading=true``.
...
def validate_smart_heading_dependencies(parser_rules: str | None = None) -> None:
    ...
    ``ONTORAG_PARSER`` rule carries ``native(smart_heading=true)``. Per-file
    ...
    from ontorag.parser.docx.smart_heading.nlp import ensure_spacy_models_installed

    ensure_spacy_models_installed(
        "smart_heading is enabled by DOCX_SMART_HEADING or a ONTORAG_PARSER rule"
    )
...
def _validate_smart_heading_max_chars() -> None:
    ...
    source :func:`~ontorag.parser.docx.smart_heading.guardrails.validate_heading_max_chars_env`,
    ...
    """
    from ontorag.parser.docx.smart_heading import guardrails
    from ontorag.parser.docx.smart_heading.title_block import (
...
def _matching_rule_directives(
    ...
    *,
    ...
    """Find the first matching ``ONTORAG_PARSER`` rule for ``file_path``.
...
def resolve_parser_directives(
    ...
    *,
    ...
    Resolution order (mirrors :func:`resolve_file_parser_engine`):
        ...
        2. ``ONTORAG_PARSER`` rules — first matching rule provides defaults
...
def resolve_stored_document_parser_engine(
    ...
    if content_data:
        ...
        # All ontorag rows reuse the already-parsed sidecar (sidecar optional;
