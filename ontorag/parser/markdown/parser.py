...
from ontorag.parser.external._common import raw_dir_for_parsed_dir
from ontorag.parser.exceptions import (
    ParseCancelled,
    first_cancellation,
    normalize_cancel_events,
)
from ontorag.parser.markdown.extract import ResolvedImage, extract_markdown
from ontorag.parser.markdown.raw_cache import (
    NativeImageRawCache,
    native_md_options_signature,
)
from ontorag.parser.native_base import NativeExtractRuntime, NativeParserBase
from ontorag.utils import logger

if TYPE_CHECKING:
    from ontorag.sidecar.ir import IRDoc
...
class _MarkdownImageResolver:
    """Concrete :class:`~ontorag.parser.markdown.extract.ImageResolver`.
    ...
    def _fetch(self, src: str, guard: _DownloadState, cap: int) -> tuple[bytes, str]:
        ...
        req = urllib.request.Request(src, headers={"User-Agent": "ontorag-native-md"})
        try:
...
class NativeMarkdownParser(NativeParserBase):
    ...
    def extract(
        ...
        *,
        ...
        # rule like ``ONTORAG_PARSER=native(smart_heading=true)`` doesn't hard
        ...
        raw_cache = NativeImageRawCache(
            ...
            force_reparse=_env_bool("ONTORAG_FORCE_REPARSE_NATIVE", False),
        )
        raw_cache.load()
    ...
    def _open_textpack(self, source: Path, tmp_dir: Path) -> tuple[str, Path]:
        from ontorag.parser.external._zip import safe_extract_zip
    ...
    def build_ir(
        ...
        *,
        ...
        from ontorag.parser.markdown.ir_builder import NativeMarkdownIRBuilder
