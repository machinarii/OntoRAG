...
:func:`ontorag.parser.markdown.extract.extract_markdown`, threaded through
...
from ontorag.parser._html_table import (
    extract_html_table_info,
    extract_thead_html,
    html_table_inner_body,
    unwrap_html_table,
)
from ontorag.parser.markdown.extract import (
    DRAWING_MARKER_RE,
    EQUATION_MARKER_RE,
    PREFACE_HEADING,
    TABLE_MARKER_RE,
...
def _build_table(placeholder: str, spec: dict) -> IRTable:
    ...
    # Keep the header in the body so chunk splitting can repeat it.
    rows = [
        [str(c) for c in row] for row in [*(header or []), *(spec.get("rows") or [])]
    ]
    num_rows = len(rows)
