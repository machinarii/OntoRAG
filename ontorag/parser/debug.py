"""Shared debug OntoRAG stand-in for registry-dispatched parsing.

A minimal ``OntoRAG`` stand-in plus a deterministic ``datetime`` shim,
shared by the unified parser debug CLI (``ontorag/parser/cli.py``),
...
def build_debug_rag(*, extract_llm_func=None):
    """Build a minimal OntoRAG stand-in that exposes what a parser reads.
    ...
    init (the parser CLI resolves ``ontorag.parser.debug`` before
