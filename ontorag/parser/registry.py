...
Like :data:`ontorag.kg.STORAGES` / ``STORAGE_IMPLEMENTATIONS``, this module
...
if TYPE_CHECKING:
    from ontorag.parser.base import BaseParser
...
class _EnvExtendedSuffixes:
    ...
    inside ``_build_parser()`` — before ``_run()`` imports ``ontorag.utils``
...
class ParserSpec:
    ...
    # mirroring how the built-in ``max_parallel_parse_*`` OntoRAG fields read
    # their env. ``None`` means this spec does not own its group's concurrency
    # (built-in groups are sized by the OntoRAG instance field instead).
    ...
    # endpoint rather than on OntoRAG: docling's legacy Office formats need
...
_REGISTRY: dict[str, ParserSpec] = {
    PARSER_ENGINE_NATIVE: ParserSpec(
        ...
        # source suffix (see ontorag.parser.native_dispatch).
        impl="ontorag.parser.native_dispatch:NativeParser",
        suffixes=frozenset({"docx", "md", "textpack"}),
        queue_group=PARSER_ENGINE_NATIVE,
        # Built-in groups are sized by the OntoRAG ``max_parallel_parse_*``
    ...
    PARSER_ENGINE_LEGACY: ParserSpec(
        ...
        impl="ontorag.parser.legacy.parser:LegacyParser",
    ...
    PARSER_ENGINE_MINERU: ParserSpec(
        ...
        impl="ontorag.parser.external.mineru.parser:MinerUParser",
    ...
    PARSER_ENGINE_DOCLING: ParserSpec(
        ...
        impl="ontorag.parser.external.docling.parser:DoclingParser",
    ...
    PARSER_ENGINE_REUSE: ParserSpec(
        ...
        impl="ontorag.parser.noop:ReuseParser",
        suffixes=frozenset(),
        user_selectable=False,
    ),
    PARSER_ENGINE_PASSTHROUGH: ParserSpec(
        engine_name=PARSER_ENGINE_PASSTHROUGH,
        impl="ontorag.parser.noop:PassthroughParser",
        suffixes=frozenset(),
        user_selectable=False,
    ),
}
