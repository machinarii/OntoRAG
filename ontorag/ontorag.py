...
from ontorag.parser.routing import _chunk_env_int
...
from ontorag.query_validation import validate_query_not_empty, validate_rag_query
...
from ontorag.constants import GRAPH_FIELD_SEP, RELATION_NO_EVIDENCE_SOURCE_IDS
from ontorag.exceptions import (
    IndexFlushError,
...
from ontorag.pipeline import _PipelineMixin
from ontorag.kg.factory import get_storage_class
from ontorag.addon_params import (
    ObservableAddonParams,
    normalize_addon_params,
)
from ontorag.llm_roles import (
    ROLE_NAMES,
    ROLES,
    RoleLLMConfig,
    RoleSpec,  # noqa: F401  # re-exported via ontorag/__init__.py
    _optional_env_int,
    _RoleLLMMixin,
    _RoleLLMState,
)
from ontorag.storage_migrations import _StorageMigrationMixin
...
class _PurgeRecoveryProof:
    ...
    See :py:meth:`OntoRAG._resolve_purge_recovery_proof`. ``proof_kind`` is
    ``None`` exactly when no proof applies, in which case ``missing_reason``
    carries the :class:`~ontorag.exceptions.RecoveryAnchorMissingError` reason
    to raise with.
    """
...
def _run_sync(
    ...
    * **A different — but still alive — loop than the storages bound to**
      ...
      ``asyncio.Lock`` objects in ``ontorag.kg.shared_storage`` are bound to
    ...
    *closed* ``owning_loop`` means the loop the storages were initialized on is
    ...
    ``OntoRAG`` instance; callers that must invoke them from multiple threads
    ...
    Args:
        ...
        owning_loop: The loop the instance's storages were initialized on
            (``OntoRAG._owning_loop``). ``None`` when storages have not been
    ...
    if (
        ...
        raise RuntimeError(
            f"{sync_name}() must run on the same event loop this OntoRAG "
            ...
            f"thread. OntoRAG's shared storage locks are bound to the original "
...
@final
@dataclass
class OntoRAG(_RoleLLMMixin, _StorageMigrationMixin, _PipelineMixin):
    """OntoRAG: Simple and Fast Retrieval-Augmented Generation."""
    ...
    :func:`ontorag.parser.routing.default_chunker_config`.  Per-doc
    ...
    ``__deepcopy__`` returning ``self``). OntoRAG deliberately does not serialize
    it — a lock owned by OntoRAG would be waited on by the event loop and would
    recreate the freeze the offload removes. The built-in ``TiktokenTokenizer``
    satisfies both. See :class:`ontorag.utils.Tokenizer`.
    """
    ...
    **When this function is actually invoked.** The chunker dispatch in
    ...
    document's ``process_options``:
      ...
      - If ``process_options`` explicitly contains a built-in chunking
        ...
        :mod:`ontorag.chunker` (``chunking_by_fixed_token`` for ``F``,
    ...
    :attr:`ontorag.parser.routing.ProcessOptions.chunking_explicit`.

    **Signature** — preserved unchanged from earlier OntoRAG releases
    ...
    Defaults to :func:`ontorag.chunker.chunking_by_token_size`.
    """
    ...
    :class:`~ontorag.exceptions.PipelineBackpressureError` (HTTP 429) when
    ...
    # TODO: Deprecated (OntoRAG will never initialize storage automatically on creation，and finalize should be call before destroying)
    auto_manage_storages_states: bool = field(default=False)
    """If True, ontorag will automatically calls initialize_storages and finalize_storages at the appropriate times."""
    ...
    # Declared last on purpose. OntoRAG is a plain dataclass, so SDK callers
    ...
    def _apply_chunk_size_overlay(self) -> None:
        ...
        2. Strategy-specific env (``CHUNK_F_SIZE`` / ``CHUNK_R_SIZE`` /
           ...
           :func:`ontorag.parser.routing.default_chunker_config` when it
        ...
        3. Legacy constructor field
           (``OntoRAG(chunk_token_size=…, chunk_overlap_token_size=…)``).
    ...
    def __post_init__(self, addon_params: dict[str, Any] | None):
        ...
        logger.debug(f"OntoRAG init with param:\n  {_print_config}\n")
        ...
        # while preventing side effects when the same EmbeddingFunc is reused across multiple OntoRAG instances
    ...
    async def initialize_storages(self):
        ...
        if self._storages_status == StoragesStatus.CREATED:
            ...
            if default_workspace is None:
                ...
                logger.info(
                    f"Creating OntoRAG instance with workspace='{self.workspace}' "
    ...
    async def ainsert(
        ...
        The OntoRAG **server / REST API does not call this method** — it
        ...
        from ontorag.parser.routing import resolve_chunk_options
    ...
    async def ainsert_custom_kg(
        ...
        .. warning:: (issue #3400 Phase 5 — direct-writer audit)
           ...
           offline ``ontorag.tools.kg_integrity_repair`` audit can
        ...
        try:
            from ontorag.utils_graph import (
    ...
    def _raise_missing_recovery_proof(
        ...
        remedy = (
            ...
            "(python -m ontorag.tools.kg_integrity_repair), then retry."
        )
        if (
    ...
    async def _raise_if_recovery_required(self) -> None:
        ...
        """
        from ontorag.exceptions import PipelineNotInitializedError
    ...
    async def adelete_by_entity(self, entity_name: str) -> DeletionResult:
        ...
        from ontorag.utils_graph import adelete_by_entity
    ...
    async def adelete_by_relation(
        ...
        from ontorag.utils_graph import adelete_by_relation
    ...
    async def get_relation_info(
        ...
        """
        from ontorag.utils_graph import get_relation_info
    ...
    async def aedit_entity(
        ...
        from ontorag.utils_graph import aedit_entity
    ...
    async def aedit_relation(
        ...
        from ontorag.utils_graph import aedit_relation
    ...
    async def acreate_entity(
        ...
        from ontorag.utils_graph import acreate_entity
    ...
    async def acreate_relation(
        ...
        from ontorag.utils_graph import acreate_relation
    ...
    async def amerge_entities(
        ...
        from ontorag.utils_graph import amerge_entities
    ...
    async def aexport_data(
        ...
        """
        from ontorag.utils import aexport_data as utils_aexport_data
...
# passed through OntoRAG(addon_params=...). InitVars are not stored as
...
OntoRAG.addon_params = property(  # type: ignore[attr-defined]
    OntoRAG._get_addon_params,
    OntoRAG._set_runtime_addon_params,
)
