"""
PostgreSQL-native graph storage for OntoRAG (plain tables, no extensions).
...
Schema (created automatically on initialize):
    ...
    ontorag_graph_nodes(workspace TEXT, namespace TEXT, id TEXT, properties JSONB)
                                                  PK(workspace, namespace, id)
    ontorag_graph_edges(workspace TEXT, namespace TEXT, src_id TEXT, tgt_id TEXT, properties JSONB)
...
_DDL = """
SELECT pg_advisory_xact_lock(hashtext('ontorag_pgtable_schema'));

CREATE TABLE IF NOT EXISTS ontorag_graph_nodes (
...
CREATE TABLE IF NOT EXISTS ontorag_graph_edges (
    workspace   TEXT        NOT NULL,
    namespace   TEXT        NOT NULL DEFAULT 'chunk_entity_relation',
...
BEGIN
    SELECT EXISTS (
        ...
        WHERE c.conname = 'ontorag_graph_nodes_pkey'
          AND c.conrelid = 'ontorag_graph_nodes'::regclass
    ...
    SELECT EXISTS (
        ...
        JOIN pg_attribute a
          ON a.attrelid = c.conrelid
         AND a.attnum = ANY(c.conkey)
        WHERE c.conname = 'ontorag_graph_edges_pkey'
          AND c.conrelid = 'ontorag_graph_edges'::regclass
          AND a.attname = 'namespace'
    ) INTO edges_pk_has_namespace;

    IF NOT nodes_pk_has_namespace OR NOT edges_pk_has_namespace THEN
        ALTER TABLE ontorag_graph_edges
            DROP CONSTRAINT IF EXISTS fk_ontorag_graph_edges_src;
        ALTER TABLE ontorag_graph_edges
            DROP CONSTRAINT IF EXISTS fk_ontorag_graph_edges_tgt;

        IF NOT edges_pk_has_namespace THEN
            ALTER TABLE ontorag_graph_edges
                DROP CONSTRAINT IF EXISTS ontorag_graph_edges_pkey;
            ...
            DELETE FROM ontorag_graph_edges a
            USING ontorag_graph_edges b
        ...
        IF NOT nodes_pk_has_namespace THEN
            ALTER TABLE ontorag_graph_nodes
                DROP CONSTRAINT IF EXISTS ontorag_graph_nodes_pkey;
            ...
            DELETE FROM ontorag_graph_nodes a
            USING ontorag_graph_nodes b
            WHERE a.workspace = b.workspace
              AND a.namespace = b.namespace
              AND a.id = b.id
              AND (a.updated_at, a.ctid) < (b.updated_at, b.ctid);
...
CREATE INDEX IF NOT EXISTS idx_ontorag_graph_edges_namespace_tgt
    ON ontorag_graph_edges (workspace, namespace, tgt_id);
...
CREATE INDEX IF NOT EXISTS idx_ontorag_graph_edges_noncanonical
    ON ontorag_graph_edges (workspace, namespace)
...
BEGIN
    SELECT EXISTS (
        SELECT 1
        FROM pg_constraint
        WHERE conname = 'fk_ontorag_graph_edges_src'
          AND conrelid = 'ontorag_graph_edges'::regclass
    ) INTO has_src_fk;

    SELECT EXISTS (
        SELECT 1
        FROM pg_constraint
        WHERE conname = 'fk_ontorag_graph_edges_tgt'
          AND conrelid = 'ontorag_graph_edges'::regclass
    ) INTO has_tgt_fk;
    ...
    -- transaction holding pg_advisory_xact_lock('ontorag_pgtable_schema'), so
    ...
    DELETE FROM ontorag_graph_edges e
    WHERE NOT EXISTS (
            SELECT 1 FROM ontorag_graph_nodes n
    ...
    IF NOT has_src_fk THEN
        ALTER TABLE ontorag_graph_edges
            ADD CONSTRAINT fk_ontorag_graph_edges_src
            FOREIGN KEY (workspace, namespace, src_id)
            REFERENCES ontorag_graph_nodes (workspace, namespace, id)
            ON DELETE CASCADE;
    END IF;

    IF NOT has_tgt_fk THEN
        ALTER TABLE ontorag_graph_edges
            ADD CONSTRAINT fk_ontorag_graph_edges_tgt
...
class PGTableGraphStorage(BaseGraphStorage):
    """OntoRAG BaseGraphStorage-compatible, PostgreSQL-native graph backend.

    This is NOT an AGE / PGGraphStorage clone. It implements the OntoRAG
    BaseGraphStorage contract directly on two plain tables (ontorag_graph_nodes,
    ontorag_graph_edges) with JSONB properties and B-tree indexes — no Apache
    ...
    async def _normalize_legacy_edges(self) -> None:
        ...
        #
        # The predicate is written to match idx_ontorag_graph_edges_noncanonical's
        ...
        needs_normalize = await self._fetchval(
            """
            SELECT EXISTS (
                SELECT 1 FROM ontorag_graph_edges
            ...
            async with conn.transaction():
                ...
                for (src, tgt), group in groups.items():
                    ...
                    await conn.execute(
                        """
                        WITH deleted AS (
                            DELETE FROM ontorag_graph_edges
                            WHERE workspace = $1
                              AND namespace = $2
                        ...
                        INSERT INTO ontorag_graph_edges
                        ...
                        WHERE ontorag_graph_edges.updated_at <= $8
                        """,
                        self.workspace,
                        self.namespace,
                        old_srcs,
    ...
    async def drop(self) -> dict[str, str]:
        try:
            ...
            # _DDL either creates both fk_ontorag_graph_edges_{src,tgt} or raises
            ...
            await self._execute(
                ...
                DELETE FROM ontorag_graph_nodes
                WHERE workspace = $1 AND namespace = $2
                """,
                self.workspace,
                self.namespace,
    ...
    async def has_node(self, node_id: str) -> bool:
        row = await self._fetchrow(
            "SELECT 1 FROM ontorag_graph_nodes WHERE workspace = $1 AND namespace = $2 AND id = $3",
            self.workspace,
            self.namespace,
            node_id,
        )
    ...
    async def get_node(self, node_id: str) -> dict[str, str] | None:
        row = await self._fetchrow(
            "SELECT properties FROM ontorag_graph_nodes WHERE workspace = $1 AND namespace = $2 AND id = $3",
            self.workspace,
            self.namespace,
            node_id,
        )
    ...
    async def upsert_node(self, node_id: str, node_data: dict[str, str]) -> None:
        ...
        await self._execute(
            """
            INSERT INTO ontorag_graph_nodes (workspace, namespace, id, properties, updated_at)
            VALUES ($1, $2, $3, $4, now())
            ...
            DO UPDATE SET
                ...
                properties = ontorag_graph_nodes.properties || EXCLUDED.properties,
                updated_at = now()
            """,
            self.workspace,
            self.namespace,
    ...
    async def delete_node(self, node_id: str) -> None:
        ...
        # fk_ontorag_graph_edges_{src,tgt}. _DDL creates both or raises and
        # aborts initialize(), so the cascade is guaranteed here — see drop().
        await self._execute(
            "DELETE FROM ontorag_graph_nodes WHERE workspace = $1 AND namespace = $2 AND id = $3",
            self.workspace,
            self.namespace,
            node_id,
        )
    ...
    async def has_edge(self, source_node_id: str, target_node_id: str) -> bool:
        ...
        row = await self._fetchrow(
            "SELECT 1 FROM ontorag_graph_edges WHERE workspace=$1 AND namespace=$2 AND src_id=$3 AND tgt_id=$4",
            self.workspace,
            self.namespace,
            src,
            tgt,
    ...
    async def get_edge(
        ...
        tgt = max(source_node_id, target_node_id)
        row = await self._fetchrow(
            "SELECT properties FROM ontorag_graph_edges WHERE workspace=$1 AND namespace=$2 AND src_id=$3 AND tgt_id=$4",
            self.workspace,
            self.namespace,
            src,
            tgt,
    ...
    async def upsert_edge(
        ...
        await self._execute(
            ...
            INSERT INTO ontorag_graph_edges (workspace, namespace, src_id, tgt_id, properties, updated_at)
    ...
    async def get_node_edges(self, source_node_id: str) -> list[tuple[str, str]] | None:
        ...
        rows = await self._fetch(
            """
            SELECT src_id, tgt_id FROM ontorag_graph_edges
    ...
    async def get_all_labels(self) -> list[str]:
        rows = await self._fetch(
            "SELECT id FROM ontorag_graph_nodes WHERE workspace = $1 AND namespace = $2",
            self.workspace,
            self.namespace,
        )
    ...
    async def get_popular_labels(self, limit: int = 300) -> list[str]:
        ...
        rows = await self._fetch(
            ...
            FROM ontorag_graph_nodes n
            LEFT JOIN (
                SELECT id, COUNT(*) AS degree
                FROM (
                    SELECT src_id AS id FROM ontorag_graph_edges
                    WHERE workspace = $1 AND namespace = $2
                    UNION ALL
                    SELECT tgt_id AS id FROM ontorag_graph_edges
                    WHERE workspace = $1 AND namespace = $2
                ) sub
                GROUP BY id
            ) d ON d.id = n.id
    ...
    async def get_all_nodes(self) -> list[dict]:
        rows = await self._fetch(
            "SELECT id, properties FROM ontorag_graph_nodes WHERE workspace = $1 AND namespace = $2",
            self.workspace,
            self.namespace,
        )
        result = []
    ...
    async def get_all_edges(self) -> list[dict]:
        rows = await self._fetch(
            "SELECT src_id, tgt_id, properties FROM ontorag_graph_edges WHERE workspace = $1 AND namespace = $2",
            self.workspace,
            self.namespace,
        )
        edges = [
    ...
    async def _bfs_frontier(
        ...
        seed_row = await self._fetchrow(
            "SELECT id, properties FROM ontorag_graph_nodes "
        ...
        while frontier and depth < max_depth and len(collected) <= node_budget:
            ...
            rows = await self._fetch(
                """
                WITH nb AS (
                    SELECT tgt_id AS nid FROM ontorag_graph_edges
                    WHERE workspace = $1 AND namespace = $2 AND src_id = ANY($3)
                  UNION
                    SELECT src_id AS nid FROM ontorag_graph_edges
    ...
    async def get_knowledge_graph(
        ...
        if node_ids:
            edge_rows = await self._fetch(
                """
                SELECT src_id, tgt_id, properties FROM ontorag_graph_edges
                WHERE workspace = $1
                  AND namespace = $2
                  AND src_id = ANY($3)
                  AND tgt_id = ANY($3)
    ...
    async def get_nodes_batch(self, node_ids: list[str]) -> dict[str, dict]:
        if not node_ids:
            return {}
        rows = await self._fetch(
            "SELECT id, properties FROM ontorag_graph_nodes WHERE workspace=$1 AND namespace=$2 AND id=ANY($3)",
            self.workspace,
            self.namespace,
            node_ids,
        )
