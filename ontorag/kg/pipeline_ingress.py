...
This module deliberately does NOT import :mod:`ontorag.kg.shared_storage` —
that module imports it, to register the hub on the OntoRAG ``SyncManager``
...
from ontorag.constants import DEFAULT_MAX_UNACKED_MANUAL_RETRIES
...
class AsyncioPipelineIngress:
    ...
    Loop affinity follows OntoRAG's sync-wrapper convention (see
    ``ontorag.py``: objects created under one ``asyncio.run()`` keep working
