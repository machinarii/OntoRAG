"""
OpenSearch Storage Implementation for OntoRAG

This module provides OpenSearch-based storage backends for OntoRAG,
...
# knobs (ontorag/kg/mongo_impl.py) so behaviour stays consistent across
...
async def _verify_mirrored_id_mapping(client: AsyncOpenSearch, index_name: str) -> None:
    ...
    cross-shard pagination tiebreaker. Indices created by older OntoRAG
    ...
    if "__mirrored_id" not in props:
        raise RuntimeError(
            ...
            f"This index was likely created by an older OntoRAG release. "
...
class OpenSearchKVStorage(BaseKVStorage):
    ...
    async def finalize(self):
        ...
        buffer counts -- otherwise ``OntoRAG.finalize_storages()`` would
    ...
    async def filter_keys(self, keys: set[str]) -> set[str]:
        ...
        try:
            ...
            # create, ontorag.py) — a fail-open here lets an existing document
...
class OpenSearchDocStatusStorage(DocStatusStorage):
    ...
    async def _ensure_content_hash_mapping(self) -> None:
        ...
        Indices created by older OntoRAG releases lack content_hash entirely.
...
class OpenSearchVectorDBStorage(BaseVectorStorage):
    ...
    async def upsert(self, data: dict[str, dict[str, Any]]) -> None:
        ...
        flush, which matches the contract used by other OntoRAG storage
    ...
    async def get_by_id(self, id: str) -> dict[str, Any] | None:
        ...
        OntoRAG vector backend (see ``NanoVectorDBStorage.get_by_id``).
