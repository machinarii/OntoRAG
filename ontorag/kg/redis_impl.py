...
async def _ensure_no_eviction_policy(redis, redis_url: str, workspace: str) -> None:
    ...
    * ``volatile-*`` / ``noeviction`` — only keys carrying a TTL can be evicted
    ...
    raise StorageControlPlaneError(
        ...
        f"(maxmemory-policy='{policy}', maxmemory={maxmemory}), but OntoRAG "
        ...
        f"(or a 'volatile-*' policy), give OntoRAG its own Redis instance/db, "
...
class RedisKVStorage(BaseKVStorage):
    ...
    async def finalize(self):
        ...
        standard shutdown path (``OntoRAG.finalize_storages``) never calls
    ...
    async def _migrate_legacy_cache_structure(self):
        ...
        """
        from ontorag.utils import generate_cache_key
...
class RedisDocStatusStorage(DocStatusStorage):
    ...
    * ``…__sched:basename:{canonical}`` — canonical basename → SET of
    ...
    as OntoRAG does not support multiple server instances sharing one
    ...
    async def _rebuild_scheduling_sidecar(self) -> None:
        ...
        worker is ever inside this method at a time; OntoRAG does not support
    ...
    async def _publish_rebuilt_index(
        ...
        than an oversight. OntoRAG runs a single service, so the only writers
