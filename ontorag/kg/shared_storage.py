...
def direct_log(message, enable_output: bool = True, level: str = "DEBUG"):
    ...
    # Get the current logger level from the ontorag logger
    try:
        from ontorag.utils import logger

        current_level = logger.getEffectiveLevel()
    except ImportError:
        # Fallback if ontorag.utils is not available
...
# Later no-arg calls (e.g. OntoRAG.__post_init__) hit the `_initialized`
...
# Workspace pipeline ingress registry (see ontorag.kg.pipeline_ingress).
...
#     workspace.
# Ownership: a mailbox belongs to the workspace, not to any OntoRAG instance —
# OntoRAG.finalize_storages() must NOT touch it; only finalize_share_data(),
...
# record belongs to the workspace, not to any OntoRAG instance.
...
class _OntoRAGManager(SyncManager):
    """SyncManager plus the OntoRAG-specific server-side types."""


# Module level so the server process can import the class under both fork and
# spawn start methods.
_OntoRAGManager.register(
...
def _ensure_ingress_on_current_loop(ingress: Any, final_namespace: str) -> None:
    """Bind a single-process ingress to the caller's loop, OntoRAG-style.

    Follows the sync-wrapper convention (see ``ontorag.py``: an object built
    ...
    # its loop, so the standard OntoRAG sync-wrapper flow never hits this.
    raise RuntimeError(
        f"pipeline ingress '{final_namespace}' belongs to another event "
        "loop that has not been closed; single-process ingress objects "
        "cannot be shared across live loops (create the OntoRAG instances "
...
async def finalize_pipeline_ingress(workspace: str | None = None) -> None:
    ...
    Teardown/tests ONLY.  The ingress is shared by every OntoRAG instance of
    the workspace, so ``OntoRAG.finalize_storages()`` must never call this —
...
def _debug_log_failure(message: str, exc: Exception) -> None:
    ...
    """
    try:
        logging.getLogger("ontorag").debug("pipeline status log: %s: %r", message, exc)
    except Exception:
        pass
...
async def acquire_enqueue_reservation(
    ...
    * no capacity → :class:`~ontorag.exceptions.PipelineBackpressureError`
