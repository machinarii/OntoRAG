...
``addon_params`` is a free-form configuration dict on :class:`OntoRAG` that
...
- :class:`ObservableAddonParams` — a ``dict`` subclass that calls a callback
  whenever the contents change so the OntoRAG runtime can invalidate cached
  derived state.
...
from ontorag.constants import DEFAULT_SUMMARY_LANGUAGE
