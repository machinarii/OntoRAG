...
to :data:`ontorag.constants.DEFAULT_R_SEPARATORS`); only when a single
...
:func:`ontorag.chunk_schema.strip_internal_multimodal_markup_for_extraction`
...
``ontorag.operate._process_single_content``) and ensures the
...
from ontorag.chunk_schema import strip_internal_multimodal_markup_for_extraction
...
from ontorag.utils import Tokenizer
...
def load_chunk_separators() -> list[str]:
    ...
    :data:`ontorag.constants.DEFAULT_R_SEPARATORS` on missing / invalid
...
def trim_content_to_budget(
    ...
    *,
    ...
    :data:`ontorag.constants.DEFAULT_MAX_EXTRACT_INPUT_TOKENS`.  Only ``content``
