#!/usr/bin/env python3
"""
Qdrant Legacy Data Preparation Tool for OntoRAG
...
New Collections (with workspace_id):
    - ontorag_vdb_chunks
    - ontorag_vdb_entities
    - ontorag_vdb_relationships
...
Usage:
    python -m ontorag.tools.prepare_qdrant_legacy_data
    # or
    python ontorag/tools/prepare_qdrant_legacy_data.py

    # Specify custom workspace
    python -m ontorag.tools.prepare_qdrant_legacy_data --workspace space1

    # Process specific collection types only
    python -m ontorag.tools.prepare_qdrant_legacy_data --types chunks,entities

    # Dry run (preview only, no actual changes)
    python -m ontorag.tools.prepare_qdrant_legacy_data --dry-run
"""
...
COLLECTION_NAMESPACES = {
    "chunks": {
        "new": "ontorag_vdb_chunks",
        "suffix": "chunks",
    },
    "entities": {
        "new": "ontorag_vdb_entities",
        "suffix": "entities",
    },
    "relationships": {
        "new": "ontorag_vdb_relationships",
...
class QdrantLegacyDataPreparationTool:
    ...
    def print_header(self):
        """Print tool header"""
        print("\n" + "=" * 60)
        print("Qdrant Legacy Data Preparation Tool - OntoRAG")
        print("=" * 60)
        if self.dry_run:
