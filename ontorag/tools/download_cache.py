...
def download_tiktoken_cache(cache_dir: str = None, models: list = None):
    ...
    # Common models used by OntoRAG and OpenAI
    if models is None:
        models = [
            "gpt-4o-mini",  # Default model for OntoRAG
            "gpt-4o",  # GPT-4 Omni
            "gpt-4",  # GPT-4
            ...
            "cl100k_base",  # Default encoding for OntoRAG
        ]
...
def main():
    ...
    parser = argparse.ArgumentParser(
        prog="ontorag-download-cache",
        description="Download cache files for OntoRAG offline deployment",
...
Examples:
  ...
  ontorag-download-cache --tiktoken
  ...
  ontorag-download-cache --tiktoken --spacy-install

For more information, visit: https://github.com/machinarii/OntoRAG
        """,
    )
    ...
    parser.add_argument(
        "--version", action="version", version="%(prog)s (OntoRAG cache downloader)"
    )
    ...
    print("=" * 70)
    print("OntoRAG Offline Cache Downloader")
    print("=" * 70)
