...
`ONTORAG_*` storage selectors; never calls the LLM or embedder):

```bash
python -m ontorag.tools.kg_integrity_repair             # report
python -m ontorag.tools.kg_integrity_repair --apply     # repair
python -m ontorag.tools.kg_integrity_repair --verbose   # per-doc details
```
