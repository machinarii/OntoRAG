#!/usr/bin/env python3
"""
Offline Vector Storage (VDB) Rebuild Tool for OntoRAG

The knowledge graph and the text_chunks KV store are the authoritative data
sources in OntoRAG. If a vector storage write fails at runtime (e.g. during
...
IMPORTANT: Shut down the OntoRAG Server (and any other writers) before
running this tool.

Usage:
    ontorag-rebuild-vdb
    # or
    python -m ontorag.tools.rebuild_vdb

Configuration is read from .env / environment variables, exactly like the
OntoRAG server (ONTORAG_GRAPH_STORAGE, ONTORAG_VECTOR_STORAGE,
ONTORAG_KV_STORAGE, WORKSPACE, WORKING_DIR, EMBEDDING_* ...). The embedding
...
from ontorag.exceptions import StorageCapabilityError
...
async def rebuild_entities_vdb(
    ...
    *,
    ...
    from ontorag.operate import _truncate_vdb_content
...
async def enumerate_kv_keys(kv) -> List[str]:
    ...
    scans (same patterns as ontorag/tools/clean_llm_query_cache.py).
    ...
    if storage_name == "PGKVStorage":
        from ontorag.kg.postgres_impl import namespace_to_table_name
...
class RebuildTool:
    ...
    def resolve_storage_names(self) -> Dict[str, str]:
        return {
            "graph": os.getenv("ONTORAG_GRAPH_STORAGE", "NetworkXStorage"),
            "vector": os.getenv("ONTORAG_VECTOR_STORAGE", "NanoVectorDBStorage"),
            "kv": os.getenv("ONTORAG_KV_STORAGE", "JsonKVStorage"),
        }
    ...
    def build_embedding_func(self) -> EmbeddingFunc | None:
        ...
        """
        try:
            from ontorag.api.config import global_args
            from ontorag.api.ontorag_server import (
                create_embedding_function_from_args,
            )
        except ImportError as e:
            print(f"\n⚠️  Could not import the OntoRAG API package: {e}")
            print('   Rebuild requires the api extra: pip install "ontorag-hku[api]"')
    ...
    def build_global_config(self) -> Dict[str, Any]:
        global_config: Dict[str, Any] = {
            ...
            # Backend selection, mirroring OntoRAG._build_global_config. PG
        ...
        if max_token_size:
            try:
                from ontorag.utils import TiktokenTokenizer
    ...
    async def setup_storages(self) -> bool:
        ...
        if not self.embedding_available:
            ...
            async def _no_embedding(*_args, **_kwargs):
                raise RuntimeError(
                    ...
                    'Install the api extra: pip install "ontorag-hku[api]"'
                )
        ...
        # Namespaces and meta_fields must match OntoRAG's own storage setup
    ...
    def print_header(self):
        print("\n" + "=" * 60)
        print(f"{BOLD_CYAN}OntoRAG Offline Vector Storage Rebuild Tool{RESET}")
        print("=" * 60)
        print("\nAuthoritative sources: graph storage + text_chunks KV store")
        print("Targets: entities_vdb, relationships_vdb, chunks_vdb")
        print("\n" + "=" * 60)
        print(f"{BOLD_RED}⚠️  IMPORTANT: STOP THE ONTORAG SERVER FIRST{RESET}")
        print("=" * 60)
        print("\nThis tool drops and rewrites vector storages. Running it while")
        print("the OntoRAG Server (or any other writer) is active can corrupt")
