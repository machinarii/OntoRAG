...
Changing `ONTORAG_GRAPH_STORAGE` normally means the previously extracted
...
**Stop every OntoRAG writer first — the tool cannot check this for you.**
...
- *Table-wide, across every workspace and namespace:* the schema migration in
  ...
  `namespace` column it **DELETEs duplicate rows** from `ontorag_graph_nodes`
  and `ontorag_graph_edges` (keeping the most recently updated row per key)
...
- `a→b` and `b→a` both exist, **whether or not the payloads match**. The
  ...
  every property while changing degree and traversal. OntoRAG's own write
