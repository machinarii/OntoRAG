#!/usr/bin/env python3
"""
Diagnostic tool to check OntoRAG initialization status.

This tool helps developers verify that their OntoRAG instance is properly
...
Usage:
    ...
    await check_ontorag_setup(rag, verbose=True)

    # Run demo from command line:
    python -m ontorag.tools.check_initialization --demo
"""
...
from ontorag.base import StoragesStatus


async def check_ontorag_setup(rag_instance: OntoRAG, verbose: bool = False) -> bool:
    """
    Check if a OntoRAG instance is properly initialized.

    Args:
        rag_instance: The OntoRAG instance to check
    ...
    print("🔍 Checking OntoRAG initialization status...\n")

    # Check storage initialization status
    if not hasattr(rag_instance, "_storages_status"):
        issues.append("OntoRAG instance missing _storages_status attribute")
    ...
    if issues:
        ...
        print(
            "\n📚 Documentation: https://github.com/machinarii/OntoRAG#important-initialization-requirements"
        )
        ...
        print("✅ OntoRAG is properly initialized and ready to use!")
...
async def demo():
    ...
    from ontorag.llm.openai import openai_embed, gpt_4o_mini_complete

    print("=" * 50)
    print("OntoRAG Initialization Diagnostic Tool")
    print("=" * 50)
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Check OntoRAG initialization status")
    parser.add_argument(
    ...
    if args.demo:
        ...
        print("Or import this module and use check_ontorag_setup() with your instance")
