...
`ontorag-rebuild-vdb` restores consistency between OntoRAG's authoritative
...
OntoRAG performs multi-step writes (graph + vector storage). If a vector
...
[#2917](https://github.com/machinarii/OntoRAG/issues/2917): graph edges with no
...
select **Rebuild ALL vector storages**. Create a new `OntoRAG` instance or
...
(`ONTORAG_GRAPH_STORAGE`, `ONTORAG_VECTOR_STORAGE`, `ONTORAG_KV_STORAGE`,
