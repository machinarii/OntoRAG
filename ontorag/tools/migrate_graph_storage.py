...
--apply, though initializing the backends still creates their schema):
    ...
    python -m ontorag.tools.migrate_graph_storage --apply [--workspace WS]
...
strips the keys as endpoint identity. OntoRAG's own edge schema uses
...
def detect_reciprocal_pairs(
    ...
    though no property is lost. OntoRAG's own write path matches edges
...
async def target_slice_is_empty(target: Any) -> bool:
    ...
    the None-returning stub in ``ontorag/base.py`` belongs to
...
def main(argv: list[str] | None = None) -> None:
    ...
    parser = argparse.ArgumentParser(
        description=(
            "Migrate a OntoRAG graph between storage backends through the "
            "public storage API (Phase 1: PGGraphStorage -> "
            "PGTableGraphStorage). Stop every OntoRAG writer first. "
