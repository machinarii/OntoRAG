...
Usage (CLI — honors WORKING_DIR / WORKSPACE and the ONTORAG_* storage env vars
from ``.env``, same as the server)::

    python -m ontorag.tools.source_conflict_repair list [--limit 50] [--all]
    python -m ontorag.tools.source_conflict_repair repair \\
        --source report.docx --primary doc-abc123 [--apply]

Usage (library — for a deployment that builds its own OntoRAG)::
...
from ontorag.base import CURSOR_END, CURSOR_START, SourceConflictSummary
from ontorag.constants import ENQUEUE_SERIALIZE_LOCK_NAMESPACE
...
async def source_conflict_repair_lock(workspace: str, canonical_source_key: str):
    ...
    * **the standalone CLI is such an "other deployment"**. It calls
    ...
    from ontorag.kg.shared_storage import get_namespace_lock
...
async def repair_one_conflict(
    ...
    *,
    ...
    ``workspace`` must be the OntoRAG instance's workspace (``rag.workspace``),
...
async def verify_repair_outcome(
    ...
    """
    from ontorag.base import SourceConflict, SourceUnique
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(
        description=(
            ...
            "primary document (OntoRAG doc_status)."
        )
    )
    parser.add_argument(
        "--workspace",
