"""Paragraph Semantic Chunking for OntoRAG.

Reads a OntoRAG ``.blocks.jsonl`` sidecar — produced by any sidecar-emitting
parser (native / mineru / docling): heading-driven blocks, tables kept whole —
and produces a chunk list compatible with
:func:`ontorag.chunker.chunking_by_token_size`.
...
from ontorag.table_markup import (
...
from ontorag.utils import Tokenizer, logger


# ---------------------------------------------------------------------------
# Threshold ratios — derived from the audit-mode constants in
# ontorag/parser/docx/parse_document.py so the trade-off curves
...
# imported from :mod:`ontorag.table_markup` so the surrounding-context
...
# Mirrors ``_MD_HEADING_RE`` in ``ontorag/parser/_markdown.py`` (the same
...
def _tables_sidecar_path(blocks_path: str) -> str | None:
    ...
    Mirrors :func:`ontorag.utils_pipeline.sidecar_modality_path` naming
...
def _split_long_block(
    ...
    *,
    ...
    Mirrors :func:`ontorag.parser.docx.parse_document.split_long_block`,
...
def _is_heading_only(block: dict[str, Any]) -> bool:
    ...
    accepted ambiguity documented in ``ontorag/parser/_markdown.py`` and is
...
def _merge_small_blocks(
    ...
    *,
    ...
    :func:`ontorag.parser.docx.parse_document.merge_small_blocks`,
...
def chunking_by_paragraph_semantic(
    ...
    *,
    ...
    Signature follows the OntoRAG chunker contract — the standard
    ...
    Args:
        tokenizer: OntoRAG tokenizer (used for all token counting; matches
