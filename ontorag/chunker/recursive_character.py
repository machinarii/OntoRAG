...
output rows in the OntoRAG file-chunker schema. The splitter walks the
...
Token accounting goes through the OntoRAG :class:`Tokenizer` via the
...
:func:`ontorag.utils.enforce_chunk_token_limit_before_embedding` does the
...
from ontorag.constants import MAX_R_SEPARATOR_CHARS, MAX_R_SEPARATORS
...
def inspect_r_separators(
    ...
    (``["\\n\\n", "\\n", " ", ""]``), which is NOT
    :data:`~ontorag.constants.DEFAULT_R_SEPARATORS` — turning one into the other
...
def chunking_by_recursive_character(
    ...
    *,
    ...
    Args:
        tokenizer: OntoRAG tokenizer; used as the length function so
        ...
        separators: Cascade of split candidates. ``None`` defers to
            ...
            :data:`~ontorag.constants.MAX_R_SEPARATOR_CHARS` are dropped and a
            cascade over :data:`~ontorag.constants.MAX_R_SEPARATORS` is
