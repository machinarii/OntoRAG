...
The chunker exposed here is ``async`` because OntoRAG's
...
Caveats:
  - SemanticChunker does NOT enforce a maximum chunk size; the caller's
    ...
    :func:`ontorag.utils.enforce_chunk_token_limit_before_embedding`.
  - When ``embedding_func`` is ``None`` we log a warning and fall back to
    :func:`ontorag.chunker.chunking_by_recursive_character` — V's only
...
class _AsyncEmbeddingFuncAdapter(Embeddings):
    """Bridge a OntoRAG :class:`EmbeddingFunc` (async) to LangChain's
...
async def chunking_by_semantic_vector(
    ...
    *,
    ...
    Args:
        tokenizer: OntoRAG tokenizer (used for output token counts).
        ...
        embedding_func: OntoRAG :class:`EmbeddingFunc`. When ``None``
