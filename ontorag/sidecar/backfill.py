...
received. When a document was parsed into the OntoRAG sidecar format
...
:func:`ontorag.sidecar.writer.write_sidecar` and
:func:`ontorag.utils_pipeline.load_ontorag_document_content` build it as
...
def _load_content_blocks(blocks_path: str) -> list[tuple[str, str]]:
    ...
    Note: ``utils_pipeline.load_ontorag_document_content`` (which produces the
...
def _build_block_spans(
    ...
    ``utils_pipeline.load_ontorag_document_content``). Returns ``(merged, spans)``
