...
These helpers are shared by the OntoRAG pipeline mixin (ontorag/pipeline.py)
and by other OntoRAG methods that touch the document ingestion paths
...
from ontorag.exceptions import StorageCapabilityError, StorageRecordNotFoundError
...
def resolve_doc_status_parse_engine(
    ...
    structured ``ontorag`` format and ``legacy`` for everything else
    ...
    return (
        PARSER_ENGINE_NATIVE
        if parse_format == FULL_DOCS_FORMAT_ONTORAG
...
def compute_file_content_hash(path_str: str) -> str | None:
    ...
    Resolves the OntoRAG ``*.blocks.jsonl`` conventions used by
    ``_load_ontorag_document_content`` so the hash matches the actual
...
async def source_candidate_set_lock(workspace: str, canonical_source_key: str):
    ...
    """
    from ontorag.constants import SOURCE_CONFLICT_LOCK_NAMESPACE
    from ontorag.kg.shared_storage import get_storage_keyed_lock
...
def make_ontorag_doc_content(merged_text: str) -> str:
    """Build the ``full_docs.content`` value for ``format=ontorag`` records.

    The result has shape ``"{{LRdoc}}<merged_text>"`` — the marker prefix
    distinguishes ontorag-format full_docs from raw-format ones, and the
    body is the complete merged text from the ``.blocks.jsonl`` content
    lines so F-chunking can run identically on raw and ontorag inputs
    (the prefix is stripped at chunking time via
    ``strip_ontorag_doc_prefix``).
    """
    return f"{ONTORAG_DOC_CONTENT_PREFIX}{merged_text or ''}"


def strip_ontorag_doc_prefix(content: str | None, parse_format: str | None) -> str:
    """Return the bare body for a stored ``full_docs.content`` value.

    The ``{{LRdoc}}`` marker is stripped **only** when ``parse_format``
    indicates the record is in ontorag format.  Any other ``parse_format``
    ...
    if (
        ...
        and content.startswith(ONTORAG_DOC_CONTENT_PREFIX)
    ):
        return content[len(ONTORAG_DOC_CONTENT_PREFIX) :]
    return content or ""
...
# ---------------------------------------------------------------------------
# OntoRAG Document blocks loader
# ---------------------------------------------------------------------------


async def load_ontorag_document_content(sidecar_uri: str) -> tuple[str, str]:
    """Load OntoRAG Document blocks and return ``(merged_text, blocks_path)``.
    ...
    if resolved is None:
        raise FileNotFoundError(
            f"OntoRAG blocks file not found from sidecar uri: {sidecar_uri}"
        )
...
# :func:`ontorag.operate.extract_entities._process_single_content` so each
