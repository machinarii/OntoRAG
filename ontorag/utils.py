...
from ontorag.exceptions import ChunkBlockMatchError, EmptyTruncatedResponseError
...
def _patch_ascii_colors_console_handler() -> None:
    ...
    if getattr(ConsoleHandler, "_ontorag_patched", False):
        return
    ...
    ConsoleHandler._ontorag_patched = True  # type: ignore[attr-defined]
...
if TYPE_CHECKING:
    from ontorag.base import BaseKVStorage, BaseVectorStorage, QueryParam
...
PERFORMANCE_TIMING_LOGS = (
    os.getenv("ONTORAG_PERFORMANCE_TIMING_LOGS", "false").lower() == "true"
)
...
class OntoragPathFilter(logging.Filter):
    """Filter for ontorag logger to filter out frequent path access logs"""
...
def setup_logger(
    ...
    Args:
        ...
        add_filter: Whether to add OntoragPathFilter to the logger
        log_file_path: Path to the log file. If None and file logging is enabled, defaults to ontorag.log in LOG_DIR or cwd
...
class EmbeddingFunc:
    ...
    Args:
        ...
        max_token_size: Enable embedding token limit checking for description summarization(Set embedding_token_limit in OntoRAG)
...
class VectorStorageConsistencyError(Exception):
    ...
    and query results may be incomplete until it is rebuilt. Stop the OntoRAG
    server and run the offline rebuild tool (``ontorag-rebuild-vdb``) to restore
    consistency.
    """
...
def priority_limit_async_func_call(
    ...
    def final_dec(func):
        ...
        publish_stats = False
        shared = None  # lazily imported ontorag.kg.shared_storage module
        ...
        def _resolve_mode() -> bool:
            ...
            if concurrency_group is None:
                ...
                return True
            if shared is None:
                from ontorag.kg import shared_storage as shared_module
...
def write_json(json_obj, file_name):
    ...
    """
    from ontorag.file_atomic import atomic_write
...
class TokenizerInterface(Protocol):
    ...
    OntoRAG runs token counting in worker threads to keep it off the asyncio
...
async def _wait_deferring_cancellation(
    ...
    ``_KeyedLockContext.__aexit__`` in ``ontorag/kg/shared_storage.py``.
...
_LOOP_SEMAPHORE_ATTR_PREFIX = "_ontorag_submit_semaphore_"
...
def get_loop_semaphore(name: str, capacity: int) -> asyncio.Semaphore:
    ...
    rather than a per-``OntoRAG`` setting: this helper is module level and is
        ...
        with _CHUNKING_EXECUTOR_GUARD:
            if _CHUNKING_EXECUTOR is None:
                _CHUNKING_EXECUTOR = ThreadPoolExecutor(
                    max_workers=1, thread_name_prefix="ontorag-chunking"
                )
...
async def run_in_chunking_executor(fn: Callable[..., Any], *args: Any, **kwargs) -> Any:
    ...
    """
    from ontorag.constants import CHUNKING_SUBMIT_LIMIT
        ...
        with _STORAGE_IO_EXECUTOR_GUARD:
            if _STORAGE_IO_EXECUTOR is None:
                _STORAGE_IO_EXECUTOR = ThreadPoolExecutor(
                    max_workers=1, thread_name_prefix="ontorag-storage-io"
                )
...
async def run_in_storage_io(fn: Callable[..., Any], *args: Any, **kwargs: Any) -> Any:
    ...
    """
    from ontorag.constants import STORAGE_IO_SUBMIT_LIMIT
...
class Tokenizer:
    ...
    state, or whatever its implementation requires. OntoRAG does not serialize
    ...
    ---------------------
    An injected tokenizer must additionally survive ``copy.deepcopy``: ``OntoRAG``
    ...
    instance as ``OntoRAG.tokenizer``. That is an identity guarantee, not a
        ...
        with _TOKENIZER_EXECUTOR_GUARD:
            if _TOKENIZER_EXECUTOR is None:
                _TOKENIZER_EXECUTOR = ThreadPoolExecutor(
                    max_workers=1, thread_name_prefix="ontorag-tokenizer"
                )
...
async def run_in_tokenizer_executor(fn: Callable[..., Any], *args: Any) -> Any:
    ...
    """
    from ontorag.constants import TOKENIZER_SUBMIT_LIMIT
...
def enforce_chunk_token_limit_before_embedding(
    ...
    ``Tokenizer.split_by_token_limit`` contract (see ``ontorag.utils.Tokenizer``),
    ...
    received (the same string :mod:`ontorag.sidecar.backfill` reconstructs
    ...
    this function runs BEFORE :func:`ontorag.sidecar.backfill.backfill_chunk_sidecars`
...
async def aexport_data(
    ...
    if file_format == "csv":
        ...
        with open(output_path, "w", encoding="utf-8") as mdfile:
            mdfile.write("# OntoRAG Data Export\n\n")
        ...
        with open(output_path, "w", encoding="utf-8") as txtfile:
            txtfile.write("ONTORAG DATA EXPORT\n")
            txtfile.write("=" * 80 + "\n\n")
...
class TruncatedResponse(str):
    ...
    in ``use_llm_func_with_cache`` and ``ontorag.operate``.
    """
...
def check_storage_env_vars(storage_name: str) -> None:
    ...
    """
    from ontorag.kg import STORAGE_ENV_REQUIREMENTS
