...
- ``UI_TEMPLATES_DIR`` set ⇒ the bundle must validate completely or server
  ...
  operator would believe the customer branding is live while OntoRAG
...
# SUPPORTED_UI_LANGUAGES in ontorag_webui/src/lib/browserLanguage.ts; the two
...
class UILocaleContent:
    ...
    # line at all -- OntoRAG never asserts a customer's copyright, and never
...
def load_ui_customization_snapshot(bundle_dir: str | Path) -> UICustomizationSnapshot:
    ...
    # OntoRAG logo — customer texts with the wrong logo is a worse branding
...
def resolve_ui_customization_snapshot(
    ...
    - **``manifest.json`` present** → full validation, unchanged. Fail-fast
      ...
      bundle refuses startup rather than silently serving OntoRAG content to
