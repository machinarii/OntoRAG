...
from ontorag.utils import setup_logger, get_env_value
...
errorlog = os.getenv("ERROR_LOG", log_file_path)  # Default write to ontorag.log
accesslog = os.getenv("ACCESS_LOG", log_file_path)  # Default write to ontorag.log
...
def on_starting(server):
    ...
    # Log the location of the OntoRAG log file
    print(f"OntoRAG log file: {log_file_path}\n")
...
def post_fork(server, worker):
    ...
    setup_logger("ontorag", log_level, add_filter=True, log_file_path=log_file_path)

    # Set up ontorag submodule loggers
    for name in logging.root.manager.loggerDict:
        if name.startswith("ontorag."):
