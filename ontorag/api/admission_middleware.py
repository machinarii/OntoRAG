...
:mod:`ontorag.api.admission`) so ONE reservation covers the whole request
instead of the route taking a second one.

Degradation is deliberate: when this middleware is not installed, or cannot
resolve the OntoRAG instance, the route's own reservation still enforces the
...
class AdmissionMiddleware:
    ...
    Args:
        ...
        rag_getter: resolves the OntoRAG instance at request time. The instance
            is built after the middleware is installed, so it cannot be captured
            by value.
        api_key: the configured ``ONTORAG_API_KEY``, for the pre-auth check.
        api_prefix: mount prefix to strip before matching paths.

    The raw-body ceiling is NOT this middleware's business; it lives in
    :class:`ontorag.api.body_limit_middleware.BodyLimitMiddleware`, installed
    ...
    def _resolve_rag(self) -> Optional[Any]:
        try:
            ...
            logger.warning(
                "Admission middleware cannot resolve the OntoRAG instance "
