#!/usr/bin/env python
"""
Start OntoRAG server with Gunicorn
"""
...
# Capture this before importing OntoRAG modules, because those imports load .env.
...
def _build_global_concurrency_limits(args) -> dict:
    ...
    """
    from ontorag.llm_roles import ROLES
...
def main():
    from ontorag.api.utils_api import display_splash_screen, check_env_file
    from ontorag.api.config import global_args, initialize_config
    ...
    os.environ["ONTORAG_GUNICORN_MODE"] = "1"
    ...
    if (
        ...
        print("     ontorag-server --workers 1")
        print("=" * 80 + "\n")
        sys.exit(1)
    ...
    print("🚀 Starting OntoRAG with Gunicorn")
    ...
    class GunicornApp(BaseApplication):
        ...
        def load(self):
            ...
            from ontorag.api.ontorag_server import get_application
    ...
    if workers_count > 1:
        ...
        os.environ["ONTORAG_MAIN_PROCESS"] = "1"
