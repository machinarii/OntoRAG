...
from ontorag import ROLES
from ontorag.utils import get_env_value, logger
from ontorag.llm.binding_options import (
    BedrockLLMOptions,
...
from ontorag.base import OllamaServerInfos
import sys
...
DEFAULT_TOKEN_SECRET = "ontorag-jwt-default-secret-key!"
...
def parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(description="OntoRAG API Server")
    ...
    parser.add_argument(
        "--key",
        type=str,
        default=get_env_value("ONTORAG_API_KEY", None),
        help="API key for authentication. This protects ontorag server against unauthorized access",
    )
    ...
    parser.add_argument(
        ...
        help="Name for the simulated Ollama model (default: from env or ontorag)",
    )
    ...
    args.kv_storage = get_env_value(
        "ONTORAG_KV_STORAGE", DefaultRAGStorageConfig.KV_STORAGE
    )
    args.doc_status_storage = get_env_value(
        "ONTORAG_DOC_STATUS_STORAGE", DefaultRAGStorageConfig.DOC_STATUS_STORAGE
    )
    args.graph_storage = get_env_value(
        "ONTORAG_GRAPH_STORAGE", DefaultRAGStorageConfig.GRAPH_STORAGE
    )
    args.vector_storage = get_env_value(
        "ONTORAG_VECTOR_STORAGE", DefaultRAGStorageConfig.VECTOR_STORAGE
    )
    ...
    ollama_server_infos.ONTORAG_NAME = args.simulated_model_name
    ollama_server_infos.ONTORAG_TAG = args.simulated_model_tag
...
def initialize_config(args=None, force=False):
    ...
    which is useful for programmatic usage, testing, or embedding OntoRAG
