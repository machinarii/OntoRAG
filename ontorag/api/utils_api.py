"""
Utility functions for the OntoRAG API.
"""
...
from ontorag.api.runtime_validation import validate_runtime_target_from_env_file
...
def path_is_whitelisted(scope: Mapping[str, Any], *, mount_prefix: str = "") -> bool:
    ...
    paths written without ``ONTORAG_API_PREFIX``; matching the raw
...
def display_splash_screen(args: argparse.Namespace) -> None:
    """
    Display a colorful splash screen showing OntoRAG server configuration
    ...
    line1_text = f"OntoRAG Server v{core_version}/{api_version}"
    ...
    ASCIIColors.yellow(f"{ollama_server_infos.ONTORAG_MODEL}")
    ...
    # neither AUTH_ACCOUNTS nor ONTORAG_API_KEY set, all requests are allowed).
    ...
    reachable from this machine. Set ONTORAG_API_KEY, or AUTH_ACCOUNTS together
    ...
    Secure the server before exposing it to a network by setting at least one of:
      - ONTORAG_API_KEY=<a-strong-secret>   (X-API-Key header authentication)
    ...
    #
    # A configured ONTORAG_API_PREFIX counts as reachable regardless of the bind
    ...
    # The prefix is normalized first, because ONTORAG_API_PREFIX=/ means "no
...
def _warn_about_svg_rasterizer() -> None:
    ...
    """
    from ontorag.parser.markdown.parser import check_svg_rasterizer
