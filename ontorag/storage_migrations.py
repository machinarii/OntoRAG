"""Storage data migration helpers for :class:`OntoRAG`.

Mixed into OntoRAG and runs once at startup (``initialize_storages`` →
...
from ontorag.kg.shared_storage import get_data_init_lock
from ontorag.utils import logger, make_relation_chunk_key


class _StorageMigrationMixin:
    """Mixin that owns one-shot data migrations on :class:`OntoRAG`.

    Mixed into OntoRAG only. Relies on attributes that the main class
