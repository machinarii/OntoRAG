...
Why this lives at the package root rather than under ``ontorag/kg/``:
``ontorag.utils.write_json`` needs ``atomic_write`` to gain crash safety,
and several ``ontorag/kg/*`` modules need both. Hosting the helpers under
``ontorag/kg/`` would create a ``utils -> kg -> utils`` import cycle.
