...
class OllamaServerInfos:
    def __init__(self, name=None, tag=None):
        self._ontorag_name = name or os.getenv(
            "OLLAMA_EMULATING_MODEL_NAME", DEFAULT_OLLAMA_MODEL_NAME
        )
        self._ontorag_tag = tag or os.getenv(
            "OLLAMA_EMULATING_MODEL_TAG", DEFAULT_OLLAMA_MODEL_TAG
        )
        self.ONTORAG_SIZE = DEFAULT_OLLAMA_MODEL_SIZE
        self.ONTORAG_CREATED_AT = DEFAULT_OLLAMA_CREATED_AT
        self.ONTORAG_DIGEST = DEFAULT_OLLAMA_DIGEST

    @property
    def ONTORAG_NAME(self):
        return self._ontorag_name

    @ONTORAG_NAME.setter
    def ONTORAG_NAME(self, value):
        self._ontorag_name = value

    @property
    def ONTORAG_TAG(self):
        return self._ontorag_tag

    @ONTORAG_TAG.setter
    def ONTORAG_TAG(self, value):
        self._ontorag_tag = value

    @property
    def ONTORAG_MODEL(self):
        return f"{self._ontorag_name}:{self._ontorag_tag}"
...
class QueryParam:
    ...
    ``OntoRAG.user_prompt_prefix``) to this value. **If this field is empty,
...
class BaseVectorStorage(StorageNameSpace, ABC):
    ...
    async def upsert(self, data: dict[str, dict[str, Any]]) -> None:
        ...
        Multi-worker note:
            ...
            ontorag-gunicorn) other workers will not observe these writes
...
class BaseKVStorage(StorageNameSpace, ABC):
    ...
    async def get_by_id_strict(self, id: str) -> dict[str, Any] | None:
        ...
        * Any transport/server error, an index/collection that is not ready,
        ...
        :class:`~ontorag.exceptions.StorageCapabilityError`; callers must gate
    ...
    async def upsert(self, data: dict[str, dict[str, Any]]) -> None:
        ...
        Multi-worker note:
            ...
            deployment (e.g. ontorag-gunicorn) other workers will not
...
class BaseGraphStorage(StorageNameSpace, ABC):
    ...
    async def upsert_node(self, node_id: str, node_data: dict[str, str]) -> None:
        ...
        too):
            ...
            error on NetworkX. ``ontorag.utils.validate_graph_attributes``
            ...
            So: ``ontorag.utils.graph_attribute_value_rejection`` at the
    ...
    async def get_knowledge_graph(
        ...
        **Scope: the ``*`` whole-graph ranking.** The non-wildcard path -- BFS
        ...
        apply it: :class:`~ontorag.kg.networkx_impl.NetworkXStorage` and
        :class:`~ontorag.kg.pgtable_impl.PGTableGraphStorage` order every
        level, while ``MongoGraphStorage`` (default ``bidirectional`` mode) and
        :class:`~ontorag.kg.opensearch_impl.OpenSearchGraphStorage` rank the
...
class DocProcessingStatus:
    ...
    def from_stored(cls, data: dict[str, Any]) -> "DocProcessingStatus":
        ...
        ``scheme_name``/``multimodal_content``, other OntoRAG versions write
...
class DocStatusStorage(BaseKVStorage, ABC):
    ...
    async def count_docs_by_statuses(
        ...
        :class:`~ontorag.exceptions.StorageCapabilityError`; deployments that
    ...
    async def list_source_conflicts_page(
        ...
        *,
        ...
        :class:`~ontorag.exceptions.StorageCapabilityError`; only backends
    ...
    async def repair_source_conflict(
        ...
        * On commit, the current candidate set is re-read strictly and a
          ...
          :class:`~ontorag.exceptions.SourceConflictRepairCASError` rather
        ...
        **What this method does NOT serialize.** The storage layer holds no
        ...
        ``ontorag.tools.source_conflict_repair.source_conflict_repair_lock``
        ...
        :class:`~ontorag.exceptions.StorageCapabilityError`.
        """
        raise StorageCapabilityError(
