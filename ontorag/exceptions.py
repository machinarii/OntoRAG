...
class StorageNotInitializedError(RuntimeError):
    ...
    def __init__(self, storage_type: str = "Storage"):
        super().__init__(
            ...
            f"\n"
            f"See: https://github.com/machinarii/OntoRAG#important-initialization-requirements"
        )
...
class RecoveryAnchorMissingError(RuntimeError):
    ...
    * ``REASON_CHUNKLESS_CONTRIBUTIONS`` — the anchors exist and name KG
    ...
    (``ontorag.tools.kg_integrity_repair``), which rebuilds anchor rows from
...
class KGPurgeOperationConflictError(RuntimeError):
    ...
    plus its chunk-id set (:func:`~ontorag.utils_pipeline.make_kg_purge_operation_id`),
...
class PipelineNotInitializedError(KeyError):
    ...
    def __init__(self, namespace: str = ""):
        msg = (
            ...
            f"  2. For multi-workspace setups, each OntoRAG instance was properly initialized\n"
            f"\n"
            f"Standard initialization:\n"
            f"  rag = OntoRAG(workspace='your_workspace')\n"
            f"  await rag.initialize_storages()  # Auto-initializes pipeline_status\n"
            f"\n"
            f"If you need manual control (advanced):\n"
            f"  from ontorag.kg.shared_storage import initialize_pipeline_status\n"
...
class ChunkBlockMatchError(ValueError):
    ...
    Sidecar backfill (``ontorag.sidecar.backfill``) maps F/R/V chunks back to
    ...
    Also raised earlier, by ``ontorag.utils.enforce_chunk_token_limit_before_embedding``,
