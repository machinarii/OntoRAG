...
help:
	...
	@printf "  $(COLOR_GREEN)make apple-up$(COLOR_RESET)                Start Postgres/Neo4j/Milvus + OntoRAG on Apple 'container'\n"
	...
	@printf "  $(COLOR_GREEN)make apple-logs SVC=ontorag$(COLOR_RESET) Tail a service's logs\n"
...
# SETUP_OPTS (e.g. make apple-up SETUP_OPTS=--no-ontorag) and a service name
# via SVC (e.g. make apple-logs SVC=ontorag).
apple-up:
