...
from ontorag.utils import logger as ontorag_logger, merge_source_ids


@contextmanager
def _captured_logs(caplog, level):
    """Capture ontorag logger records.

    The ontorag logger sets ``propagate = False``, so caplog cannot see it
    unless propagation is re-enabled for the duration of the call.
    """
    original_propagate = ontorag_logger.propagate
    ontorag_logger.propagate = True
    try:
        with caplog.at_level(level, logger=ontorag_logger.name):
            yield
    finally:
        ontorag_logger.propagate = original_propagate
...
def test_merge_source_ids_does_not_log_for_normal_input(caplog):
    ...
    assert not [r for r in caplog.records if r.name == ontorag_logger.name]
