...
* T1: identical input text produces identical chunking inputs whether it
  arrives as raw or as a ontorag ``.blocks.jsonl``.
* T2: ``full_docs.content`` for ontorag carries the *full* merged text
...
* T3: ``extraction_meta["parse_format"]`` (surfaced via
  ``doc_status.metadata``) is now ``"ontorag"`` for ontorag docs —
...
def _write_ontorag_blocks(blocks_path: Path, body_paragraphs: list[str]) -> None:
    """Write a minimal valid OntoRAG ``.blocks.jsonl`` with body paragraphs."""
    lines = [
        json.dumps(
            {
                "type": "meta",
...
@pytest.mark.offline
def test_chunking_input_parity_raw_vs_ontorag(tmp_path, monkeypatch):
    """Same body text in raw and ontorag formats must reach
    ...
    async def _run():
        ...
        _write_ontorag_blocks(blocks_path, paragraphs)
        ...
        try:
            await rag_lr.apipeline_enqueue_documents(
                "",
                file_paths="parity.ontorag",
                docs_format=FULL_DOCS_FORMAT_ONTORAG,
                ontorag_document_paths="__parsed__/parity.blocks.jsonl",
                track_id="track-lr",
            )
    ...
    async def _run():
        ...
        try:
            await rag.apipeline_enqueue_documents(
                "",
                file_paths="big.ontorag",
                docs_format=FULL_DOCS_FORMAT_ONTORAG,
                ontorag_document_paths="__parsed__/big.blocks.jsonl",
                track_id="track-big",
            )

            doc_id = compute_mdhash_id("big.ontorag", prefix="doc-")
...
def test_extraction_meta_records_ontorag_parse_format(tmp_path, monkeypatch):
    ...
    async def _run():
        ...
        try:
            await rag.apipeline_enqueue_documents(
                "",
                file_paths="tag.ontorag",
                docs_format=FULL_DOCS_FORMAT_ONTORAG,
                ontorag_document_paths="__parsed__/tag.blocks.jsonl",
                track_id="track-tag",
            )
            await rag.apipeline_process_enqueue_documents()

            doc_id = compute_mdhash_id("tag.ontorag", prefix="doc-")
            status_doc = await rag.doc_status.get_by_id(doc_id)
            assert status_doc is not None
            metadata = (
