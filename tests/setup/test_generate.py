...
def test_generate_docker_compose_removes_ontorag_container_name_from_existing_output(
    tmp_path: Path,
) -> None:
    """Compose regeneration should strip fixed ontorag container names from prior output."""
    write_text_lines(
...
def test_generate_docker_compose_preserves_list_style_ontorag_environment(
    tmp_path: Path,
) -> None:
...
def test_generate_docker_compose_injects_healthchecks_and_ontorag_depends_on(
    tmp_path: Path,
) -> None:
    """Generated compose should gate OntoRAG on all managed dependencies becoming healthy."""
    write_text_lines(
        tmp_path / "docker-compose.yml",
        ["services:", "  ontorag:", "    image: example/ontorag:test"],
    )
    write_text_lines(
        tmp_path / "env.example",
        (REPO_ROOT / "env.example").read_text(encoding="utf-8").splitlines(),
...
""")
    ...
    ontorag_start = generated_compose.index("  ontorag:\n")
    embed_start = generated_compose.index("\n  vllm-embed:\n")
    ontorag_block = generated_compose[ontorag_start:embed_start]
    assert "    depends_on:" in generated_compose
    assert "    depends_on:" in ontorag_block
    for service_name in (
        "postgres",
        "neo4j",
        "mongodb",
...
def test_generate_docker_compose_repairs_misplaced_ontorag_depends_on_from_existing_output(
    tmp_path: Path,
) -> None:
    """Regeneration should move stale ontorag depends_on content back onto the ontorag service."""
    write_text_lines(
        tmp_path / "docker-compose.final.yml",
        [
            "services:",
...
""")
    ...
    ontorag_block = generated_compose[ontorag_start:rerank_start]
...
def test_generate_docker_compose_normalizes_ontorag_restart_policy_from_existing_output(
    tmp_path: Path,
) -> None:
    """Regeneration should replace legacy ontorag restart with deploy.restart_policy."""
    write_text_lines(
        tmp_path / "docker-compose.final.yml",
        [
            "services:",
...
""")
    ...
    ontorag_start = generated_compose.index("  ontorag:\n")
    ontorag_block = generated_compose[ontorag_start:]
    assert "    restart: unless-stopped" not in ontorag_block
    assert "    deploy:\n" in ontorag_block
...
def test_generate_docker_compose_normalizes_ontorag_restart_policy_without_blank_line_before_deploy(
    tmp_path: Path,
) -> None:
...
ENV_VALUES[POSTGRES_USER]="ontorag"
...
def test_generate_docker_compose_cleans_marker_and_blank_lines_when_only_ontorag_remains(
    tmp_path: Path,
) -> None:
...
def test_generate_docker_compose_keeps_the_port_mapping_inside_ontorag_before_a_top_level_section(
    tmp_path: Path,
) -> None:
    """A user-defined top-level section must not swallow the port mapping.

    The port injector closed the ontorag service only on a two-space service
    ...
    YAML, and the mapping was gone from ontorag.
    """
    write_text_lines(
        tmp_path / "docker-compose.final.yml",
        [
...
""")
    ...
    assert parsed["services"]["ontorag"]["ports"] == [
        "${HOST:-0.0.0.0}:${PORT:-9621}:9621"
    ]
    assert parsed["networks"] == {"backbone": {"driver": "bridge"}}


def test_inject_ontorag_port_mapping_closes_an_existing_ports_block_at_any_top_level_key(
    tmp_path: Path,
) -> None:
...
inject_ontorag_port_mapping "{compose_file}" "9621:9621"
""")
    parsed = yaml.safe_load(compose_file.read_text(encoding="utf-8"))
    assert parsed["services"]["ontorag"]["ports"] == ["8000:8000", "9621:9621"]
    assert parsed["configs"] == {"myconf": {"file": "./conf.txt"}}


@pytest.mark.parametrize("indent", ["", "  "], ids=["column-zero", "service-indent"])
def test_inject_ontorag_port_mapping_does_not_end_the_service_on_a_comment(
...
""")
    ...
    assert yaml.safe_load(result)["services"]["ontorag"]["ports"] == [
        "8000:8000",
        "9621:9621",
    ]
...
@pytest.mark.parametrize("indent", ["", "  "], ids=["column-zero", "service-indent"])
def test_inject_ontorag_bind_mounts_does_not_end_the_service_on_a_comment(
    tmp_path: Path, indent: str
) -> None:
    """The sibling injector shares the boundary, so it shares the defect.

    A comment between the ontorag keys and the operator's ``volumes:`` block
...
inject_ontorag_bind_mounts "{compose_file}" "./certs:/app/certs:ro"
""")
    result = compose_file.read_text(encoding="utf-8")
    assert result.count("    volumes:") == 1
    assert yaml.safe_load(result)["services"]["ontorag"]["volumes"] == [
        "./data:/app/data",
        "./certs:/app/certs:ro",
    ]
...
_strip_ontorag_wizard_ports "{compose_file}"
inject_ontorag_port_mapping "{compose_file}" '{mapping}'
""")
    result = compose_file.read_text(encoding="utf-8")
    assert yaml.safe_load(result)["services"]["ontorag"]["ports"] == [
        "80:80",
...
@pytest.mark.parametrize("indent", ["", "  "], ids=["column-zero", "service-indent"])
def test_ontorag_environment_has_key_looks_past_a_comment(
    tmp_path: Path, indent: str
) -> None:
...
if _ontorag_environment_has_key "{compose_file}" "UI_TEMPLATES_DIR"; then
  echo "declared"
else
  echo "absent"
fi
...
def test_generate_docker_compose_injects_env_overrides_into_ontorag_not_after_managed_services(
    tmp_path: Path,
) -> None:
    """Env overrides must appear inside the ontorag environment block, not after managed services.
    ...
    the ontorag service once postgres/neo4j were merged in.
    """
    compose_file = tmp_path / "docker-compose.yml"
    compose_file.write_text(
        "\n".join(
...
REPO_ROOT="{tmp_path}"
reset_state
ENV_VALUES[UI_TEMPLATES_DIR]="./ontorag_webui/ui_templates"

generate_docker_compose "$REPO_ROOT/docker-compose.final.yml\"
""")
    result = (tmp_path / "docker-compose.final.yml").read_text(encoding="utf-8")
    assert '      UI_TEMPLATES_DIR: "/app/data/ui_templates"' in result
    assert "./ontorag_webui/ui_templates" not in result
...
""")
    result = (tmp_path / "docker-compose.final.yml").read_text(encoding="utf-8")
    environment = yaml.safe_load(result)["services"]["ontorag"]["environment"]
...
read_service_environment_value "{compose_file}" "ontorag" "SSL_CERTFILE"
""")
