...
def test_generated_env_file_maps_storage_to_in_network_ips(tmp_path) -> None:
    ...
    assert values["ONTORAG_KV_STORAGE"] == "PGKVStorage"
    assert values["ONTORAG_DOC_STATUS_STORAGE"] == "PGDocStatusStorage"
...
def test_container_and_volume_names_derive_from_prefix() -> None:
    ...
    out = run_bash(
        f'export ONTORAG_AC_PREFIX=proj2-; source "{SCRIPT}"; '
...
def test_reads_db_credentials_and_milvus_db_from_env_source(tmp_path) -> None:
    ...
    # database matches what the ontorag container connects with.
    ...
    out = run_bash(
        ...
        f'export ONTORAG_AC_ENV_FILE="{envf}"; source "{SCRIPT}"; '
...
def test_image_tags_stay_in_sync_with_repo() -> None:
    ...
    assert 'IMG_PG="${ONTORAG_AC_IMG_PG:-pgvector/pgvector:pg18}"' in script_text
