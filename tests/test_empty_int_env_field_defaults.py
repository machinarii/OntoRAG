"""Empty env-backed int defaults must not crash OntoRAG class construction.

Follow-up to the COSINE_THRESHOLD fix: several ``OntoRAG`` field defaults still
...
def _import_field_default(env_key: str, env_value: str, field_name: str) -> str:
    ...
    result = subprocess.run(
        [
            ...
            f"print(OntoRAG.__dataclass_fields__[{field_name!r}].default)",
        ],
        cwd=REPO_ROOT,
        env=env,
        capture_output=True,
