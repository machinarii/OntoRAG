...
import ontorag.tools.rebuild_vdb as rebuild_vdb
from ontorag.kg.noop_vector_db_impl import NoopVectorDBStorage
from ontorag.tools.rebuild_vdb import (
...
# ontorag/tools/rebuild_vdb.py history): the AGE ``agtype::text`` cast emits
...
def single_attempt_vdb_ops(monkeypatch):
    ...
    import ontorag.utils_graph as utils_graph
...
async def test_merge_relation_vdb_delete_failure_raises_consistency_error(
    ...
    from ontorag.utils_graph import _merge_entities_impl
    ...
    assert "ontorag-rebuild-vdb" in str(excinfo.value)
...
async def test_merge_source_entity_delete_failure_raises_consistency_error(
    ...
    assert "ontorag-rebuild-vdb" in message
...
async def test_check_only_stub_carries_embedding_model_name(monkeypatch):
    ...
    import ontorag.kg.factory as kg_factory
    from ontorag.tools.rebuild_vdb import RebuildTool
...
def _runnable_tool(monkeypatch, inputs):
    ...
    import ontorag.kg.shared_storage as shared
