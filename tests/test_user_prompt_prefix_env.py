...
The loader must also never raise: it runs inside ``OntoRAG()`` through a
...
from ontorag.prompt import load_user_prompt_prefix_source
...
def _clear_prefix_env(monkeypatch):
    ...
    # The ontorag logger sets propagate=False, so caplog sees nothing without
...
def test_rejected_file_names_degrade_to_no_prefix(
    ...
    with caplog.at_level("ERROR", logger="ontorag"):
        assert load_user_prompt_prefix_source() == ""
    assert "USER_PROMPT_PREFIX_FILE" in caplog.text
...
def test_unresolvable_prompt_dir_does_not_abort_ontorag_construction(monkeypatch):
    ...
    does not merely skip the prefix -- it makes ``OntoRAG()`` unconstructible,
    ...
    factory = OntoRAG.__dataclass_fields__["user_prompt_prefix"].default_factory
    assert factory() == ""
...
def test_ontorag_field_factory_reads_the_env(monkeypatch):
