...
import ontorag.operate as operate
...
def _propagate_ontorag_logger(monkeypatch):
    """``ontorag.utils.logger`` disables propagation; restore it so caplog
    can see WARNING records emitted from inside ``ontorag.operate``."""
...
async def test_every_occurrence_still_reaches_the_server_log(
    _high_token_limit, status_messages, caplog, _propagate_ontorag_logger
):
...
class TestParseFailureAttribution:
    ...
    async def test_text_mode_undelimited_result_names_truncation(
        self, caplog, _propagate_ontorag_logger
    ):
        with caplog.at_level("WARNING", logger="ontorag"):
