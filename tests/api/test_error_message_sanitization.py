...
Importing ``ontorag.api.*`` eagerly instantiates ``AuthHandler`` (auth.py),
...
def _init_config(monkeypatch):
    """Initialize config with a clean argv so importing ontorag.api.* is safe.
...
class _FakeOntoRAG:
    """Minimal stand-in for OntoRAG so create_app touches no backend I/O."""
...
def _build_client(monkeypatch):
    ...
    app = ontorag_server.create_app(global_args)
