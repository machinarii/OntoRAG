...
``ontorag.api.utils_api._token_renewal_cache``.
...
# ``ontorag.api.config`` resolves its args lazily on first attribute access and
...
sys.argv = [sys.argv[0]]
config_module = importlib.import_module("ontorag.api.config")
utils_api = importlib.import_module("ontorag.api.utils_api")
_auth = importlib.import_module("ontorag.api.auth")
...
class TestConfiguredAccountsRespectTheSameBound:
    ...
    def test_oversized_username_error_does_not_log_the_password(
        ...
        monkeypatch.setattr(ontorag_logger, "propagate", True)
...
class TestRenewalLogSanitization:
    ...
    def test_crlf_in_subject_is_neutralized(self, monkeypatch, caplog):
        ...
        fabricated ``CRITICAL:ontorag:`` entry.
        """
        client = _client(monkeypatch, auth_configured=False, api_key=None)
        # The ontorag logger does not propagate, so caplog sees nothing by default.
        ontorag_logger = logging.getLogger("ontorag")
        monkeypatch.setattr(ontorag_logger, "propagate", True)
        injected = "alice\nCRITICAL:ontorag: SECURITY AUDIT PASSED - admin login\nbob"
...
class TestRenewalSkipUnderApiPrefix:
    ...
    and under ``ONTORAG_API_PREFIX`` no skip entry matched. The frequently-polled
    ...
    def _client(monkeypatch, api_prefix: str) -> TestClient:
        monkeypatch.setattr(utils_api, "auth_configured", False)
        server = importlib.import_module("ontorag.api.ontorag_server")
