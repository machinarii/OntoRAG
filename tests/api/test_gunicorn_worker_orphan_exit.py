...
from ontorag.api.gunicorn_worker import OntoRAGUvicornWorker
...
def _worker(ppid: int) -> OntoRAGUvicornWorker:
    ...
    """
    worker = OntoRAGUvicornWorker.__new__(OntoRAGUvicornWorker)
    worker.ppid = ppid
    worker.log = _Log()
    return worker
...
async def test_the_check_rides_the_notify_tick_and_still_notifies(monkeypatch):
    ...
    notified: list[bool] = []
    monkeypatch.setattr(
        OntoRAGUvicornWorker, "notify", lambda self: notified.append(True)
    )
    signals: list[tuple[int, int]] = []
    monkeypatch.setattr(os, "kill", lambda pid, sig: signals.append((pid, sig)))
...
def test_the_configured_worker_class_resolves(monkeypatch):
    ...
    assert load_class(gunicorn_config.worker_class) is OntoRAGUvicornWorker, (
