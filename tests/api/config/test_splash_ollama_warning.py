...
skipped the canonical multi-site deployment: with ONTORAG_API_PREFIX set, the
...
The prefix is normalized before it is believed. ONTORAG_API_PREFIX=/ (or
...
def _splash(monkeypatch, *, host: str, api_prefix: str | None) -> str:
    ...
    monkeypatch.setenv("ONTORAG_API_KEY", "an-api-key")

    argv = ["ontorag-server", "--host", host]
