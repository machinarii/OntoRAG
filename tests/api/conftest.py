...
``ontorag.api.config`` / ``auth`` / ``utils_api`` compute module-level state at
...
tries to open up auth by patching ``sys.modules["ontorag.api.utils_api"]``
...
_API_MODULE_PREFIX = "ontorag.api"


@pytest.fixture(autouse=True)
def _restore_ontorag_api_module_identity():
    """Restore ``ontorag.api.*`` entries a test replaced or removed.
