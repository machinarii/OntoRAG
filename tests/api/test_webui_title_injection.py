...
The staging approach (tmp index.html + patched `ontorag_server.__file__`)
...
STATIC_TITLE = "<title>OntoRAG</title>"
...
def _config_payload(body: str) -> dict:
    match = re.search(r"window\.__ONTORAG_CONFIG__ = (\{.*?\});", body)
...
class TestWebuiTitleInjection:
    def test_custom_title_is_published_and_applied(self, tmp_path, monkeypatch):
        ...
        assert "document.title=window.__ONTORAG_CONFIG__.webuiTitle" in body
