...
class TestCopyrightBundle:
    ...
    the manifest: there is no OntoRAG default to inherit, and no bundle text
    that turns into one.
    """
...
def _build_app(tmp_path, monkeypatch, *cli_args):
    from ontorag.api.config import parse_args, initialize_config
    ...
    monkeypatch.setattr(
        ontorag_server, "create_query_routes", lambda *_a, **_k: APIRouter()
    )
    monkeypatch.setattr(
        ontorag_server, "create_graph_routes", lambda *_a, **_k: APIRouter()
    )
    monkeypatch.setattr(ontorag_server, "OllamaAPI", _FakeOllamaAPI)
    monkeypatch.setattr(
        ontorag_server, "get_namespace_data", AsyncMock(return_value={"busy": False})
    )
    ...
    return ontorag_server.create_app(args)
...
class TestCustomizationEndpointNoBundle:
    def test_customized_false_with_brand(self, tmp_path, monkeypatch):
        ...
        # render. OntoRAG never puts its own notice on a deployment's page.
...
class TestCustomizationEndpointWithBundle:
    ...
    def test_copyright_absent_by_default(self, tmp_path, monkeypatch):
        """A bundle that declares none serves null — never OntoRAG's own."""
        data = (
            self._client(tmp_path, monkeypatch)
            .get("/ui/customization?locale=zh")
            .json()
...
class TestChromeTranslationCoverage:
    ...
    def test_startup_warns_about_a_bundle_the_chrome_cannot_follow(
        ...
        ontorag_logger.addHandler(handler)
        try:
            _build_app(tmp_path, monkeypatch)
        finally:
            ontorag_logger.removeHandler(handler)
...
class TestLogoUrlAltCoupling:
    ...
    def _app(self, snapshot):
        ...
        from ontorag.api.routers.ui_customization_routes import (
...
class TestUnpopulatedBundleDirectory:
    ...
    that a half-written bundle never silently serves OntoRAG content.
    """
