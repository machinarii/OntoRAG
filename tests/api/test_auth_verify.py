...
profile (ONTORAG_API_KEY set, no AUTH_ACCOUNTS):
...
def _pin_auth_profile(monkeypatch):
    ...
    is a singleton built from `global_args` when ontorag.api.auth is first
    ...
    reference (utils_api, ontorag_server) observes the same object.
    """
