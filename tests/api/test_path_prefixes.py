...
class TestUvicornRootPathSemantics:
    ...
    def _build_app_with_prefix(self, prefix):
        original_argv = sys.argv.copy()
        try:
            sys.argv = ["ontorag-server", "--api-prefix", prefix]
            from ontorag.api.config import parse_args
            from ontorag.api.ontorag_server import create_app
    ...
    def test_launcher_does_not_pass_root_path_to_uvicorn(self, monkeypatch, tmp_path):
        ...
        Mocks uvicorn.run and exercises ontorag_server.main() far enough
        to capture the config dict, then asserts root_path is absent.
        """
        monkeypatch.setenv("ONTORAG_API_PREFIX", "/site01")
        ...
        monkeypatch.setattr(ontorag_server, "uvicorn", MagicMock(run=fake_run))
        monkeypatch.setattr(ontorag_server, "check_env_file", lambda: True)
        monkeypatch.setattr(
            ontorag_server, "check_and_install_dependencies", lambda: None
        )
        monkeypatch.setattr(ontorag_server, "configure_logging", lambda: None)
        monkeypatch.setattr(ontorag_server, "update_uvicorn_mode_config", lambda: None)
        monkeypatch.setattr(ontorag_server, "display_splash_screen", lambda *_: None)
        with patch("ontorag.api.ontorag_server.OntoRAG") as mock_rag:
            mock_rag.return_value = MagicMock()
            # Re-parse args under the patched env so global_args picks up the prefix.
            from ontorag.api.config import parse_args, global_args as _ga
    ...
    def test_gunicorn_worker_does_not_inject_root_path(self):
        ...
        (``OntoRAGUvicornWorker`` adds the orphan check gunicorn's own workers
        ...
        from ontorag.api import gunicorn_config
...
class TestWhitelistUnderApiPrefix:
    ...
    def _default_whitelist(self, monkeypatch):
        ...
        try:
            ...
            sys.argv = [sys.argv[0]]
            from ontorag.api import utils_api
        finally:
            sys.argv = original_argv
    ...
    def test_whitelisted_routes_stay_open_under_a_prefix(
        ...
        OntoRAG that is only a MagicMock here: /health reads shared storage that
