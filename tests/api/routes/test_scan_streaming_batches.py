...
from ontorag.base import SourceAbsent  # noqa: E402
...
def _ensure_shared_storage_initialized():
    ...
    importlib.import_module("ontorag.kg.shared_storage").initialize_share_data()
    yield
...
def test_an_unusable_spool_directory_fails_the_scan_closed(
    ...
    async def _run():
        ...
        ontorag_logger = importlib.import_module("ontorag.utils").logger
        previous = ontorag_logger.propagate
        ontorag_logger.propagate = True
        try:
            with caplog.at_level("ERROR", logger=ontorag_logger.name):
                await run_scanning_process(rag, doc_manager, "track-bad-spool")
        finally:
            ontorag_logger.propagate = previous
...
def test_batch_size_falls_back_when_unconfigured(monkeypatch):
    ...
    from ontorag.constants import DEFAULT_SCAN_ENQUEUE_BATCH_SIZE
