...
_parser_routing = importlib.import_module("ontorag.parser.routing")
_parser_registry = importlib.import_module("ontorag.parser.registry")
_parser_base = importlib.import_module("ontorag.parser.base")
...
async def test_pipeline_enqueue_scan_batch_leaves_ontorag_document_docx_batch(
    tmp_path, monkeypatch
):
    monkeypatch.setenv("ONTORAG_PARSER", "docx:native")
...
def test_ontorag_document_reprocess_uses_full_docs_without_reparse():
    ...
    # All ontorag rows route to the internal reuse handler (reuse the stored
...
def test_unroutable_suffix_needs_rule_or_hint(tmp_path, monkeypatch):
    ...
    actually reaches that engine: a bare filename needs a ONTORAG_PARSER
    ...
    monkeypatch.setenv("ONTORAG_PARSER", "png:mineru")
...
def test_third_party_engine_suffixes_join_allowlist_and_scan(tmp_path, monkeypatch):
    ...
    try:
        ...
        monkeypatch.setenv("ONTORAG_PARSER", "foo:fooengine")
...
class _ClearRag:
    """Mock OntoRAG exposing the storages that clear_documents drops."""
