...
def _build_server(monkeypatch):
    ...
    Returns the ontorag_server module so callers can reach auth_handler.
    """
    ...
    for factory in (
        ...
        monkeypatch.setattr(ontorag_server, factory, lambda *_a, **_k: APIRouter())
    ...
    app = ontorag_server.create_app(args)
    return ontorag_server, app
...
def _client_with_account(monkeypatch, username="admin", password="s3cret-plain"):
    ...
    ontorag_server, app = _build_server(monkeypatch)
    monkeypatch.setattr(ontorag_server.auth_handler, "accounts", {username: password})
    return ontorag_server, TestClient(app)
...
def test_login_offloads_verification_to_thread(monkeypatch):
    ...
    costs one bcrypt).
    """
    ontorag_server, _app = _build_server(monkeypatch)
    monkeypatch.setattr(
        ontorag_server.auth_handler, "accounts", {"admin": "s3cret-plain"}
    )
    ...
    monkeypatch.setattr(ontorag_server.asyncio, "to_thread", spy)
