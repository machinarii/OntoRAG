...
from ontorag.base import QueryResult
...
def _isolate_env(monkeypatch):
    ...
    auth_module = sys.modules.get("ontorag.api.auth")
    if auth_module is not None:
        monkeypatch.setattr(auth_module.auth_handler, "accounts", {}, raising=False)
    utils_api_module = sys.modules.get("ontorag.api.utils_api")
...
def _build_client(llm_response: dict):
    ...
    try:
        ...
        with patch("ontorag.api.ontorag_server.OntoRAG", return_value=mock_rag):
            app = create_app(args)
...
async def test_canned_no_context_result_is_not_llm_generated():
    ...
    reaching a model."""
    from ontorag.operate import kg_query
