...
sys.argv = [sys.argv[0]]
_ollama_api = importlib.import_module("ontorag.api.routers.ollama_api")
_utils_api = importlib.import_module("ontorag.api.utils_api")
sys.argv = _original_argv

from ontorag.constants import (  # noqa: E402
...
class _ExplodingRAG:
    ...
    def __init__(self):
        self.ollama_server_infos = SimpleNamespace(
            ONTORAG_MODEL="ontorag:latest",
            ONTORAG_SIZE=1,
            ONTORAG_CREATED_AT="now",
            ONTORAG_DIGEST="sha",
        )
...
def test_generate_refuses_an_empty_prompt(monkeypatch, prompt):
    ...
    response = TestClient(app).post(
        "/api/generate",
        json={"model": "ontorag:latest", "prompt": prompt, "stream": False},
    )
...
def test_generate_does_not_apply_the_rag_minimum(monkeypatch):
    ...
    response = TestClient(app).post(
        "/api/generate",
        json={"model": "ontorag:latest", "prompt": "中", "stream": False},
    )
...
def test_a_malformed_body_is_still_a_400_not_a_413(client):
    ...
    response = client.post("/api/chat", json={"model": "ontorag:latest"})
    assert response.status_code == 400
