...
def _propagate_ontorag_logger(monkeypatch):
    # The ``ontorag`` logger sets ``propagate=False`` (see ontorag.utils), so
...
def test_extract_table_id_variants():
    from ontorag.table_markup import extract_table_id
...
def test_inject_header_into_table_slice_json_and_html():
    from ontorag.chunker.paragraph_semantic import _inject_header_into_table_slice
