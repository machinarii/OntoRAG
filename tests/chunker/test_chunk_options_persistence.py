...
1. **env-driven snapshot**: env vars (CHUNK_R_OVERLAP_SIZE,
   ...
   :func:`ontorag.parser.routing.default_chunker_config`, then into
...
from ontorag.constants import DEFAULT_R_SEPARATORS
...
def test_env_driven_snapshot_persisted_in_full_docs(tmp_path, monkeypatch):
    ...
    async def _run():
        ...
        try:
            # F slot — mirror what ``OntoRAG.ainsert`` does: build the
...
def test_constructor_chunk_size_overlays_addon_params(tmp_path, monkeypatch):
    """``OntoRAG(chunk_token_size=N, chunk_overlap_token_size=M)`` must
...
def test_strategy_env_wins_over_legacy_ctor_field(tmp_path, monkeypatch):
    ...
    ``OntoRAG(chunk_overlap_token_size=2)`` is the legacy
...
def test_p_strategy_defaults_to_dedicated_size_when_env_unset(tmp_path, monkeypatch):
    ...
    ``OntoRAG(chunk_token_size=…)``.  Paragraph-semantic merging needs
    ...
    from ontorag.constants import DEFAULT_CHUNK_P_SIZE
...
def test_runtime_chunker_config_is_corrected_once_and_cached(tmp_path, monkeypatch):
    ...
    """
    import ontorag.chunker.recursive_character as recursive_character
...
def test_constructor_supplied_chunker_is_bounded_without_mutating_the_caller(
    ...
    """``OntoRAG(addon_params=...)`` is a configuration ingress point too.
...
def test_a_plain_mapping_is_not_corrected_but_is_still_bounded_downstream():
    ...
    """
    from ontorag.chunker.recursive_character import normalize_r_separators
    from ontorag.parser.routing import resolve_chunk_options
...
def test_r_strategy_falls_back_to_global_chunk_size(tmp_path, monkeypatch):
    ...
    from the standard chain (here: ``OntoRAG(chunk_token_size=…)``)."""
...
def test_f_strategy_falls_back_to_top_level_size(tmp_path, monkeypatch):
    ...
    the top-level resolved size (here from ``OntoRAG(chunk_token_size=…)``) —
...
def test_chunk_opts_metadata_records_only_drop_rf():
    ...
    from ontorag.pipeline import _format_chunking_params
