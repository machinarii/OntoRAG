"""F-chunking parity between raw and ontorag formats.

After the F-chunking unification, ``apipeline_process_enqueue_documents``
strips the ``{{LRdoc}}`` marker from ontorag-format content and feeds the
...
* T1: identical input text produces identical chunking inputs whether it
  arrives as raw or as an already-parsed ontorag full_docs row pulled
  back through the parse queue on resume (ReuseParser) — the only
  production path for ontorag rows now that the ``docs_format="ontorag"``
...
* T3: ``extraction_meta["parse_format"]`` (surfaced via
  ``doc_status.metadata``) is ``"ontorag"`` for natively-parsed docs —
...
* T6: a ``pending_parse`` document that resolves to ontorag at parse
...
* T7: a raw document whose body *literally* starts with ``{{LRdoc}}``
  ...
  unconditional ``strip_ontorag_doc_prefix`` at the chunking boundary
...
from ontorag.utils_pipeline import make_ontorag_doc_content
...
def _attach_chunking_spy(rag: OntoRAG) -> dict:
...
# ---------------------------------------------------------------------------
# T1 — parity: raw vs ontorag produce identical chunking input
# ---------------------------------------------------------------------------


async def _seed_ontorag_row(rag: OntoRAG, doc_id: str, *, body: str, file_path: str):
    """Seed an already-parsed ontorag full_docs row + a PENDING doc_status.

    Mirrors how ontorag-format rows exist in production: written by the
    ...
    await rag.full_docs.upsert(
        {
            doc_id: {
                "content": make_ontorag_doc_content(body),
                "file_path": file_path,
...
@pytest.mark.offline
def test_chunking_input_parity_raw_vs_ontorag(tmp_path):
    """Same body text must reach ``chunking_func`` with byte-identical input
    whether it arrives as raw or as a resumed ontorag full_docs row."""
    ...
    async def _run():
        ...
        assert spy_lr["calls"] >= 1, "ontorag doc never reached chunking_func"
        assert spy_lr["input"] == spy_raw["input"] == expected_merged, (
            "chunking_func received different inputs for raw vs ontorag; "
            f"raw={spy_raw['input']!r}\nlr={spy_lr['input']!r}"
        )
        assert not spy_lr["input"].startswith(ONTORAG_DOC_CONTENT_PREFIX), (
...
def _stub_docx_blocks(monkeypatch, body_paragraphs: list[str]) -> None:
    ...
    persists the ontorag-format full_docs row."""
...
def test_full_docs_content_carries_full_merged_text(tmp_path, monkeypatch):
    ...
    async def _run():
        ...
        try:
            ...
            assert full_doc["content"] == ONTORAG_DOC_CONTENT_PREFIX + body
            assert full_doc.get("parse_format") == FULL_DOCS_FORMAT_ONTORAG
            assert full_doc.get("sidecar_location"), (
                "native parse must record the sidecar_location on the "
                "ontorag full_docs row"
            )
            ...
            assert not summary.startswith(ONTORAG_DOC_CONTENT_PREFIX)
        finally:
...
def test_extraction_meta_records_ontorag_parse_format(tmp_path, monkeypatch):
    ...
    async def _run():
        ...
        try:
            ...
            assert metadata.get("parse_format") == FULL_DOCS_FORMAT_ONTORAG, (
                f"doc_status.metadata.parse_format="
                f"{metadata.get('parse_format')!r}; "
                f"expected {FULL_DOCS_FORMAT_ONTORAG!r} so the multimodal "
...
class _ListHandler(logging.Handler):
    ...
    The ``ontorag`` logger has ``propagate = False`` so pytest's caplog
...
def test_explicit_selector_reports_custom_chunking_func_bypass(
    ...
    async def _run():
        ...
        ontorag_logger = logging.getLogger("ontorag")
        list_handler = _ListHandler()
        list_handler.setLevel(logging.WARNING)
        ontorag_logger.addHandler(list_handler)
        ...
        try:
            ...
            ontorag_logger.removeHandler(list_handler)
            await rag.finalize_storages()
...
def test_explicit_R_dispatches_to_recursive_character(tmp_path, monkeypatch):
    ...
    import ontorag.chunker as chunker_pkg
    from ontorag.chunker import chunking_by_recursive_character as real_r
    ...
    # The dispatcher does ``from ontorag.chunker import …`` inside the
...
def test_explicit_V_dispatches_to_semantic_vector(tmp_path, monkeypatch):
    ...
    async def _run():
        ...
        assert captured.get("embedding_func") is rag.embedding_func, (
            "dispatcher must hand the OntoRAG embedding_func to the V chunker"
        )
        assert legacy_spy["calls"] == 0, (
...
# ---------------------------------------------------------------------------
# T6 — pending_parse → ontorag summary is populated after PROCESSED
# ---------------------------------------------------------------------------
...
def test_pending_parse_ontorag_summary_populated_after_processed(
    ...
    async def _run():
        ...
        try:
            ...
            assert not final_summary.startswith(ONTORAG_DOC_CONTENT_PREFIX), (
...
def test_raw_text_starting_with_marker_chunked_verbatim(tmp_path):
    ...
    characters.  ``strip_ontorag_doc_prefix`` is a ontorag-only contract
    ...
    body_with_marker = ONTORAG_DOC_CONTENT_PREFIX + (
    ...
    async def _run():
        ...
        assert spy["input"].startswith(ONTORAG_DOC_CONTENT_PREFIX), (
