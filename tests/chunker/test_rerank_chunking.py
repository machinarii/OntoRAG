...
in ontorag/rerank.py to ensure proper document splitting and score aggregation.
"""
...
class TestChunkDocumentsForRerank:
    ...
    def test_chunking_with_character_fallback(self):
        ...
        with patch("ontorag.utils.TiktokenTokenizer", side_effect=ImportError):
            chunked_docs, doc_indices = chunk_documents_for_rerank(
                documents,
...
class TestMalformedProviderResults:
    ...
    async def test_generic_rerank_discards_malformed_items(self):
        ...
        with (
            patch("ontorag.rerank.aiohttp.ClientSession", return_value=mock_session),
            patch("ontorag.rerank.logger.warning") as mock_warning,
        ):
    ...
    async def test_generic_rerank_returns_empty_for_all_invalid_items(self):
        ...
        with patch("ontorag.rerank.aiohttp.ClientSession", return_value=mock_session):
            result = await generic_rerank_api(
                query="test",
...
class TestCohereRerankChunking:
    ...
    async def test_cohere_rerank_with_chunking_disabled(self):
        ...
        with patch(
            "ontorag.rerank.generic_rerank_api", new_callable=AsyncMock
        ) as mock_api:
