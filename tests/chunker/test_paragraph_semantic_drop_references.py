...
from ontorag.constants import DEFAULT_P_REFERENCES_HEADINGS
from ontorag.utils import Tokenizer, TokenizerInterface, logger as _ontorag_logger
...
def test_drop_references_emits_info_log(tmp_path, caplog):
    ...
    # The ontorag logger sets propagate=False, so caplog can't see it by
    # default — enable propagation for the duration of the call.
    original_propagate = _ontorag_logger.propagate
    _ontorag_logger.propagate = True
    try:
        with caplog.at_level(logging.INFO, logger=_ontorag_logger.name):
            chunking_by_paragraph_semantic(
                tokenizer,
                "",
                2000,
        ...
        _ontorag_logger.propagate = original_propagate
