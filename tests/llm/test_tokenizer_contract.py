...
* OntoRAG really does not serialize callers itself — a lock owned here would be
...
# suite reloads ontorag.utils in place, which would leave from-imported class
# references pointing at the pre-reload class.
from ontorag import utils as lr_utils
...
class _LockHoldingTokenizer:
    ...
    '_thread.lock' object``, and ``OntoRAG`` is a dataclass whose
...
def test_builtin_tokenizers_all_share_one_underlying_encoding():
    ...
    Every ``TiktokenTokenizer`` for a given model — in OntoRAG, in the Ollama
...
# ---------------------------------------------------------------------------
# OntoRAG must not serialize callers, and must not copy the tokenizer
# ---------------------------------------------------------------------------


def test_concurrent_callers_are_not_serialized_by_the_wrapper():
    """Fix-proof against reintroducing a OntoRAG-owned tokenizer lock.
...
def test_build_global_config_hands_out_the_tokenizer_itself(tmp_path):
    ...
    the same object as ``OntoRAG.tokenizer``, instead of a per-operation copy
...
def test_a_lock_based_tokenizer_is_injectable_when_it_declares_deepcopy(tmp_path):
    ...
    ``OntoRAG`` is a dataclass and ``__post_init__`` runs ``asdict`` over it, so
...
def test_tokenizer_wrapper_is_deepcopyable(tmp_path):
    ...
    Deep copies of a ``OntoRAG`` remain possible outside the hot path, so a lock
