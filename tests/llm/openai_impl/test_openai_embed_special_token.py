...
``ontorag.utils.Tokenizer.encode``.

Scope: the truncation block only runs where ``max_token_size`` actually arrives,
i.e. direct library use (``OntoRAG(embedding_func=openai_embed)``), because
...
from ontorag.llm.openai import _get_tiktoken_encoding_for_model, openai_embed
