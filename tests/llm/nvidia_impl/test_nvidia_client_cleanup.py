...
from ontorag.llm.nvidia_openai import nvidia_openai_embed
...
async def test_client_closed_after_success():
    ...
    with patch("ontorag.llm.nvidia_openai.AsyncOpenAI", return_value=fake):
...
async def test_explicit_api_key_is_request_local(monkeypatch):
    ...
    with patch(
        "ontorag.llm.nvidia_openai.AsyncOpenAI", return_value=fake
    ) as client_cls:
