...
def _reload_api_modules_if_mocked() -> None:
    """Drop cached ontorag.api entries so importlib reloads with isolated env.
...
def _setup_bedrock_app_modules(monkeypatch, args):
    """Prepare an isolated ontorag_server module for create_app tests."""
    _reload_api_modules_if_mocked()
    monkeypatch.setattr(sys, "argv", ["pytest"])
    config = importlib.import_module("ontorag.api.config")
...
async def test_create_app_bedrock_base_llm_func_passes_global_timeout(
    ...
    even if ontorag_server.py forgets to forward args.llm_timeout.
    """
    args = _make_args(tmp_path)
    ontorag_server = _setup_bedrock_app_modules(monkeypatch, args)
    ...
    with patch(
        ...
        ontorag_server.create_app(args)
        base_func = _FakeOntoRAG.last_init_kwargs["llm_model_func"]
...
async def test_bedrock_max_tokens_stop_reason_marks_result_truncated(monkeypatch):
    ...
    from ontorag.utils import is_truncated_response
