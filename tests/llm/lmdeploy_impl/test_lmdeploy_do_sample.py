...
its documented default) -- see ontorag/llm/lmdeploy.py.

Imports only from ontorag.llm.lmdeploy directly (not via
tests/extraction/test_keyword_extraction_drivers.py, which also imports
ontorag.llm.ollama at module level -- that import fails in environments
...
async def test_generation_token_limit_precedence(
    ...
    """The native setting wins over OntoRAG's generic alias."""
    captured = _install_fake_lmdeploy(monkeypatch, less_than_0_6_0=False)
