...
def _load_gemini_module(monkeypatch, request):
    ...
    # Force a fresh import of ontorag.llm.gemini against the fakes above,
    ...
    # package, and `from ontorag.llm import gemini` resolves via that
    # attribute. Both pointers must be cleared.
    parent = sys.modules.get("ontorag.llm")
    original_gemini = sys.modules.get("ontorag.llm.gemini")
    ...
    def _restore_gemini():
        if original_gemini is not None:
            sys.modules["ontorag.llm.gemini"] = original_gemini
        else:
            sys.modules.pop("ontorag.llm.gemini", None)
...
async def test_gemini_max_tokens_alias_precedence(
    ...
    """OntoRAG's generic max_tokens kwarg reaches Gemini's native config field."""
    gemini_module = _load_gemini_module(monkeypatch, request)
