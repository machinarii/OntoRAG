...
from ontorag.llm.ollama import InvalidResponseError, _ollama_model_if_cache
...
async def test_ollama_length_done_reason_marks_result_truncated():
    ...
    with patch("ontorag.llm.ollama.ollama.AsyncClient", return_value=fake_client):
...
async def test_ollama_empty_length_truncated_response_raises(
    caplog, _propagate_ontorag_logger, content
):
