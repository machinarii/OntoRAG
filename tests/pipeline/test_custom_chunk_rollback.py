...
Real OntoRAG object (JSON storages, offline), extraction monkeypatched to a
...
from ontorag.operate import KGRebuildReport
...
async def test_rollback_report_samples_are_capped(tmp_path, monkeypatch):
    ...
    rag = await _build_rag(tmp_path)
    try:
        monkeypatch.setattr(ontorag_module, "ROLLBACK_REPORT_SAMPLE_CAP", 2)
        doc_ids = ["doc-a", "doc-b", "doc-c"]
        await _seed_journaled_docs(rag, monkeypatch, doc_ids)
