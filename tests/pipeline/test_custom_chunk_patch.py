...
Drives the real OntoRAG object (JSON storages, offline) with extraction
...
import ontorag.operate as operate_module
...
async def _build_rag(tmp_path, **overrides) -> OntoRAG:
    rag = OntoRAG(
...
async def _fail_one_merge_then_restore(monkeypatch):
    """Make ontorag.ontorag.merge_nodes_and_edges raise once, then behave."""
    calls = {"n": 0}
    orig_merge = ontorag_module.merge_nodes_and_edges
...
async def test_delete_covers_journal_only_graph_candidates(tmp_path, monkeypatch):
    ...
    try:
        ...
        monkeypatch.setattr(ontorag_module, "merge_nodes_and_edges", merge_then_boom)
