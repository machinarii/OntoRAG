...
def _patch_custom_chunk_saga(monkeypatch, rag):
    """Wire a bare OntoRAG for the #3400 Phase-3 custom-chunk saga: a
...
async def test_custom_chunks_use_canonical_unknown_source_before_upsert(monkeypatch):
    ...
    monkeypatch.setattr(ontorag_module, "get_namespace_data", fake_namespace_data)
    monkeypatch.setattr(ontorag_module, "get_namespace_lock", fake_namespace_lock)
...
async def test_custom_chunks_merge_extracted_entities_into_kg(monkeypatch):
    ...
    monkeypatch.setattr(ontorag_module, "merge_nodes_and_edges", fake_merge)
    monkeypatch.setattr(ontorag_module, "get_namespace_data", fake_namespace_data)
    monkeypatch.setattr(ontorag_module, "get_namespace_lock", fake_namespace_lock)
...
async def test_process_extract_entities_surfaces_real_error_without_lock(monkeypatch):
    ...
    monkeypatch.setattr(ontorag_module, "extract_entities", boom)
...
def _custom_chunks_rag(monkeypatch, status):
    """A bare OntoRAG wired for ainsert_custom_chunks with a shared, mutable
    ...
    monkeypatch.setattr(ontorag_module, "merge_nodes_and_edges", fake_merge)
    monkeypatch.setattr(
        ontorag_module, "get_pipeline_ingress", fake_get_pipeline_ingress
    )
...
async def test_custom_chunks_resolve_failure_fails_toward_handoff(monkeypatch):
    ...
    monkeypatch.setattr(
        ontorag_module, "get_pipeline_ingress", _broken_get_pipeline_ingress
    )
