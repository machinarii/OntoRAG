...
def test_filename_hint_missing_required_endpoint_rejects(monkeypatch):
    from ontorag.parser.routing import resolve_file_parser_directives
...
def test_validate_process_options_rejects_invalid_combos():
    from ontorag.parser.routing import validate_process_options
...
def test_carry_over_keys_grouped_by_stage():
    ...
    """
    from ontorag.utils_pipeline import _DOC_STATUS_METADATA_CARRY_OVER_KEYS
...
async def _seed_pre_graph_doc_status(rag: OntoRAG, doc_id: str) -> None:
...
def test_acquire_failure_does_not_stamp_stopped_bookkeeping(tmp_path, monkeypatch):
    ...
    async def _run():
        import ontorag.pipeline as pipeline_mod
        from ontorag.kg.shared_storage import get_namespace_data
...
def test_enqueue_publish_failure_falls_back_to_auto_rescan(tmp_path, monkeypatch):
    ...
    async def _run():
        from ontorag.kg.shared_storage import get_pipeline_ingress
...
def test_enqueue_status_upsert_failure_survives_wake_failure(
    ...
    async def _run():
        ...
        try:
            ...
            # The "ontorag" logger sets propagate=False, so caplog (a root
...
def test_decide_pipeline_next_step_atomic_handoff(tmp_path):
    ...
    async def _run():
        ...
        from ontorag.pipeline import PipelineNextStep
...
def test_enqueue_rejects_removed_or_unknown_docs_format(tmp_path):
    """The 'ontorag' ingestion entrypoint was removed: enqueue accepts only
...
def test_parse_mineru_to_ontorag_document(tmp_path, monkeypatch):
    ...
    """
    from ontorag.parser.external.mineru import compute_size_and_hash
    from ontorag.parser.external.mineru.cache import current_mineru_options_signature
    from ontorag.parser.external.mineru.client import MinerURawClient
    from ontorag.parser.external.mineru.manifest import (
        Manifest,
        ManifestFile,
        write_manifest,
    )
...
def test_engine_params_survive_persist_to_full_docs(tmp_path, monkeypatch):
    ...
    """
    from ontorag.parser.external.mineru import compute_size_and_hash
    from ontorag.parser.external.mineru.cache import (
