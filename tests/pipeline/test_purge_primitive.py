...
from ontorag.constants import GRAPH_FIELD_SEP, KG_PURGE_METADATA_KEY
from ontorag.exceptions import StorageRecordNotFoundError
from ontorag.utils import compute_mdhash_id, make_relation_chunk_key
...
async def test_purge_tracking_update_is_placeholder_free_before_the_rebuild(
    ...
    monkeypatch.setattr("ontorag.ontorag.rebuild_knowledge_from_chunks", rebuild_boom)
    status, lock = _status()
