...
These lock the ``OntoRAG._insert_done`` / ``_discard_pending_index_ops`` /
...
backend flush is exercised — this is pure ontorag.py logic.
"""
...
from ontorag.exceptions import IndexFlushError
...
def _propagate_ontorag_logs():
    """The ``ontorag`` logger sets ``propagate=False``, so caplog's root
...
async def _make_rag(tmp_path) -> OntoRAG:
    rag = OntoRAG(
