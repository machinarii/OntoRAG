...
def _new_rag(tmp_path: Path, *, with_embedding: bool = True, **kwargs) -> OntoRAG:
    return OntoRAG(
...
async def _ingest(rag: OntoRAG, *, doc_id: str, process_options: str, body: str):
...
def test_v_without_an_embedding_function_does_not_hold_the_loop(monkeypatch):
    ...
    through the pipeline because ``OntoRAG`` refuses to construct without an
    ...
    from ontorag.chunker.semantic_vector import chunking_by_semantic_vector
...
def test_v_post_processing_does_not_hold_the_loop(monkeypatch):
    ...
    """
    import ontorag.chunker.recursive_character as rc_mod
    import ontorag.chunker.semantic_vector as sv_mod
...
def test_the_sidecar_backfill_does_not_hold_the_loop(tmp_path, monkeypatch):
    ...
    """
    import ontorag.sidecar as sidecar_mod
