...
from ontorag.utils import EmbeddingFunc, Tokenizer, TokenizerInterface, logger
from ontorag.utils_pipeline import apply_trusted_sentence_split_regex
...
@contextmanager
def _capturable_ontorag_logger():
    # The ontorag logger sets propagate=False, so caplog's root handler never
...
def test_empty_addon_params_falls_back_to_env_default():
    # A OntoRAG instance whose addon_params were never populated must still get
...
def test_discarding_a_snapshot_value_leaves_an_audit_line(caplog):
    ...
    with _capturable_ontorag_logger(), caplog.at_level(logging.WARNING, "ontorag"):
