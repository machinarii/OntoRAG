...
from ontorag.parser.registry import parser_specs_snapshot
from ontorag.utils import EmbeddingFunc, Tokenizer, get_content_summary
...
def _build_rag(tmp_path: Path) -> OntoRAG:
...
async def _make_ctx(rag: OntoRAG) -> _BatchRunContext:
...
async def _seed_doc_status(rag: OntoRAG, doc_id: str, *, process_options: str = ""):
    now = datetime.now(timezone.utc).isoformat()
...
async def test_parse_worker_flushes_smart_heading_cache_before_handoff(
    ...
    try:
        ...
        monkeypatch.setattr("ontorag.pipeline.get_parser", lambda *_a, **_k: _Parser())
        await ctx.parse_queues["native"].put(
...
async def _drive_process_and_collect_chunks(
    rag: OntoRAG, ctx: _BatchRunContext, doc_id: str
) -> tuple[dict, str]:
...
@pytest.mark.asyncio
async def test_process_strips_ontorag_marker_on_reread(tmp_path):
    """For a ontorag-format record, the re-read must strip the ``{{LRdoc}}``
    ...
    try:
        ...
        bare_body = "".join(f"Ontorag body line {i}. " for i in range(5))

        await rag.full_docs.upsert(
            {
                doc_id: {
                    # Stored WITH the marker, as parse persists ontorag docs.
                    "content": make_ontorag_doc_content(bare_body),
                    "file_path": f"{doc_id}.txt",
