...
All on a real OntoRAG with in-memory JSON storages (offline).
"""
...
from ontorag.base import CURSOR_END, CURSOR_START
from ontorag.pipeline import (
...
from ontorag.utils_pipeline import CUSTOM_CHUNK_PATCH_METADATA_KEY
...
async def _build_rag(tmp_path, extract, *, max_parallel_insert: int = 1) -> OntoRAG:
    rag = OntoRAG(
...
async def test_enqueue_publishes_documents_inside_status_lock(tmp_path, monkeypatch):
    ...
    import ontorag.kg.shared_storage as shared_storage_module
...
async def test_direct_delete_acquire_window_failure_hands_off(tmp_path, monkeypatch):
    ...
    try:
        ...
        real_acquire = ontorag_module.acquire_reservation
        ...
        monkeypatch.setattr(ontorag_module, "acquire_reservation", gated_acquire)
        delete_task = asyncio.create_task(rag.adelete_by_doc_id(a_id))
