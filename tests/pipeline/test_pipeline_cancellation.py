...
from ontorag.parser.exceptions import ParsePipelineCancelled
from ontorag.parser.llm_bridge import SyncLLMBridge
...
def _build_rag(tmp_path: Path, *, vlm_func=None) -> OntoRAG:
    role_configs = {}
    for spec in ROLES:
...
async def _shutdown_role_workers(rag: OntoRAG) -> None:
    ...
    for func in rag.role_llm_funcs.values():
        try:
            ...
            logging.getLogger("ontorag").warning(
                f"role worker shutdown raised during test teardown: {exc}"
            )


async def _make_ctx(rag: OntoRAG) -> tuple[_BatchRunContext, dict, Any]:
...
async def test_parse_worker_drains_queue_when_cancelled_before_start(
    ...
    try:
        ...
        monkeypatch.setattr("ontorag.pipeline.get_parser", get_parser_spy)
...
async def test_pipeline_cancel_interrupts_inflight_native_parser_llm(
    ...
    try:
        ...
        monkeypatch.setattr(
            "ontorag.pipeline.get_parser", lambda *_a, **_k: _BlockingNativeParser()
        )
...
async def test_analyze_multimodal_inflight_cancellation_polls_flag(
    ...
    try:
        ...
        # NamespaceLock used during OntoRAG initialization paths.
