...
def single_process_shared_data():
    ...
    ``OntoRAG.ainsert_custom_kg`` calls ``_raise_if_recovery_required``,
...
async def test_ainsert_custom_kg_locks_every_entity_and_endpoint(
    ...
    """
    from ontorag import ontorag as ontorag_module
    ...
    with patch.object(ontorag_module, "get_storage_keyed_lock", lock_spy):
        with pytest.raises(_LockCaptured):
