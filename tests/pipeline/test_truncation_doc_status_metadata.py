...
def _new_rag(tmp_path: Path, llm_func) -> OntoRAG:
    return OntoRAG(
...
async def _ingest(rag: OntoRAG, *, doc_id: str, body: str) -> dict:
...
def test_the_summary_marks_a_pending_row_as_carrying_stale_attempt_metadata():
    from ontorag.utils_pipeline import _DOC_STATUS_METADATA_ATTEMPT_KEYS
