...
from ontorag.tools.kg_integrity_repair import audit_kg_integrity
...
def _wire_fake_extraction(rag: OntoRAG) -> None:
...
async def _build_rag(tmp_path, workspace: str) -> OntoRAG:
    rag = OntoRAG(
...
async def _ingest(rag: OntoRAG, file_path: str = "d.txt") -> str:
...
async def _ingest_skip_kg(rag: OntoRAG, file_path: str = "nokg.txt") -> str:
...
async def _strip_write_state(rag: OntoRAG, doc_id: str) -> None:
...
async def _drop_anchors(rag: OntoRAG, doc_id: str) -> None:
...
async def _kg_snapshot(rag: OntoRAG, chunk_ids: list[str]) -> dict:
    return {
...
async def _chunk_ids(rag: OntoRAG, doc_id: str) -> list[str]:
    row = await rag.doc_status.get_by_id(doc_id)
...
def _wire_empty_extraction(rag: OntoRAG, per_chunk: bool) -> None:
