...
from ontorag.base import CURSOR_END, CURSOR_START, DocStatus
from ontorag.constants import CUSTOM_CHUNK_PATCH_METADATA_KEY
from ontorag.exceptions import PipelineRecoveryRequiredError
from ontorag.kg.shared_storage import get_namespace_data, get_pipeline_ingress
...
async def _build_rag(tmp_path) -> OntoRAG:
    rag = OntoRAG(
...
async def _status_of(rag: OntoRAG, doc_id: str) -> str:
    row = await rag.doc_status.get_by_id(doc_id)
    raw = (row or {}).get("status")
    return raw.value if isinstance(raw, DocStatus) else str(raw or "<missing>")


async def _seed_journaled_row(rag: OntoRAG, name: str, status: DocStatus) -> str:
...
async def _seed_failed_with_content(rag: OntoRAG, name: str) -> str:
...
def test_force_reset_endpoint_unblocks_the_documented_recovery_order(tmp_path):
    ...
    dr = importlib.import_module("ontorag.api.routers.document_routes")
    _sys.argv = _argv
...
def test_drain_progress_counter_resets_on_a_changing_id_set(tmp_path):
    ...
    from ontorag.pipeline import _MANUAL_DRAIN_STALL_ROUNDS, _ManualDrainProgress
