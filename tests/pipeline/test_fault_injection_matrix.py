...
simulate a process restart (finalize storages, build a FRESH OntoRAG over
...
async def _assert_converged(rag: OntoRAG, doc_id: str) -> None:
...
async def test_custom_chunk_failure_then_restart_resume_converges(
    ...
    try:
        ...
        monkeypatch.setattr(ontorag_module, "merge_nodes_and_edges", merge_boom)
...
def _wire_fake_extraction_with_relation(rag: OntoRAG) -> None:
