"""Injection point B: ``OntoRAG._persist_parsed_full_docs`` strips C0
...
(``ontorag.parser.debug.build_debug_rag``) so no real storage backends are
needed.
"""
...
async def test_persist_content_hash_matches_cleaned_body():
    ...
    expected = compute_text_content_hash(
        strip_ontorag_doc_prefix("alphabeta", FULL_DOCS_FORMAT_RAW)
    )
