"""Unit tests for the legacy engine adapter (ontorag.parser.legacy.parser).
...
import ontorag.pipeline as _pipeline
from ontorag.constants import FULL_DOCS_FORMAT_RAW
from ontorag.parser.base import ParseContext
from ontorag.parser.legacy.extractors import LegacyExtractionError
from ontorag.parser.legacy.parser import LegacyParser
...
async def test_legacy_parse_whitespace_only_extraction_raises(
    ...
    monkeypatch.setattr(
        "ontorag.parser.legacy.extractors.extract_text",
...
async def test_legacy_parse_passes_pdf_password_from_env(
    ...
    monkeypatch.setattr("ontorag.parser.legacy.extractors.extract_text", _capture)
