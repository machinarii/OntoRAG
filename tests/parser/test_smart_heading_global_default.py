...
fails fast when the switch (or a ``ONTORAG_PARSER`` rule) enables
...
of any ambient ``ONTORAG_PARSER``; ``DOCX_SMART_HEADING`` is always set or
...
def test_max_chars_low_value_warns_not_raises(monkeypatch) -> None:
    ...
    from ontorag.parser import routing
