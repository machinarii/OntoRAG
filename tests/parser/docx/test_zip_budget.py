...
from ontorag.parser.docx.parser import NativeDocxParser
from ontorag.parser.docx.zip_budget import (
...
def test_padded_member_is_rejected_by_every_legacy_ooxml_entrypoint(
    ...
    from ontorag.parser.legacy.extractors import extract_text
...
def test_budget_runs_in_the_executor_before_artifact_cleanup():
    ...
    from ontorag.parser.native_base import NativeParserBase
...
def _parse_via_real_pipeline(source_path, tmp_path, monkeypatch, doc_id):
    ...
    with mock.patch(
        "ontorag.parser.docx.parse_document.extract_docx_blocks", _stub_blocks
    ):
        return asyncio.run(
...
def test_a_failure_after_cleanup_began_still_rolls_back(tmp_path, monkeypatch):
    ...
    _benign(source_path)
    with (
        mock.patch("ontorag.parser.docx.parser.NativeDocxParser.extract", _boom),
...
def test_parser_shutdown_stops_the_worker_before_cleanup(tmp_path, monkeypatch):
    ...
    from ontorag.parser.base import ParseContext
    from ontorag.parser.debug import build_debug_rag
    from ontorag.parser.exceptions import ParseShutdown
    from ontorag.parser.registry import get_parser
