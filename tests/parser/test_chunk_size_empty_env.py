...
@pytest.mark.offline
@pytest.mark.parametrize("env_value", ["", "  "])
def test_empty_chunk_size_env_does_not_crash_ontorag_init(
    ...
    monkeypatch.setattr(
        "ontorag.ontorag.verify_storage_implementation", lambda *a: None
    )
    monkeypatch.setattr("ontorag.ontorag.check_storage_env_vars", lambda *a: None)
    ...
    monkeypatch.setattr(
        "ontorag.ontorag.get_storage_class", lambda *a, **k: DummyStorage
    )
...
def test_malformed_chunk_size_envs_raise_in_ontorag_init(
    env_key: str, tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    """Malformed non-empty env values must raise ValueError on OntoRAG init."""
