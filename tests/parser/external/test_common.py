"""Tests for shared helpers in ``ontorag/parser/external/``.
...
from ontorag.parser.external import (
    Manifest,
    ManifestFile,
    clear_dir_contents,
...
async def test_download_timeout_uses_stdlib_asyncio_on_311_plus(monkeypatch):
    ...
    from ontorag.parser.external._common import download_timeout

    monkeypatch.setattr(
        "ontorag.parser.external._common.sys.version_info", (3, 11, 0, "final", 0)
    )
...
async def test_download_timeout_uses_async_timeout_backport_below_311(monkeypatch):
    ...
    monkeypatch.setattr(
        "ontorag.parser.external._common.sys.version_info", (3, 10, 0, "final", 0)
    )
    result = download_timeout(5.0)
...
async def test_stream_capped_get_sends_accept_encoding_identity():
    ...
    from ontorag.parser.external._common import stream_capped_get
