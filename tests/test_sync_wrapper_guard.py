...
``OntoRAG``'s synchronous wrappers (``insert``, ``query``,
``delete_by_entity`` …) all delegate to :func:`ontorag.ontorag._run_sync`,
...
storages were initialized on (``OntoRAG._owning_loop``).  Two misuse modes
break this:
...
* Driven from a different — but still alive — loop, e.g.
  ...
  the shared ``asyncio.Lock`` objects in ``ontorag.kg.shared_storage`` to the
...
from ontorag.ontorag import _run_sync
