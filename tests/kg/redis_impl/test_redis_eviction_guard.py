...
def _reset_eviction_cache():
    ...
    fixture error.
    """
    from ontorag.kg import redis_impl
...
def fake(monkeypatch):
    instance = FakeRedis()
    monkeypatch.setattr(
        "ontorag.kg.redis_impl.RedisConnectionManager.get_pool",
        lambda redis_url: MagicMock(name="fake_pool"),
    )
    monkeypatch.setattr(
        "ontorag.kg.redis_impl.Redis", lambda connection_pool=None, **_: instance
    )
    return instance


def _doc_status_storage(workspace: str = "evict-ws"):
    from ontorag.kg.redis_impl import RedisDocStatusStorage
