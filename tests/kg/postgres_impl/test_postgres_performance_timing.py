...
def _read_performance_timing_flag() -> bool:
    ...
    ``ontorag.utils`` to a brand-new object in the shared, in-place module,
    silently breaking ``isinstance`` identity for the many modules that do
    ``from ontorag.utils import X`` — most visibly the LLM providers' shared
    ...
    env parsing while leaving ``sys.modules['ontorag.utils']`` — and everyone's
    by-value imports — untouched.
    """
    spec = importlib.util.spec_from_file_location(
        "ontorag.utils", utils_module.__file__
    )
...
def test_performance_timing_logs_reads_new_env_only(monkeypatch):
    monkeypatch.setenv("ONTORAG_DOC_QUERY_TIMING_LOGS", "false")
    monkeypatch.setenv("ONTORAG_PERFORMANCE_TIMING_LOGS", "true")
    assert _read_performance_timing_flag() is True


def test_performance_timing_logs_ignores_old_env(monkeypatch):
    monkeypatch.setenv("ONTORAG_DOC_QUERY_TIMING_LOGS", "true")
    monkeypatch.setenv("ONTORAG_PERFORMANCE_TIMING_LOGS", "false")
