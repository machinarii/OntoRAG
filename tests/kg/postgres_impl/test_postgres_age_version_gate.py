...
def _capture_gate_logs(caplog, level=logging.DEBUG):
    """Capture records emitted on the ``ontorag`` logger.

    ``ontorag/utils.py`` sets ``propagate = False`` on that logger, so caplog --
...
async def test_gate_stays_armed_for_non_truthy_override_values(monkeypatch, value):
    ...
    ``ontorag.utils.get_env_value``, outside this gate -- which is exactly why the
