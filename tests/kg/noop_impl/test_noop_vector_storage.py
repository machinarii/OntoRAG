...
from ontorag.kg import STORAGE_IMPLEMENTATIONS, STORAGES
...
from ontorag.utils import EmbeddingFunc, compute_mdhash_id
...
async def test_noop_vector_storage_contract_never_embeds() -> None:
    ...
    with pytest.raises(RuntimeError, match="ontorag-rebuild-vdb"):
...
@pytest.mark.offline
@pytest.mark.asyncio
async def test_graph_only_ontorag_initializes_without_embedding_func(tmp_path) -> None:
    rag = OntoRAG(
...
async def test_graph_only_query_reports_vector_storage_failure(tmp_path) -> None:
    ...
    assert "ontorag-rebuild-vdb" in result["message"]
    await rag.finalize_storages()
