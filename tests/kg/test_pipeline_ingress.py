...
import ontorag.kg.shared_storage as shared_storage
import ontorag.kg.pipeline_ingress as pipeline_ingress
...
def test_finalize_storages_never_touches_ingress():
    """Ownership guard: the ingress is workspace-shared, so no OntoRAG
    instance teardown path may finalize it (only finalize_share_data /
    explicit workspace teardown / tests)."""
    ontorag_src = (
        Path(shared_storage.__file__).parent.parent / "ontorag.py"
    ).read_text(encoding="utf-8")
    assert "finalize_pipeline_ingress" not in ontorag_src
