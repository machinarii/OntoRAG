...
import ontorag.kg.opensearch_impl
...
def patch_namespace_lock():
    ...
    with patch("ontorag.kg.opensearch_impl.get_namespace_lock", side_effect=factory):
        yield
...
class TestBulkBatchLimits:
    ...
    def test_resolve_non_positive_disables_and_warns(self):
        ...
        with patch.dict("os.environ", env, clear=True):
            with patch("ontorag.kg.opensearch_impl.logger") as mock_logger:
                payload, upserts, deletes = _resolve_bulk_batch_limits()
...
class TestKVStorageBatching:
    ...
    async def test_kv_drop_discards_buffers_and_serialises_with_flush(
        ...
        with patch.object(ClientManager, "get_client", return_value=mock_client):
            with patch("ontorag.kg.opensearch_impl.helpers.async_bulk", new=slow_bulk):
                s = self._make(global_config, embed_func)
                await s.initialize()
                await s.upsert({"k1": {"content": "x"}})
                await s.delete(["k2"])
...
class TestGraphStorage:
    ...
    async def test_migrate_edges_logs_progress_for_large_scan(
        ...
        with (
            patch("ontorag.kg.opensearch_impl._EDGE_MIGRATION_PROGRESS_INTERVAL", 2),
            patch(
                "ontorag.kg.opensearch_impl.helpers.async_bulk",
                new=AsyncMock(return_value=(0, [])),
            ),
            patch("ontorag.kg.opensearch_impl.logger", fake_logger),
        ):
            await s._migrate_edges_to_canonical_id_if_needed()
...
class TestVectorStorageBatching:
    ...
    def test_extract_bulk_failed_ids_classifies_by_status(self):
        from ontorag.kg.opensearch_impl import _extract_bulk_failed_ids
...
class TestEmbeddingBatchNumDiagnosis:
    ...
    (ontorag/operate.py). ``EMBEDDING_BATCH_NUM`` only slices the items
...
class TestGraphReadContract:
    ...
    async def test_isolated_backfill_sorts_by_label_not_shard_order(
        ...
        with patch.object(
            ontorag.kg.opensearch_impl, "_shard_doc_supported", shard_doc_supported
        ):
            with patch.object(ClientManager, "get_client", return_value=mock_client):
                s = self._make(global_config, embed_func)
                await s.initialize()
