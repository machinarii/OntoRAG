...
from ontorag.kg.faiss_impl import FaissVectorDBStorage  # noqa: E402
from ontorag.kg import write_seq  # noqa: E402
from ontorag.kg.write_seq import WRITE_SEQ_FIELD  # noqa: E402
from ontorag.kg.shared_storage import (  # noqa: E402
    initialize_share_data,
    finalize_share_data,
)
from ontorag.utils import EmbeddingFunc  # noqa: E402
...
def _frozen_clock(stamp: int):
    ...
    """
    with patch("ontorag.kg.faiss_impl.time.time", return_value=stamp):
        yield
...
async def test_reload_warns_on_index_meta_skew(tmp_path, caplog):
    ...
    # The ontorag logger sets propagate=False (ontorag/utils.py), so caplog —
    # which attaches to root by default — never sees its records. Flip propagate
    # for the duration of the reload, then restore.
    caplog.clear()
    old_propagate = ontorag_logger.propagate
    ontorag_logger.propagate = True
    try:
        with caplog.at_level(logging.WARNING, logger="ontorag"):
            reader = _make_storage(tmp_path, embed)
            await reader.initialize()
    finally:
        ontorag_logger.propagate = old_propagate
