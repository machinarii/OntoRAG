...
def make_storage(global_config=None, namespace=GRAPH_NAMESPACE):
    ...
    from ontorag.kg.pgtable_impl import PGTableGraphStorage
...
async def test_initialize_forwards_configured_vector_storage_verbatim(
    ...
    with (
        patch(
            "ontorag.kg.postgres_impl.ClientManager.get_client",
            new=AsyncMock(return_value=db),
        ) as get_client,
        patch(
            "ontorag.kg.pgtable_impl.get_data_init_lock",
...
def test_ddl_adds_cascading_edge_foreign_keys():
    ...
    from ontorag.kg.pgtable_impl import _DDL

    assert "PRIMARY KEY (workspace, namespace, id)" in _DDL
    assert "PRIMARY KEY (workspace, namespace, src_id, tgt_id)" in _DDL
    assert "fk_ontorag_graph_edges_src" in _DDL
    assert "fk_ontorag_graph_edges_tgt" in _DDL
    assert "FOREIGN KEY (workspace, namespace, src_id)" in _DDL
    assert "REFERENCES ontorag_graph_nodes (workspace, namespace, id)" in _DDL
    assert "ON DELETE CASCADE" in _DDL
    assert "DELETE FROM ontorag_graph_edges e" in _DDL
...
def test_ddl_gates_orphan_sweep_behind_missing_foreign_keys():
    ...
    sweep = fk_block.index("DELETE FROM ontorag_graph_edges e")
    add_src_fk = fk_block.index("ADD CONSTRAINT fk_ontorag_graph_edges_src")
    ...
    assert _DDL.count("DELETE FROM ontorag_graph_edges e") == 1
...
def test_normalize_guard_predicate_matches_the_partial_index():
    ...
    idx_ontorag_graph_edges_noncanonical if it can prove the query predicate
    ...
    from ontorag.kg.pgtable_impl import _DDL, PGTableGraphStorage

    predicate = 'src_id COLLATE "C" > tgt_id COLLATE "C"'

    assert "idx_ontorag_graph_edges_noncanonical" in _DDL
...
def test_postgres_impl_loads_pgvector_lazily_for_vector_connections():
    ...
    src = (
        Path(__file__).parents[3] / "ontorag" / "kg" / "postgres_impl.py"
    ).read_text()
...
async def test_upsert_node_merges_properties():
    ...
    assert "ontorag_graph_nodes.properties || EXCLUDED.properties" in sql
...
async def test_get_knowledge_graph_wildcard_uses_flat_limited_query():
    ...
    async def fetch_rows(sql, *args):
        normalized_sql = " ".join(sql.split()).upper()
        if "FROM ONTORAG_GRAPH_NODES" in normalized_sql:
...
async def test_get_popular_labels_includes_isolated_nodes():
    ...
    assert "ontorag_graph_nodes" in sql  # ranks from node table, not edges alone
...
async def test_batch_limits_come_from_the_shared_pg_resolver(monkeypatch):
    ...
    from ontorag.kg.postgres_impl import _resolve_pg_batch_limits
