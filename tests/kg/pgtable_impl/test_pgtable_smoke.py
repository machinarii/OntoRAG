...
PGTableGraphStorage when ONTORAG_GRAPH_STORAGE happens to point at it), with the
...
async def test_initialize_dedupes_before_rebuilding_pk(store):
    ...
    await store.db.execute(
        "ALTER TABLE ontorag_graph_nodes DROP CONSTRAINT ontorag_graph_nodes_pkey"
    )
    for tag in ("older", "newer"):
        await store._execute(
            "INSERT INTO ontorag_graph_nodes "
    ...
    rows = await store._fetch(
        "SELECT id FROM ontorag_graph_nodes "
...
async def test_orphan_sweep_still_runs_when_foreign_keys_are_missing(store):
    ...
    async def _count_fks(conn):
        return await conn.fetchval(
            ...
            "WHERE conrelid = 'ontorag_graph_edges'::regclass "
            "AND conname IN ('fk_ontorag_graph_edges_src', "
            "'fk_ontorag_graph_edges_tgt')"
        )
...
# ONTORAG_GRAPH_STORAGE, so it only exercises PGTableGraphStorage when a
...
async def test_escaped_entity_ids_round_trip_across_every_path(store):
    ...
    node_payloads = {
        ...
        backslash_id: _kg_node(
            backslash_id,
            r"Windows path C:\Program Files\OntoRAG\bin",
