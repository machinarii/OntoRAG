...
from ontorag.kg.milvus_impl import MilvusVectorDBStorage, _escape_milvus_str
...
def patch_namespace_lock() -> Generator[
    ...
    with patch("ontorag.kg.milvus_impl.get_namespace_lock", side_effect=factory):
        yield cache
...
class TestGetVectorsByIdsPaging:
    ...
    async def test_record_cap_binds_at_small_dimension(self) -> None:
        ...
        with patch("ontorag.kg.milvus_impl.MILVUS_QUERY_MAX_RECORDS_PER_BATCH", 5):
    ...
    async def test_returns_buffered_portion_on_page_failure(
        ...
        previous_propagate = ontorag_logger.propagate
        ontorag_logger.propagate = True  # caplog hooks the root logger
        try:
            with caplog.at_level(logging.ERROR, logger="ontorag"):
                with patch(
                    "ontorag.kg.milvus_impl.MILVUS_QUERY_MAX_RECORDS_PER_BATCH", 2
                ):
                    vectors = await s.get_vectors_by_ids(ids)
        finally:
            ontorag_logger.propagate = previous_propagate
...
class TestGetByIdsPaging:
    ...
    async def test_escapes_special_characters_across_pages(self) -> None:
        ...
        with patch("ontorag.kg.milvus_impl.MILVUS_QUERY_MAX_RECORDS_PER_BATCH", 2):
...
class TestOversizePageBisection:
    ...
    async def test_oversize_metadata_page_is_bisected_and_returns_every_row(
        ...
        with patch("ontorag.kg.milvus_impl.MILVUS_QUERY_MAX_RECORDS_PER_BATCH", 8):
    ...
    async def test_non_size_error_is_not_bisected(self) -> None:
        ...
        ids = [f"srv-{i}" for i in range(8)]
        with patch("ontorag.kg.milvus_impl.MILVUS_QUERY_MAX_RECORDS_PER_BATCH", 4):
            results = await s.get_by_ids(ids)
    ...
    async def test_overflow_surviving_to_a_single_id_returns_no_partial_rows(
        ...
        try:
            with caplog.at_level(logging.ERROR, logger="ontorag"):
                with patch(
                    "ontorag.kg.milvus_impl.MILVUS_QUERY_MAX_RECORDS_PER_BATCH", 8
                ):
                    results = await s.get_by_ids(ids)
        finally:
            ontorag_logger.propagate = previous_propagate
