"""``QueryParam`` and ``OntoRAG`` are plain dataclasses: field ORDER is public API.
...
- ``OntoRAG``: inserting mid-class shifted 55 of 72 constructor parameters, so
...
def _ontorag_init_params() -> list[str]:
    """Introspect the signature rather than constructing OntoRAG.
    ...
    """
    return list(inspect.signature(OntoRAG.__init__).parameters)[1:]  # drop self


def test_legacy_ontorag_init_order_is_unchanged():
    params = _ontorag_init_params()
    assert params[: len(_LEGACY_ONTORAG_INIT_ORDER)] == _LEGACY_ONTORAG_INIT_ORDER
...
def test_ontorag_positional_anchors_did_not_shift():
    ...
    assert params[len(_LEGACY_ONTORAG_INIT_ORDER) - 1] == "_storages_status"
